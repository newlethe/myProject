...
<html>
	<head>
		...
		window.onload=function(){
			...
			var selectPid = document.getElementById("addrPid");
			
			var pidArr = USERPIDS.split(",");
			var pNameArr = USERPNAMES.split(",");
			for (i=0; i<pidArr.length; i++) {
				var objOption=document.createElement("OPTION");   
				objOption.value=pidArr[i];   
				objOption.text=pNameArr[i];   
				selectPid.add(objOption);
				if(pidArr[i] == CURRENTAPPID)
					selectPid.options[i].selected='selected';
			}
				switch(selectPid.getAttribute('mSty')){
					case 'redLine':
						mySelect.Create(selectPid,'redLine');
						break;
					case 'blueCircle':
						mySelect.Create(selectPid,'blueCircle',true);
						break;
					case 'orangeHeart':
						mySelect.Create(selectPid,'orangeHeart',true);
						break;
				}
		}
		...
		function changePid(){
			var selectPid = document.getElementById("addrPid");
			var pid = selectPid.value;   
	        var pname = selectPid.options[selectPid.selectedIndex].text;  
			if(pid!=CURRENTAPPID) {
				commonUtilDwr.changeCurrentAppPid(pid, pname, function() {
					CURRENTAPPID = pid;
					CURRENTAPPNAME = pname;
				});
				setTimeout(function(){window.frames["contentFrame"].location.reload();}, 300);
			}
		}
		</script>
	</head>
	<style>
			...
			.current-c{background-image: url(<%=path%>/jsp/res/images/bullet_star.png) !important;color:#FFC66F !important;}
	...
	<script type='text/javascript' src='<%=request.getContextPath()%>/dwr/interface/systemMgm.js'></script>
		<script type='text/javascript' src='<%=request.getContextPath()%>/dwr/interface/appMgm.js'></script>
	...
	<body>
		...
		<script type="text/javascript" src="<%=request.getContextPath()%>/extExtend/Ext.ux.Notification.js"></script>
			...
			<%--<div class="topNav">
			    <ul>
					...
					<% for(int i=0;i<list.size();i++){ %>
						<li><a id="<%=list.get(i).getPowerpk()%>" href="javascript:loadSecondNavi('<%=list.get(i).getPowerpk()%>', '<%=list.get(i).getUrl()%>')"><span><%=list.get(i).getPowername()%></span></a></li>
					<% }%>
			    </ul>
		  	</div>
		--%>
		...
		<div id="pidList" class="name">
	   		<form name="goodjob" action="" method="get">
				<select id="addrPid" name="addr" style="display:none" mSty="blueCircle" onchange="changePid();">
				</select>
			</form>
		</div>
	</body>
