...
<html>
	...
	<style>
			...
			.current-c{background-image: url(<%=path%>/jsp/res/images/bullet_star.png) !important;color:#FFC66F}
		...
		<%--	<div class="topNav">
			    <ul>
					...
					<% for(int i=0;i<list.size();i++){ %>
						<li><a id="<%=list.get(i).getPowerpk()%>" href="javascript:loadSecondNavi('<%=list.get(i).getPowerpk()%>', '<%=list.get(i).getPowername()%>')"><span><%=list.get(i).getPowername()%></span></a></li>
					<% }%>
			    </ul>
		  	</div>
		  	--%>
