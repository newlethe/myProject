...
String imagePath = path + "/login/gf/images";
String indexNewsId = request.getParameter("newsid") == null ? "" : request.getParameter("newsid");
String pageNum = request.getParameter("page") == null ? "" : request.getParameter("page");
%>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<title>您好，欢迎您访问<%=Constant.DefaultModuleRootName%></title>
...
body{font-family:"宋体";margin:0;padding:0; background:#fff;}
...
a.red:link,a.red:visited,a.red:active{color:#f00;text-decoration: none;}
a.red:hover{color:#E9967A;text-decoration: none;}

#top{margin:0 auto; width:980px; height:20px; line-height:20px;}
#top #topdate{float:right;}
#logo{margin:0px auto; width:980px; height:100px;}

#login{margin:0 auto; width:980px; height:32px; line-height:32px;color:#fff;background:#C01913;text-align:right;}
#login .LoginInput{border:none;font-size:12px; font-weight:bold; margin-top:3px;padding:5px 2px;width:110px;}
#login .LoginSubmit{
	border:none; 
	background:url(<%=imagePath%>/anniu.gif) no-repeat 0px 1px;
	height:26px;
	width:88px;
	margin-right:10px;
}
#login .LoginSubmit:hover{
	...
	border:none;
	margin-right:10px;
	background:url(<%=imagePath%>/anniu-red.gif) no-repeat 0px 1px;
}

#bigimg{margin:5px auto; width:980px; height:280px;}

#main{margin:0 auto; width:980px;}
#main #left{float:left;width:700px; background: #fff;}

#newsClassList{
	background:#C01913; display:block; width:690px; height:26px; line-height:26px; color:#fff;
}
#newsClassList b{color:#C01913; float:left; text-align:center; display:block; margin:3px 20px 0 20px; height:23px;line-height:23px; width:70px; background:#fff;}
#newsClassList a{color:#fff; float:left; font-weight: 100;margin:3px 20px 0 20px; height:23px;line-height:23px;}
#newsClassList a:hover{color:#FFBB00;}

#newsList{width:660px; padding:0 10px;}
...
#newsListPage{background: #fff ;width:690px; height:36px; line-height:36px; color:#000; text-align: center;}
#newsListPage a{color:#000; padding:4px 5px; border:#fff solid 1px; margin:0 5px;}
#newsListPage a:hover{color:#fff; background:#C01913;}
#newsListPage span{color:#555;padding:4px 5px; border:#fff solid 1px; margin:0 5px;}
...
.right{float:right;}
.right a{color:#fff;font-weight:100;margin-right:5px;}
.right a:hover{color:#FFBB00;font-weight:100;margin-right:5px;}
.newsPart01{width:340px; float:left; margin:0 10px 10px 0; }
.newsPart02{width:280px; float:left; margin:0 0 10px 0; }
.title01{background:url('<%=imagePath%>/hongse.png') no-repeat; display:block; width:330px; height:26px; line-height:26px; color:#fff; font-weight:bold; padding-left:10px;}
.title02{background:url('<%=imagePath%>/hongse.png') no-repeat; display:block; width:270px; height:26px; line-height:26px; color:#fff; font-weight:bold; padding-left:10px;}
...
function gotoPageFun(){
	var page = $("gotoPage").value;
	location.href = "<%=basePath%>?newsid=<%=indexNewsId%>&page="+page+"#news";
}
...
-->
	...
	public String getPropertyCodeByName(String typeName) {
		String newsClassSql = "SELECT c.property_code,c.property_name FROM property_code c "
				+ " WHERE c.type_name =	(SELECT t.uids FROM property_type t "
				+ " WHERE t.type_name = '"+typeName+"') ORDER BY c.item_id";
		List newsClassList = JdbcUtil.query(newsClassSql);
		StringBuilder htmlStr = new StringBuilder();
		if (newsClassList != null && newsClassList.size() > 0) {
		htmlStr.append("<span class=\"title02\">");
		htmlStr.append(typeName);
		htmlStr.append("</span>");
		htmlStr.append("<ul>");
			for (int i = 0; i < 9; i++) {
				if(i >= newsClassList.size()){
					...
					Map newsClassMap = (Map) newsClassList.get(i);
					String newsClassUrl = newsClassMap.get("property_code").toString();
					String newsClassName = newsClassMap.get("property_name").toString();
					htmlStr.append("<li><a href=\"");
					htmlStr.append(newsClassUrl);
					htmlStr.append("\" target=\"_blank\">");
					htmlStr.append(newsClassName);
					htmlStr.append("</a></li>");
				}
			}
		htmlStr.append("</ul>");
	...
	public List getNewsClass() {
		String newsClassSql = "SELECT c.property_code,c.property_name FROM property_code c "
				+ " WHERE c.type_name =	(SELECT t.uids FROM property_type t "
				+ " WHERE t.type_name = '新闻通知分类') ORDER BY c.item_id";
		List newsClassList = JdbcUtil.query(newsClassSql);
		return newsClassList;
	}

	public String showMoreNewsList(String newsclass, Integer pageNum,
			String pagePath) {
		Integer pageSize = 20;
		StringBuilder htmlStr = new StringBuilder();
		List newsClassList = getNewsClass();
		htmlStr.append("<div id=\"newsClassList\">");
		htmlStr.append("<span class=\"right\"><a href=\"/frame\">[返回]</a></span>");
		if (newsClassList != null && newsClassList.size() > 0) {
			for (int i = 0; i < newsClassList.size(); i++) {
				Map newsClassMap = (Map) newsClassList.get(i);
				String newsClassId = newsClassMap.get("property_code")
						.toString();
				String newsClassName = newsClassMap.get("property_name")
						.toString();
				if (newsclass.equals(newsClassId)) {
					htmlStr.append("<b>");
					htmlStr.append(newsClassName);
					htmlStr.append("</b>");
				} else {
					htmlStr.append("<a href=\"?newsid=");
					htmlStr.append(newsClassId);
					htmlStr.append("#news\">");
					htmlStr.append(newsClassName);
					htmlStr.append("</a>");
				}
		...
		Integer totalNumber;
		String totalSql = "select * from app_news t where "
				+ " t.status=1 and t.newsclass='" + newsclass + "'";
		List list = JdbcUtil.query(totalSql);
		totalNumber = list.size();

		htmlStr.append("<div id=\"newsList\">");
		htmlStr.append(showNewsList(newsclass, true, pageNum, pageSize));
		htmlStr.append("</div>");
		htmlStr.append("<div id=\"newsListPage\">");
		htmlStr.append(showPage(totalNumber, pageSize, pageNum, pagePath, newsclass));
		htmlStr.append("</div>");

		return htmlStr.toString();
	}

	public String showPage(Integer totalNumber, Integer pageSize,
			Integer pageNum, String pagePath, String newsclass) {
		StringBuilder htmlStr = new StringBuilder();
		htmlStr.append("<a href=\"");
		htmlStr.append(pagePath);
		htmlStr.append("?newsid=");
		htmlStr.append(newsclass);
		htmlStr.append("#news\">首页</a>");
		Integer totalPage = 0;
		...
		String prev = "<a href=\"" + pagePath + "?newsid="+newsclass+"&page="
				+ (pageNum - 1) + "#news\">上一页</a>";
		if (pageNum <= 1)
			prev = "<span>上一页</span>";
		htmlStr.append(prev);
		String next = "<a href=\"" + pagePath + "?newsid="+newsclass+"&page="
				+ (pageNum + 1) + "#news\">下一页</a>";
		if (pageNum >= totalPage)
			next = "<span>下一页</span>";	
		htmlStr.append(next);
		
		htmlStr.append("<span>共");
		htmlStr.append(totalPage);
		htmlStr.append("页/当前第");
		htmlStr.append(pageNum);
		htmlStr.append("页</span>");
		//htmlStr.append("<span>跳转至<input type=\"text\" size=\"3\" maxlength=\"3\">页</span>");
		htmlStr.append("<span>跳转至<select id=\"gotoPage\" onchange=\"gotoPageFun(this.value)\">");
		for (int i = 1; i <= totalPage; i++) {
			htmlStr.append("<option ");
			if(i == pageNum)htmlStr.append("selected");
		...
		htmlStr.append("</select>页</span>");

		return htmlStr.toString();
	}

	public String showNewsClass() {
		List newsClassList = getNewsClass();
		StringBuilder htmlStr = new StringBuilder();
		if (newsClassList != null && newsClassList.size() > 0) {
			for (int i = 0; i < newsClassList.size(); i++) {
				...
				String newsClassName = newsClassMap.get("property_name")
						.toString();
				htmlStr.append("<div class=\"newsPart01\">");
				htmlStr.append("<span class=\"title01\"><span class=\"right\">");
				htmlStr.append("<a href=\"?newsid=");
				htmlStr.append(newsClassId);
				htmlStr.append("\">更多...</a></span>");
				htmlStr.append(newsClassName);
				htmlStr.append("</span>");
				htmlStr.append(showNewsList(newsClassId, false, 0, 0));
				htmlStr.append("</div>");
			}
		}
		return htmlStr.toString();
	...
	public String showNewsList(String newsclass, Boolean page, Integer pageNum,
			Integer pageSize) {
		String whereSql = "select t.uids,t.title,to_char(t.createtime, 'YYYY-MM-DD') createtime,t.newsclass "
				+ " from app_news t where t.status=1 and t.newsclass='"
				+ newsclass + "' order by t.createtime desc";
		if (page) {
			whereSql = "SELECT x.* FROM (SELECT a.*,ROWNUM r FROM (" + whereSql
					+ ") a ) x ";
			if (pageNum != null && pageNum >= 0) {
				if (pageNum == 0)
					pageNum = 1;
				Integer first = pageSize * (pageNum - 1) + 1;
				Integer max = pageSize * pageNum;
				whereSql += "WHERE x.r >= " + first + " AND x.r <= " + max;
			}
		}
		...
		for (int i = 0; i < listNum; i++) {
			...
			if (i >= list.size()) {
				...
				if (m.get("createtime") != null) {
					String str = (String) m.get("createtime");
					try {
						pubDate = sdf.parse(str);
					} catch (ParseException e) {
						e.printStackTrace();
					}
					long milliseconds = date.getTime() - pubDate.getTime();
					long days = milliseconds / (long) (1000 * 60 * 60 * 24);
					if (days < 7)
						color = "red title";
				}
				htmlStr.append("<li><span class=\"right\">&nbsp;&nbsp;[");
				htmlStr.append(m.get("createtime"));
				htmlStr.append("]</span><div class=\"title\"><a href=\"javascript:openPubInfoFile(\'");
				htmlStr.append(m.get("uids"));
				htmlStr.append("\')\" class=\"");
				htmlStr.append(color);
				htmlStr.append("\')\" title=\"");
				htmlStr.append(m.get("title"));
				htmlStr.append("\">");
				htmlStr.append(m.get("title"));
				htmlStr.append("</a></div></li>");
				n++;
			}
		}
		htmlStr.append("</ul>");
...
<div id="top"><span id="topdate"></span><a href="#" onclick="this.style.behavior='url(#default#homepage)';this.setHomePage('<%=basePath%>')">设为首页</a> | <a href="#" onclick="window.external.AddFavorite('<%=basePath%>', '山西国峰煤电责任有限公司')">加入收藏</a></div>
<div id="logo"><img src="<%=imagePath%>/logo.png" width="500" height="100" /></div>
<div id="login">
  <form name="memberLogin" action="#" method="post">
用户名：<input name="username" type="text" class="LoginInput" id="username"  />
密码：<input name="password" type="password" class="LoginInput" id="password"  />
  <input name="submit" type="button" class="LoginSubmit" value=" "  onclick="doLogin();"/>
  </form>
</div>
<script type="text/javascript">
       var imgNum=5;//首页展示图片的数量
        var htmlStr='<div id="bigimg">'+
	     '<div id=idContainer2 class=container>'+
	     '<table id=idSlider2 border=0 cellSpacing=0 cellPadding=0><tr>';
       for(var i=1;i<=imgNum;i++){
    	   htmlStr+='<td class=td_f><IMG src="<%=imagePath%>/datu0'+i+'.jpg" width="980" height="280"></td>';
       }
       htmlStr+='</tr></table><ul id=idNum class=num></ul></div></div>';
...
<div id="left">
	<%
		if (null != indexNewsId && !"".equals(indexNewsId)) {
			Integer pageInt = 1;
			try {
			...
			out.println(showMoreNewsList(indexNewsId, pageInt, basePath));
		} else {
			out.println(showNewsClass());
		}
	%>
</div>
<div id="right">
	<div class="newsPart02">
    	<span class="title02">重要活动</span>
    	<!--焦点图开始-->
		<script type="text/javascript">
		<!--
		var focus_width=280
		...
		<%
			//查询已发布的前5条新闻纪录
			String whereSgccSql = "select * from sgcc_attach_list l1,"
					+ " (select transaction_id, min(upload_date) minDate from "
					+ " sgcc_attach_list where transaction_id in (select uids from "
					+ " (select * from app_news where uids in(select  an.uids from app_news an, "
					+ " sgcc_attach_list sal where an.status = 1 AND upper(SAL.FILE_NAME) LIKE '%.JPG' "
					+ " and sal.transaction_id = an.uids) "
					+ " order by createtime desc) where rownum<=10) group by transaction_id ) l2 "
					+ " where l1.transaction_id = l2.transaction_id and l1.upload_date = l2.minDate "
					+ " order by l1.transaction_id desc";
			List listSgcc = JdbcUtil.query(whereSgccSql);
			String linksStr = "";
			String picsStr = "";
			String textsStr = "";
			if (listSgcc != null && listSgcc.size() > 0) {
				for (int i = 0; i < listSgcc.size(); i++) {
					...
					String suffixName = fileNameStr.substring(
							fileNameStr.lastIndexOf(".") + 1,
							fileNameStr.length());
					if (!suffixName.equalsIgnoreCase("jpg"))
						continue;//只能flash焦点图只能显示jpg格式图片

					List listNews = JdbcUtil
							.query("select title from app_news where uids='"
									+ uids + "' order by createtime desc");
					...
					picsStr += "|" + basePath
							+ "/Business/newsManage/guoj/news/upload/"
							+ picName;
					linksStr += "|" + basePath
							+ "/login/gf/news.login.jsp?uids=" + uids;
					textsStr += "|" + title;
				}
				//构建flash焦点图需要的参数
				if (!picsStr.equals("")) {
					picsStr = picsStr.substring(1);
					linksStr = linksStr.substring(1);
					textsStr = textsStr.substring(1);
				}
			}%>   
		var pics = "<%=picsStr%>";
		var links = "<%=linksStr%>";
		...
		document.write('<param name="allowScriptAccess" value="sameDomain"><param name="movie" value="<%=imagePath%>/focus.swf"><param name="quality" value="high"><param name="bgcolor" value="#dddddd" >');
		document.write('<param name="menu" value="false"><param name=wmode value="opaque">');
		document.write('<param name="FlashVars" value="pics='+pics+'&links='+links+'&texts='+texts+'&borderwidth='+focus_width+'&borderheight='+focus_height+'&textheight='+text_height+'">');
		document.write('<embed src="<%=imagePath%>/focus.swf" wmode="opaque" FlashVars="pics='+pics+'&links='+links+'&texts='+texts+'&borderwidth='+focus_width+'&borderheight='+focus_height+'&textheight='+text_height+'" menu="false" bgcolor="#dddddd" quality="high" width="'+ focus_width +'" height="'+ swf_height +'" allowScriptAccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />');
		document.write('</object>');
		//-->
		</script>
    </div>
	<div class="newsPart02">
		<%=getPropertyCodeByName("常用链接") %>
		<div style="height:10px;background:#fff;"></div>
		<%=getPropertyCodeByName("成员单位") %>
    </div>
</div>
<div id="foot">
	<p>Copyright &copy; 2011 山西国峰煤电有限责任公司 版权所有  晋ICP备05038150号</p>
	<p>地址：山西省汾阳市三泉镇尚文村  邮编：032200</p> 
</div>
...
var ndate = d.getYear()+"年"+(d.getMonth()+1)+"月"+d.getDate()+"日&nbsp;&nbsp;"+week;
topdate.innerHTML= ndate;


function openPubInfoFile(uids){
	var CONTEXT_PATH = '<%= path %>';
	//新闻详情窗口居中显示参数WLeft,WTop
	var WLeft = Math.ceil((window.screen.width-850)/2);   
	var WTop = Math.ceil((window.screen.height-600)/2);	
	
	var winUrl = CONTEXT_PATH+"/login/gf/news.login.jsp?uids="+uids;
	window.open(winUrl, '新闻详情', 'width=850px, height=600px, top='+WTop+', left='+WLeft+', toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no');
}
</script>
...
<SCRIPT>
	var imgLength = 0;
		...
		for (var i = 0; i < tag.length; i++) {
			if (tag[i].className.indexOf("td_f") != -1) {
				imgLength++;
			}
		}
	...
	var st = new SlideTrans("idContainer2", "idSlider2", imgLength, { Vertical: false });
