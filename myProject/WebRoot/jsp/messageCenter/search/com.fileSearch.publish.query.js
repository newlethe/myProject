...
Ext.onReady(function() {
	...
	if ( record.data.fileUnitName == data ){
			...
			// 所有工具栏按钮数组
			var toolbarItems = [updateInfoBtn, '-',markAsReadBtn,markAllReadBtn,'->',
											 dateSearchCombo,
...
function onItemClick(item) {
	switch (item.id) {
		...
		case 'mark-all-read':
			var remain=0;//未读的数量
			DWREngine.setAsync(false);
            ComFileManageDWR.getModuleUrlByUserModouleId(FUN_ID, USERID, function(url) {
            	if(url){
...
function markAllAsRead(){
	ComFileManageDWR.markAllAsReadForUser(USERID, selectNode.id,function(retVal) {
				if (retVal) {
					Ext.example.msg('操作提示', '状态标记成功!');
					dsResult.reload();
				} else {
