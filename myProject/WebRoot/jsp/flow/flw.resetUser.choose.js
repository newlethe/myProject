...
var cgetUsers = new Ext.Button({
	...
	handler: function(){
		...
		if (records.length > 0){
			var record = Ext.getCmp('resetGrid').getSelectionModel().getSelected();
			record.set('tonode', records[0].get('userid'))
			record.set('toname', records[0].get('realname'))
			cuserWindow.hide();
		}
	}
})
