...
Ext.onReady(function(){
	...
	var appletTpl = new Ext.XTemplate(
		...
		'name="configApplet" width="700" height="400" ',
    ...
    configPanel = new Ext.form.FormPanel({
    	...
    	items: [
    		new Ext.form.FieldSet({
    			...
    			layout: 'form',
    			items: [
    				{xtype: 'textfield', name: 'name', fieldLabel: '节点名称', allowBlank: false, anchor: "89%"},
    				{
    					xtype: 'combo', 
    					name: 'bifurcate', 
    					fieldLabel: '节点分裂', 
						...
						store:new Ext.data.SimpleStore({
							fields: ['k', 'v'],
							data: [['S','普通'],['BALL','无条件分裂'],['BSOLE','选择性分裂']]
						})
    				},{
    					xtype: 'combo', 
    					name: 'merge', 
						...
						store:new Ext.data.SimpleStore({
							fields: ['k', 'v'],
							data: [['S','普通'],['MAND','无条件合并'],['MOR','选择性合并']]
						})
    				},{
    					xtype: 'combo', 
    					name: 'istopromoter', 
						...
						items: [{
								anchor: "89%", layout: 'form', border: false,
								items: [{
    			...
    			items: [
    				{xtype: 'textfield', name: 'cnodeid', fieldLabel: '状态点ID', hidden: true, hideLabel: true},
    	...
    	bbar: ['<div id="prompt" style="padding-left: 0px;"></div>', '->', saveBtn]
    });
...
function findFlowNode(cnodeid){
	for (var i = 0; i < parent._allFlowNode.length; i++) {
		if (parent._allFlowNode[i].cnodeid == cnodeid)
			return i;
	}
	return null;
}
...
function checkNextBtn(){
	...
	if (_figures.length > 0 && _figures.length == parent._allFlowNode.length + 2){
		return true;
	}
	return false;
}
...
function saveFlwNode(){
	...
	if (_form.isValid() && roleGrid.getSelectionModel().getCount() > 0){
		var _flowNode = new FlowCommonNode();
		var _arrRole = getRoles();
		_flowNode.cnodeid = _form.findField('cnodeid').getValue();
		_flowNode.nodeid = parent.B_NODEID;
		_flowNode.flowid = parent.FLOW_ID;
		...
		_flowNode.bifurcate = _form.findField('bifurcate').getValue();
		_flowNode.merge = _form.findField('merge').getValue();
		_flowNode.istopromoter = _form.findField('istopromoter').getValue();
		var _index = findFlowNode(_flowNode.cnodeid);
		(null != _index) ? parent._allFlowNode[_index] = _flowNode : parent._allFlowNode.push(_flowNode);
		
		document.applets[0].saveTaskFigure(_flowNode.name, _flowNode.realname, '');
		if (checkNextBtn()) {
			parent.flwCommonPanel.getBottomToolbar().items.get('next').enable();
...
function doAction(strNodeInfo){
	...
	if (nodeInfo[3].split(':')[1] == '1'){
		var _form = configPanel.getForm();
		_form.reset();
		setBtnEnable(true);
		var cnodeid = nodeInfo[0].split(':')[1];
		var _index = findFlowNode(cnodeid);
		if (null != _index){
			//从数据库中检测是否已经定义
			DWREngine.setAsync(false);
			baseMgm.findById("com.sgepit.frame.flow.hbm.FlwCommonNode", cnodeid, function(obj){
			  if(obj){
					_form.loadRecord(new NodeRecord(Ext.apply(obj,parent._allFlowNode[_index])));
					selRole();
					selSelected(_form.findField('role').getValue());
			  }
			});
		} else {
			_form.findField('cnodeid').setValue(nodeInfo[0].split(':')[1]);
