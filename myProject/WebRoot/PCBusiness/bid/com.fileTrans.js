...
//	    maxSize: 400,
	...
	dsResult = new Ext.data.GroupingStore({
            ...
            proxy: new Ext.data.HttpProxy({
  				url: CONTEXT_PATH + '/servlet/PcBidServlet'
  			}),
    ...
    dsResult.on("beforeload",function(ds1){
    	Ext.apply(ds1.baseParams ,{ 
			ac : 'getJsonStrForTransToZLSByType',
			yjrName : USERNAME,
			fileId : fileId,
			type:type
		})
...
function yjzlFun(){
	...
	if(records.length>0 && zlSortSelected!=""){
		if(confirm("将选择的资料移交到【"+zlSortNameSelected+"】分类下,请确认!")){
			...
			PCBidService.transToZLSByType(CURRENTAPPID, USERDEPTID, REALNAME, type, fileLshs.substr(1,fileLshs.length-1),
						fileNames.substr(1,fileNames.length-1), fileIds.substr(1,fileIds.length-1), zlSortSelected, function(dat){
				if(dat){
					dsResult.reload();
...
function cxyj(lsh){
	if(confirm("将收回移交的资料,请确认!")){
		PCBidService.cancelTrans(lsh,function(dat){
			if(dat){				
				dsResult.reload();
