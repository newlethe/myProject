...
var acceptTbdocGrid;
var acceptTbdocDs;
var acceptTbdocBeanName = "com.sgepit.pcmis.bid.hbm.PcBidAcceptTbdocAndBond";
var plantInt;
var bidContentId = null; //招标内容主键
var bidFileBusinessType = "PCBidFile";
var acceptTbdocOhterBusinessType = "PCBidAcceptTbdocOther";
var tbUnitStore, tbUnitCombo;
var curDeletedAcceptTbdocId;
...
//							}
	...
	// 是否缴纳投标保证金
	var payBondStore = new Ext.data.SimpleStore({
				fields : ['k', 'v'],
				data : [[0, '未交纳'], [1, '已交纳']]
			});

	var payBondCombo = new Ext.form.ComboBox({
				triggerAction : 'all',
				mode : 'local',
				value : 0,
				lazyRender : true,
	...
	var cm = new Ext.grid.ColumnModel({
				columns : [new Ext.grid.RowNumberer(),sm, {
							...
							renderer : function(value, metadata, record,
								...
								var count=0;
								DWREngine.setAsync(false);
						        db2Json.selectData("select count(file_lsh) as num from sgcc_attach_list where transaction_id='"+record.data.uids+"' and transaction_type='"+bidFileBusinessType+"'", function (jsonData) {
							...
							renderer : function(value, metadata, record,
								...
								var count=0;
								DWREngine.setAsync(false);
						        db2Json.selectData("select count(file_lsh) as num from sgcc_attach_list where transaction_id='"+record.data.uids+"' and transaction_type='"+acceptTbdocOhterBusinessType+"'", function (jsonData) {
							    var list = eval(jsonData);
							    if(list!=null){
							   	 count=list[0].num;
							     		 }  
								...
								return '<a href="javascript:showUploadWin(\''
										+ acceptTbdocOhterBusinessType
										+ '\', '
										+ !disableBtn
										+ ', \''
										+ record.data.uids
										+ '\', \'其它资料\',\''+acceptTbdocBeanName+'\'  )">'
										+ downloadStr + '</a>'
							}
						}, {
							id : 'isPayBond',
	...
	acceptTbdocDs = new Ext.data.Store({
		baseParams : {
			ac : 'list', // 表示取列表
			bean : acceptTbdocBeanName,
			business : "baseMgm",
	...
	acceptTbdocDs.on('beforeload', function(store, options) {
	...
	var gridLabel = new Ext.form.Label({
				html : '<b>接收投标文件及投标保证金</b>',
				cls : 'gridTitle'
			});

	acceptTbdocGrid = new Ext.grid.EditorGridTbarPanel({
				region : 'center',
				ds : acceptTbdocDs, // 数据源
				cm : cm, // 列模型
				sm : sm,
			...
			//	header : true, //
				...
				deleteHandler : function() {
					...
					if (selSm.getCount() > 0) {
						curDeletedAcceptTbdocId = sm.getSelected().data.uids;
	...
	acceptTbdocGrid.on('beforeinsert', function() {
	...
	acceptTbdocGrid.on('afterdelete', function() {
				if (curDeletedAcceptTbdocId) {
					deleteAcceptTbdocAttachment();
				}
			});
			
	formPanel.setTitle('接收投标文件及投标保证金-工作进度信息');
	var mainPanel = new Ext.Panel({
				layout : 'border',
				tbar : bidApplySelectBar,
				items : [formPanel, acceptTbdocGrid]
			});
	...
	if(acceptTbdocGrid.getTopToolbar().items.get('del'))	
		acceptTbdocGrid.getTopToolbar().items.get('del').disable();
	
	sm.on('rowselect', function(sm, idx, r) {
		if(acceptTbdocGrid.getTopToolbar().items.get('del'))	
			acceptTbdocGrid.getTopToolbar().items.get('del').enable();				
	});
...
function saveTbUnit(){
	...
	if(records&&records.length>0){
		...
		DWREngine.setAsync(false);
		PCBidDWR.savePcBidAcceptTbdocAndBondByTbUnits(currentPid,bidContentId,rateStatus,isPayBond,tbUidsArray,function(uidsArr){
		...
		if (!doExchange) {
			...
			var updArr = new Array();
			//参数说明:业务bean，业务id数据，是否立即发送，发送单位，接收单位
			PCBidDWR.excDataZbForSave(acceptTbdocBeanName,updArr,uidsAfterInsert,false, currentPid, defaultOrgRootID,function(flag){	
			})				
		}
		DWREngine.setAsync(true);//清空全选框表头
		if( gridTbUnit.getEl()){
	...
	if( gridTbUnit.getEl()){
		...
		dsResultTbUnit.on("beforeload", function(ds1) {
					Ext.apply(ds1.baseParams, {
								...
								tbUnitType:'PcBidAcceptTbdocAndBond'
							})
				});	
...
function loadProgressForm() {
	if (bidContentId == null || bidContentId == '') {
			...
			formPanel.setTitle('接收招标文件及投标保证金-工作进度信息');
			return;
	}
	DWREngine.setAsync(false);
	PCBidDWR.getCurrentPhaseProgress(bidContentId, progressType, function(
				...
				+ '】接收招标文件及投标保证金-工作进度信息'+"     "+"<a style='cursor:hand;text-decoration:underline' onclick='clickByFormText(\""+ uids+ "\")'>"+"移交文件"+"</a>");						
			});
	DWREngine.setAsync(true);
}
...
//	if (contentId != null && contentId != '')
		acceptTbdocDs.reload();
}

function deleteAcceptTbdocAttachment() {
	var bizTypes = new Array();
	bizTypes.push(bidFileBusinessType, acceptTbdocOhterBusinessType);
	PCBidDWR.deleteZbAttachment(bizTypes, curDeletedAcceptTbdocId, function(
					retVal) {
				curDeletedAcceptTbdocId = null;
			});
}
...
function onItemClick(item) {
	switch (item.id) {	
		case 'transfer' :
			var selRecords = acceptTbdocGrid.getSelectionModel().getSelections();
...
function yjzls(filePk) {
	var rtn = window
			.showModalDialog(
					CONTEXT_PATH
							+ "/PCBusiness/bid/com.fileTrans.jsp?type=PcBidAcceptTbdocAndBond&fileId="
							+ filePk,
					null,
					"dialogWidth:1000px;dialogHeight:500px;center:yes;resizable:yes;Minimize=yes;Maximize=yes");
	if (rtn) {
