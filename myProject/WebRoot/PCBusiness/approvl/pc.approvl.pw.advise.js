...
var kqGridPanel
var treeLoaderURL = BASE_PATH + 'servlet/RlzyServlet' + '?ac=getKqGridTree'
		+ '&sjType=' + nowSj;
var treeLoader
...
Ext.onReady(function() {
	...
	rlzyMgm.findSjListForKqDeptByDeptId(deptId, function(list) {
				for (i = 0; i < list.length; i++) {
					...
					temp.push(list[i].toString().substr(0, 4) + '年'
							+ list[i].toString().substr(4, 6) + '月');
					months.push(temp);
				}
			});
	...
	treePanel = new Ext.tree.ColumnTree({
		...
		columns : [{
					...
					renderer : function(vl, n, a) {
						if (a['deptId'] != defaultOrgRootID
								&& a['status'] != null && a['status'] != '1') {
							return vl;
						} else {
							return "<u title='"
									+ vl
					...
					renderer : function(value, n, a) {
						if (value == null || value == '0')
							// return '<font color=gray>未上报</font>&nbsp;&nbsp;<u
							// onclick=openCell(\''+ a['lsh'] +'\',\''+
							// a['deptId'] +'\') style=\'color:green\'>直接编制</u>'
							return '<font color=gray>未上报</font>'
						else if (value == '1')
							return '<B><font color=green>已上报</font></B>&nbsp;&nbsp;<u onclick=uploadById(\''
									+ a['lsh']
									+ '\',\'4\')  style=\'color:red\'>退回</u>&nbsp;&nbsp;<u onclick=permit(\''
									+ a['lsh']
									+ '\',\'3\') style=\'color:blue\'>批准</u>'
						else if (value == '2')
							return '<B><font color=red>申请重报</font></B>&nbsp;&nbsp;<u onclick=uploadById(\''
									+ a['lsh']
									+ '\',\'4\')  style=\'color:red\'>退回</u>&nbsp;&nbsp;<u onclick=permit(\''
									+ a['lsh']
									+ '\',\'3\') style=\'color:blue\'>批准</u>'
	...
	cellPanel = new Ext.Panel({
		...
		// region:'center',
		frame : true,
		html : '<iframe name="cellFrame" src="'
				+ CONTEXT_PATH
				+ '/cell/eReport.jsp?openCellType=iframe" frameborder=0 style="width:100%;height:100%;"></iframe>'
			// renderTo: "center",
			// contentEl: "cell"
	});
...
function loadTreeGrid() {
	treeLoaderURL = BASE_PATH + 'servlet/RlzyServlet' + '?ac=getKqGridTree'
			+ '&sjType=' + nowSj;
...
/*
 * function upload(lsh,status){ DWREngine.setAsync(false);
 * if(btn_upload.getText()=='退回重报'){ kqDaysDeptZb.status = "0" }
 * //kqDaysDeptZb.userId = USERID kqDaysDeptZb.latestDate = new Date()
 * rlzyMgm.updateKqDaysDeptZb(kqDaysDeptZb,function(rtn){ if(rtn){
 * setStatus(kqDaysDeptZb.status) treeLoader.load(treePanel.getRootNode(),
 * function(){ while (root.childNodes.length > 1) { root.firstChild.remove() } }) }
 * }); DWREngine.setAsync(true); }
 */
				...
				if (btn == 'ok' || btn == 'yes') {
					...
					rlzyMgm.updateKqDaysDeptZb(kqDaysDeptZb, function(rtn) {
								if (rtn) {
									...
									rlzyMgm.makeKqAnnualleave(
											kqDaysDeptZb.sjType.substr(0, 4),
											kqDaysDeptZb.unitId, "unit",
											function(rtn) {
												if (!rtn) {
													alert('计算年休假有错误')
												}
	...
	win = new Ext.Window({
				...
				buttons : [{
					...
					handler : function() {
						...
						nowSjDesc = nowSj.substr(0, 4) + '年'
								+ nowSj.substr(4, 6) + '月'
						if (!findKqDeptZb()) {
							alert('没有数据')
						} else {
							store_month.add(new Ext.data.Record({
										value : nowSj,
										text : nowSjDesc
									}))
							combox_month.setValue(nowSj)
...
function setStatus(status) {
	...
	try {
		window.frames["cellFrame"].document.all.cmdFileSave.style.display = status == null
				|| status == '0' ? '' : 'none'
	} catch (e) {
	}
}
...
function openCell(_lsh, _deptId) {
	...
	deptId = _deptId
	/*
	 * p_type 报表类型 必选 无 属性表中定义了报表类型 p_date 数据期别 必选 无 必须符合系统的数据期别规则 p_corp
	 * 数据及报表所属单位（部门） 在报表中定义{CORP}时，替换成该参数对应的单位名称。 必选 无 可以为
	 * 单位ID/部门ID模式，此种模式下表示应用于同单位下多个部门情况，根据部门ID查找报表模板，根据单位ID进行数据的存储。如果为单位ID模式，则报表模板和报表数据均用同一个单位ID。
	 * p_inx 指标（产品ID）。在报表中定义{INX}时，替换成该参数对应的指标（产品）名称。 可选 无 指标管理中的指标ID saveAsFile
	 * 是否将cell保存为文件 可选 False 布尔类型 savable 是否具备保存权限 可选 False 布尔类型 p_key_col
	 * 保存数据主表主键 有主从关系结构且需要保存时，必选 无 数据表列 p_key_val 保存数据主表主键值 有主从关系结构且需要保存时，必选 无
	 * 主表主键值 拓展的参数 p_remarkTable 数据说明信息对应的数据表 需要填写数据说明时，必选 无 数据表 p_remarkCol
	 * 数据说明信息对应的数据列 需要填写数据说明时，必选 无 数据列 p_where
	 * 拓展的数据过滤条件。针对仅用于数据查询的报表，除了报表的基础过滤条件外增加了该参数。 可选 无
	 * table1`whereStr1|table2`whereStr2 p_showVersion 是否显示历史版本按钮 可选 “none”
	 * 字符串，none或block p_showRemark 是否显示数据说明按钮 可选 “none” 字符串，none或block
	 */
	cellType = '17'
	cellDate = nowSj
	...
	cellUrl = CONTEXT_PATH + "/cell/eReport.jsp?openCellType=iframe&p_type="
			+ cellType + "&p_date=" + cellDate + "&p_corp=" + corp + "&p_inx="
			+ GUIDELINEIDS + "&savable=" + cellSaveable
			+ "&p_key_col=MASTERLSH&p_key_val=" + reportIdTemp
...
function afterCellSaved(CellWeb, callback) {
	...
	rlzyMgm.calcKqDaysTjDataByKey(lsh, function(rtn) {
				if (rtn) {
					for (i = 0; i < CellWeb.GetTotalSheets(); i++) {
						...
						for (row = 0; row < maxRow; row++) {
							for (col = 0; col < maxCol; col++) {
								if (CellWeb.GetCellString(col, row, i)
										.indexOf("table:") != -1) {
									CellWeb.ClearArea(col + 1, row + 1, maxCol,
											maxRow, i, 1);
								}
							}
						}
					...
					cellXML.db2xml(def, '17', deptId, nowSj, (USERORGID == ""
									? UNITID
									: USERORGID), 'zb_seqno', '', callback);
				}
			});
	DWREngine.setAsync(true);
}
