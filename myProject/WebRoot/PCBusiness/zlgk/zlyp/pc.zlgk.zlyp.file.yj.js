...
Ext.onReady(function (){
    ...
    dsResult.on("beforeload",function(ds1){
    	Ext.apply(ds1.baseParams ,{ 
			ac : 'getJsonStrForTransToZlgkZlyp',
			yjrName : USERNAME,
			fileId : fileId,
			type:type,
			uuid : uuid
...
function yjzlFun(){
    ...
    var tableName = "PC_ZLGK_ZLYP_RECORD"+"`"+uuid;
	var fileLshs = "";
	var fileNames = "";
	var fileIds = "";
	     ...
	     if(list.length == 0){
				if(records.length>0 && zlSortSelected!=""){
					if(confirm("将选择的资料移交到【"+zlSortNameSelected+"】分类下,请确认!")){
							zlgkMgm.pcZlgkZlypTransToZLSByType(CURRENTAPPID,USERDEPTID,REALNAME,type,fileLshs.substr(1,fileLshs.length-1),
...
function cxyj(lsh){
	if(confirm("将收回移交的资料,请确认!")){
		zlgkMgm.cancelPcZlgkTrans(lsh,function(dat){
			if(dat){
				dsResult.load();
