...
//预警处理状态 warnstatus---(1: 未处理, 2: 处理中, 3: 已处理(已关闭))
...
//获取预警模块名称
 ...
 //获取人员信息
 baseDao.findByWhere2("com.sgepit.frame.sysman.hbm.RockUser", "1=1", function(list){
			  for(var i=0;i<list.length;i++){
			      var temp = new Array();
...
Ext.onReady(function(){ 
        ...
        listeners :{
            click : function(n,e){
                if(n.id!=''){
                    ...
                    store.baseParams.params=" moduleid='"+n.id+"' and pid=" + CURRENTAPPID;
                    }
                    northReload();
                }
            }
    ...
    store.setDefaultSort("warntime", "desc");
            ...
            renderer : function (value){
               for(var i=0;i<unitTypeStore.getCount();i++){
                   if(value == unitTypeStore.getAt(i).get('unitid')){
                   	var pname = unitTypeStore.getAt(i).get('unitname');
                   	return "<a title=" + pname + ">" + pname + "</a>";
                   }
               }
            }
         },{
    ...
    var grid = new Ext.grid.GridPanel({
	    ...
	    viewConfig: {
	        ...
	        getRowClass : function (rec,index,p,store){
	            if(this.showPreview){
	            	var warnContent = rec.data.warncontent;
    ...
    function PersonListView(){
        ...
        if(typeof(rec)=='undefined' || rec==null){
	        ...
	        if(rec.get('warnstatus')=='3'){
		       ...
		       DWREngine.setAsync(false);
		       	   baseDao.findByWhere2("com.sgepit.pcmis.warn.hbm.PcWarnRuleDutyperson", "moduleid='"+moduleid+"'", 
		       	   function(list){
		       	   		if(list.length > 0) {
		       	   			dutyPersonId = list[0].dutyperson;
		       	   		}					
		       	   });
		       DWREngine.setAsync(true);
...
//		        var splitstr =dutyPersonId.split(',');
		        if(dutyPersonId != USERID){
		            Ext.Msg.alert('提示信息','您不具备分配人员权限');
		            return ;
		        }
		        cuserWindow.show(USERBELONGUNITID,USERBELONGUNITNAME,rec.get('pid'),rec.get('uids'),"", "", this);
		    }
        }
    }
