...
function resetCellFrm(){
		DWREngine.setAsync(false);
		baseDao.findByWhere2("com.sgepit.pcmis.jdgk.hbm.VPcJdgkReport",
...
function getTimeStoreArr(unitid){
	var sjArr=new Array();
	DWREngine.setAsync(false);
	var whereSql="pid='"+ edit_unitid+ "' and state='3' order by sj_type asc";
	...
	baseDao.getData("select distinct t.SJ_TYPE from V_Pc_Jdgk_Report t where "+whereSql, function(list){
		if(list.length > 0){   
