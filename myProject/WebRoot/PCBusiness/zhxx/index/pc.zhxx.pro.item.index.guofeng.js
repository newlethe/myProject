...
var projectItemArr = new Array();
var projectTargetArr = new Array();
var safetyTargetArr = new Array();
var modelLinkArr=new Array();
DWREngine.setAsync(false);
appMgm.getCodeValue('项目组成', function(list) {
	for (var i = 0; i < list.length; i++) {
		var temp = new Array();
		temp.push(list[i].propertyCode);
		temp.push(list[i].propertyName);
		projectItemArr.push(temp);
	}
});
appMgm.getCodeValue('项目质量目标', function(list) {
	for (var i = 0; i < list.length; i++) {
		var temp = new Array();
		temp.push(list[i].propertyCode);
		temp.push(list[i].propertyName);
		projectTargetArr.push(temp);
	}
});
appMgm.getCodeValue('快捷功能链接', function(list) {
	for (var i = 0; i < list.length; i++) {
		var temp = new Array();
		temp.push(list[i].propertyCode);
		temp.push(list[i].propertyName);
		temp.push(list[i].detailType);
		modelLinkArr.push(temp);
	}
});
appMgm.getCodeValue('安全管理总目标', function(list) {
	for (var i = 0; i < list.length; i++) {
		var temp = new Array();
		temp.push(list[i].propertyCode);
		temp.push(list[i].propertyName);
		safetyTargetArr.push(temp);
	}
});
DWREngine.setAsync(true);
var projectLength=Math.max(projectItemArr.length,projectTargetArr.length,safetyTargetArr.length);
var centerItemStr="";
for(var i=0;i<projectLength;i++){
	if(i<projectItemArr.length){
		centerItemStr+='<tr height="8%">' +
'    <td>&nbsp;</td>' +
'    <td colspan="12" align="left"><img src="PCBusiness/zhxx/index/images/flag-blue.png"></img>'+projectItemArr[i][1]+'</td>' +
'    <td>&nbsp;</td>';
	}else{
		centerItemStr+='<tr height="8%">' +
'    <td>&nbsp;</td>' +
...
'    <td>&nbsp;</td>';
	...
	if(i<projectTargetArr.length){
		centerItemStr+='    <td>&nbsp;</td>' +
'    <td colspan="9" align="left"><img src="PCBusiness/zhxx/index/images/flag-red.png"></img>'+projectTargetArr[i][1]+'</td>' +
'    <td>&nbsp;</td>';
	}else{
		centerItemStr+='    <td>&nbsp;</td>' +
...
'    <td>&nbsp;</td>';
	...
	if(i<safetyTargetArr.length){
		centerItemStr+='    <td>&nbsp;</td>' +
'    <td colspan="11" align="left"><img src="PCBusiness/zhxx/index/images/flag-green.png"></img>'+safetyTargetArr[i][1]+'</td>' +
'    <td>&nbsp;</td>' +
'  </tr>';
	}else{
		centerItemStr+='    <td>&nbsp;</td>' +
...
'<table width="100%" height="60%" border="0" bgcolor="#FFFFFF" cellpadding="0" cellspacing="0" style="table-layout:fixed;border-bottom: 1px solid #d0d0d0;">' +
'  <tr height="10%">' +
'    <td colspan="20" height="60" align="center"><p class="projTitle">{prjName}</p></td>' +
'  </tr>' +
'  <tr height="8%">' +
'    <td style="width:5%">&nbsp;</td>' +
...
'    <td>&nbsp;</td>' +
'    <td>&nbsp;</td>' +
'    <td rowspan="3" colspan="9" align="center" bgcolor="#ffffff">' +
'      <div class="countdown">' +
...
'    <td align="center">' +
'  		<span style="color:red;" id="conMonthMoneyNum"  name="conMonthMoneyNum" >{conMonthMoneyNum}</span>' +
' 		' +
'  	 </td>' +
'	 <td>个</td>' +
'  	 <td align="center" colspan="3"><span style="color:red;" id="conMonthMoney"  name="conMonthMoney" >{conMonthMoney}</span></td>' +
'    <td>万元</td>' +
'    <td>&nbsp;</td>' +
'    <td  colspan="4">自开工累计签订合同</td>' +
'    <td align="center"><span style="color:red;" id="allTotalConMoneyNum"  name="allTotalConMoneyNum" >{allTotalConMoneyNum}</span></td>' +
'    <td>个</td>' +
 '   <td align="center" colspan="3"><span style="color:red;" id="allTotalConMoney"  name="allTotalConMoney" >{allTotalConMoney}</span></td>' +
'    <td>万元</td>' +
'    <td>&nbsp;</td>' +
'  </tr>' +
'  <tr height="8%">' +
...
'    <td align="center"><span style="color:red;" id="monthPayMoneyNum"  name="monthPayMoneyNum" >{monthPayMoneyNum}</span></td>' +
'    <td>个</td>' +
 '   <td colspan="3" align="center"><span style="color:red;" id="monthPayMoney"  name="monthPayMoney" >{monthPayMoney}</span></td>' +
'    <td>万元</td>' +
'    <td>&nbsp;</td>' +
'    <td colspan="4">自开工累计合同付款</td>' +
'    <td align="center"><span style="color:red;" id="allTotalPayMoneyNum"  name="allTotalPayMoneyNum" >{allTotalPayMoneyNum}</span></td>' +
'    <td>个</td>' +
 '   <td colspan="3" align="center"><span style="color:red;" id="allTotalPayMoney"  name="allTotalPayMoney" >{allTotalPayMoney}</span></td>' +
'    <td>万元</td>' +
'    <td>&nbsp;</td>' +
'  </tr>' +
'  <tr height="8%">' +
...
'    <td align="center"><span style="color:red;" id="conMonthChangeMoneyNum"  name="conMonthChangeMoneyNum" >{conMonthChangeMoneyNum}</span></td>' +
'    <td>个</td>' +
'    <td colspan="3" align="center"><span style="color:red;" id="conMonthChangeMoney"  name="conMonthChangeMoney" >{conMonthChangeMoney}</span></td>' +
'    <td>万元</td>' +
'    <td>&nbsp;</td>' +
'    <td colspan="4">自开工累计合同变更</td>' +
'    <td align="center"><span style="color:red;" id="allTotalChangeMoneyNum"  name="allTotalChangeMoneyNum" >{allTotalChangeMoneyNum}</span></td>' +
'    <td>个</td>' +
'    <td colspan="3" align="center"><span style="color:red;" id="allTotalChangeMoney"  name="allTotalChangeMoney" >{allTotalChangeMoney}</span></td>' +
'    <td>万元</td>' +
'    <td>&nbsp;</td>' +
'  </tr>' +
*/
...
'  </tr>' +
'</table>' +
'<table width="100%" height="40%" border="0" bgcolor="#FFFFFF" cellpadding="0" cellspacing="0" style="table-layout:fixed;">' +
'  <tr height="8%">' +
'    <td style="width:5%">&nbsp;</td>' +
'    <td colspan="12" align="left">项目组成部分：</td>' +
'    <td>&nbsp;</td>' +
'    <td>&nbsp;</td>' +
'    <td colspan="9" align="left">项目质量总目标：</td>' +
'    <td>&nbsp;</td>' +
'    <td>&nbsp;</td>' +
'    <td colspan="10" align="left">安全管理总目标：</td>' +
'    <td>&nbsp;</td>' +
'    <td>&nbsp;</td>' +
'  </tr>' +
centerItemStr+
...
var eastTable =
	'<div id="liTable">' +
	'<div id="myChart"></div>' +
	'<table width="98%" id="liListTable" height="100%" align="center" bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="5">' +
		'<tr>' +
	'<tpl for="lichengbei">' +
			'<td>{id}.{name}</td>' +
		'<tpl if="id % 3 === 0">' +
...
var southTable=
'			<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="" class="buttons1">'+
'				<tr">'+
'					<td>'+
...
'				</tr>'+
'			</table>';
var southTable='<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="" class="buttons1">';
var modelCount=0;
for(var j=0;j<modelLinkArr.length;j++){
	if(modelCount%5==0){
		if(modelCount==0){
			southTable+='<tr>'
		}else{
	...
	southTable+='<td><input value="'+modelLinkArr[j][1]+'" type="button" id="'+modelLinkArr[j][0]+'" name="'+modelLinkArr[j][0]+'" onclick="openUrl(this)">' +
			'</td>'
	modelCount++;
}
southTable+='</table>';
...
Ext.onReady(function(){
	...
	var southRight =new Ext.Panel({ 
		...
		split:true,
		collapsed : false,
		html:'<div id="msg" style="height:50%"></div>'	
	});
    ...
    chart = new Carton("/"+ROOT_CHART+"/XCarton.swf", "ChartId", "500", "100%", "#FFFFFF", "1");
    chart.render("myChart");
    chart.setParam("projectuid",projectuid);
    //读取服务器端的配置文件，根据不同项目类型读取不同cml文件
    if(itemType == "HDX"){
		//火电项目新里程碑图，6条水平的线表示1#，2#，公共部分计划与完成 shuz 2014-9-17
	...
	function openUrl(btn){
		...
		var btnId =  btn.id;
		var scheduleURL ="";
		btnId=btnId.substring(btnId.length-3,btnId.length);
		var index=parseInt(btnId,10);
		index=index>0?(index-1):index
		scheduleURL = BASE_PATH+modelLinkArr[index][2];
	...
	function loadMsg(){
		var tpl = new Ext.Template(
			'<div class="bq">',
			    ...
			    '<a href="javascript:void(0);" class="score">上月数据完整性考核分数<span id="lastMonthScore">0</span>分</a>',
		    '</div>'
		 )
...
function goToConMonthMoneyPage(){//本月签订合同页面
	...
	var month=SYS_TIME_STR.substring(5,7);
	var arr='';
	var sql = "select a.* from (SELECT t.* FROM sgcc_ini_unit t START WITH t.unitid = '"+USERDEPTID+"' connect by t.unitid = PRIOR t.upunit) a where a.unit_type_id='7'";
	var count;
	DWREngine.setAsync(false);
