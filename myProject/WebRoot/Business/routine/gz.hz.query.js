...
var bean_fw = "com.sgepit.pmis.routine.hbm.GzJhZj"
...
Ext.onReady(function(){
 ...
 ///-------获取年份
     ...
     var comboFilter_y =  new Ext.form.ComboBox({
			...
			listeners:{select:function(combo,record,index){
				if(comboFilter_m.getValue()!=""){
					...
					ds.baseParams.params = "to_char(month,'yyyy-mm')='"+record.data.k+'-'+comboFilter_m.getValue()+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
    ...
    var comboFilter_m =  new Ext.form.ComboBox({
            ...
            listeners:{select:function(combo,record,index){
            	if(comboFilter_y.getValue()!=""){
            		...
            		ds.baseParams.params = " to_char(month,'yyyy-mm')='"+comboFilter_y.getValue()+'-'+record.data.id+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
    ...
    var deptCombo = new Ext.form.ComboBox({
			...
			DWREngine.setAsync(false);
					ds.baseParams.params = " unitid='"+com.value+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
	...
	gridPanel = new Ext.grid.EditorGridTbarPanel({
		...
		//tbar : ['<font color=#15428b><B>当月工作总结<B></font>','-',addBtn,editBtn,delBtn,'-','按上报时间查询:',comboFilter_y,comboFilter_m,'->'],
				tbar : ['<font color=#15428b><B>当月工作总结<B></font>','-','按上报时间查询:',comboFilter_y,comboFilter_m,'-','部门查询:',deptCombo],
    ...
    function insertFun_plan(){
    	var url = BASE_PATH+"Business/routine/gz.month.plan.addorupdate.jsp";
		window.location.href = url;
    }
    function toEditHandler_plan(){
		var record = gridPanel.getSelectionModel().getSelected();
		var url = BASE_PATH+"Business/routine/gz.month.plan.addorupdate.jsp?uids="+record.get('uids');
		window.location.href = url;
    }
    //----------------------细表范围----------------------------//
 	var fm_fw = Ext.form;
	...
	gridPanel_fw = new Ext.grid.EditorGridTbarPanel({
		...
		tbar : ['<font color=#15428b><B>工作汇报计划审核单<B></font>','-'],
		animCollapse : false, // 折叠时显示动画
		loadMask : true, // 加载时是否显示进度
		stripeRows: true,
		viewConfig : {
	...
	function renderDyzj(value, metadata, record){
		if(record.get('state1')==1){
			...
			output += 'Business/routine/gz.month.plan.addorupdate?uids='+getUids+'\'">'+ value+'</span>'
			return output;
			}
		else{
			var getUids = record.get('uids');
