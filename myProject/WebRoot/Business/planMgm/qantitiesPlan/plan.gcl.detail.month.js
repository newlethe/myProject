var bean = "com.sgepit.pmis.planMgm.hbm.PlanMonth";
...
Ext.onReady(function() {
	...
	initDataBtn = new Ext.Button({
				...
				handler : function() {
					if (parent.selectMasterId != "") {
						investmentPlanService.initQuantitiesPlanData(
								parent.selectMasterId, function(dat) {
									if (dat) {
										ds.reload();
									} else {
	...
	var Columns = [{
			...
			},// 合同ID
			{
				...
				name : 'monthQuantities',
				type : 'float'
			}, {
				name : 'monthAmount',
				type : 'float'
			}, {
				name : 'quantitiesAddup',
				type : 'float'
			}, {
				name : 'perQuantitiesAddup',
				type : 'float'
			}, {
				name : 'amountAddup',
				type : 'float'
	...
	var editField = new Ext.form.NumberField(fc['monthQuantities'])
	...
	sm, {
				...
				id : 'quantitiesId',
				header : fc['quantitiesId'].fieldLabel,
				dataIndex : fc['quantitiesId'].name,
				hidden : true,
				width : 0
			}, {
				id : 'quantitiesName',
				header : fc['quantitiesName'].fieldLabel,
				dataIndex : fc['quantitiesName'].name,
				width : 200,
				sortable : false
			}, {
				id : 'monthQuantities',
				header : fc['monthQuantities'].fieldLabel,
				dataIndex : fc['monthQuantities'].name,
				width : 100,
				editor : editField
			}, {
				id : 'quantitiesAddup',
				header : fc['quantitiesAddup'].fieldLabel,
				dataIndex : fc['quantitiesAddup'].name,
				width : 100
			}, {
				id : 'perQuantitiesAddup',
				header : fc['perQuantitiesAddup'].fieldLabel,
				dataIndex : fc['perQuantitiesAddup'].name,
				width : 100
			}, {
				id : 'quantitiesPrice',
				header : fc['quantitiesPrice'].fieldLabel,
				dataIndex : fc['quantitiesPrice'].name,
				width : 100,
				renderer:cnMoney,
				sortable : false
			}, {
				id : 'monthAmount',
				header : fc['monthAmount'].fieldLabel,
				dataIndex : fc['monthAmount'].name,
				width : 100,
				renderer : function(value, cellmeta, record, rowIndex,
						columnIndex, store) {
					var price = record.data.quantitiesPrice;
					var gcl = record.data.monthQuantities;
					var val = price * gcl;
					record.data.monthAmount = val;
					return cnMoneyToPrec(val, 0);
				}
			}, {
				id : 'amountAddup',
	...
	gridPanel.getTopToolbar().add("->", calAddupBtn, '-', initDataBtn, '-',
			exportExcelBtn)
	ds.load({
				params : {
					start : 0,
...
function calAddData() {
	if (parent.selectMasterId != "") {
		investmentPlanService.updateDataAddup(parent.businessType,
				parent.selectConId, parent.selectSjType, function(d) {
					if (d) {
						ds.reload();
					}
				});
