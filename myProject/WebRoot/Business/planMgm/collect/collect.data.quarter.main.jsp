...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		var masterRec = null;
		var collectUrl = "Business/planMgm/collect/collect.data.quarter.jsp";
		var sjType = "";
		var unitID = CURRENTAPPID;
		var tab1Title = "投资计划汇总表";
		var tab2Title = "建筑安装投资计划";
		var businessType = "<%=PlanMgmConstant.INVEST_PLAN_QUARTER%>";
		if(businessFlag=="F") {
			tab1Title = "资金计划汇总表";
			tab2Title = "建筑安装资金计划";
			businessType = "<%=PlanMgmConstant.FUND_PLAN_QUARTER%>";
		}
	</script>	
	...
	<script type="text/javascript">
		Ext.onReady(function(){
			...
			investmentPlanService.getConOveInfo("", function(list){
		 	...
		 	baseMgm.getData("select distinct substr(sj_type,0,4), substr(sj_type,0,4)||'年' from plan_master where unit_id='" + unitID + "' and business_type='" + "<%=PlanMgmConstant.INVEST_PLAN_QUARTER%>" + "' and length(sj_type)=5 order by substr(sj_type,0,4) desc",function(list){  
				for(i = 0; i < list.length; i++) {
					var temp = new Array();
					temp.push(list[i][0]);
					temp.push(list[i][1]);
			...
			var dsQuarter = new Ext.data.SimpleStore({
		 	...
		 	var quarterCombo = new Ext.form.ComboBox({
			   	name:'quarter',
				fieldLabel:'季度'	,
				store: dsQuarter,
				displayField:'v',
			...
			yearCombo.on('select', function(c, r, i){
				sjType = yearCombo.getValue()+quarterCombo.getValue();
				reloadAllData(businessType, unitID, sjType);
			});
			quarterCombo.on('select', function(c, r, i){
				sjType = yearCombo.getValue()+quarterCombo.getValue();
				reloadAllData(businessType, unitID, sjType);
			});
			...
			var url1 = "";
			if(businessFlag=="I") {
				url1 = "Business/planMgm/investmentPlan/dept/investment.data.quarter.bycontract.jsp?fromM=collect&businessType=" + "<%=PlanMgmConstant.INSTALL_INVEST_PLAN_QUARTER%>" + "&unitId=" + unitID;
			} else if (businessFlag=="F") {
				url1 = "Business/planMgm/investmentPlan/dept/investment.data.quarter.bycontract.jsp?fromM=collect&businessType=" + "<%=PlanMgmConstant.INSTALL_FUND_PLAN_QUARTER%>" + "&unitId=" + unitID;
			}
			...
			var url2 = "Business/planMgm/investmentPlan/dept/investment.data.quarter.bycontract.jsp?fromM=collect&businessType=" + "<%=PlanMgmConstant.EQUIPMENT_PLAN_QUARTER%>" + "&unitId=" + unitID;
			var tab3 = new Ext.Panel({
				id: 'tab3',
				title: '设备购置费计划',
				html: '<iframe name="data2Frame" src="' + url2 + '" frameborder=0 style="width:100%;height:100%;"></iframe>'
			});
			var url3 = "Business/planMgm/investmentPlan/dept/investment.OtherCost.main.jsp?fromM=collect&businessType=" + "<%=PlanMgmConstant.OtherCost_PLAN_QUARTER%>" + "&unitId=" + unitID;
			...
			var mainPanel = new Ext.Panel({
				...
				border:false,
				layout: 'border',
				tbar: [yearCombo, quarterCombo, '->', viewReportBtn, '-', exportExcelBtn],
				items: [dataPanel]
			});
	    ...
	    function viewReportFun(){
			...
			var url = CONTEXT_PATH + "/Business/planMgm/collect/collect.main.jsp?sjFlag=Q&businessFlag=" + businessFlag;
