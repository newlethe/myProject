...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		var masterRec = null;
		var collectUrl = "Business/planMgm/collect/collect.data.month.jsp";
		var sjType = "";
		var unitID = CURRENTAPPID;
		var tab1Title = "投资计划汇总表";
		var tab2Title = "建筑安装投资计划";
		var businessType = "<%=PlanMgmConstant.INVEST_PLAN_MONTH%>";
		if(businessFlag=="F") {
			tab1Title = "资金计划汇总表";
			tab2Title = "建筑安装资金计划";
			businessType = "<%=PlanMgmConstant.FUND_PLAN_MONTH%>";
		}
	</script>	
	...
	<script type="text/javascript">
		Ext.onReady(function(){
			...
			investmentPlanService.getConOveInfo("", function(list){
		 	...
		 	baseMgm.getData("select distinct substr(sj_type,0,4), substr(sj_type,0,4)||'年' from plan_master where unit_id='" + unitID + "' and business_type='" + "<%=PlanMgmConstant.INVEST_PLAN_MONTH%>" + "' and length(sj_type)=6 order by substr(sj_type,0,4) desc",function(list){  
				for(i = 0; i < list.length; i++) {
					var temp = new Array();
					temp.push(list[i][0]);
					temp.push(list[i][1]);
			...
			monthArr = [['01','1月'],['02','2月'],['03','3月'],['04','4月'],['05','5月'],['06','6月'],['07','7月'],['08','8月'],['09','9月'],['10','10月'],['11','11月'],['12','12月']];	
			...
			yearCombo.on('select', function(c, r, i){
				sjType = yearCombo.getValue()+monthCombo.getValue();
				reloadAllData(businessType, unitID, sjType);
			});
			monthCombo.on('select', function(c, r, i){
				sjType = yearCombo.getValue()+monthCombo.getValue();
				reloadAllData(businessType, unitID, sjType);
			});
			...
			var url1 = "";
			if(businessFlag=="I") {
				url1 = "Business/planMgm/investmentPlan/dept/investment.data.month.bycontract.jsp?fromM=collect&businessType=" + "<%=PlanMgmConstant.INSTALL_INVEST_PLAN_MONTH%>" + "&unitId=" + unitID;
			} else if (businessFlag=="F") {
				url1 = "Business/planMgm/investmentPlan/dept/investment.data.month.bycontract.jsp?fromM=collect&businessType=" + "<%=PlanMgmConstant.INSTALL_FUND_PLAN_MONTH%>" + "&unitId=" + unitID;
			}
			...
			var url2 = "Business/planMgm/investmentPlan/dept/investment.data.month.bycontract.jsp?fromM=collect&businessType=" + "<%=PlanMgmConstant.EQUIPMENT_PLAN_MONTH%>" + "&unitId=" + unitID;
			var tab3 = new Ext.Panel({
				id: 'tab3',
				title: '设备购置费计划',
				html: '<iframe name="data2Frame" src="' + url2 + '" frameborder=0 style="width:100%;height:100%;"></iframe>'
			});
//			var url3 = "Business/planMgm/investmentPlan/dept/investment.data.month.bybdj.jsp?fromM=collect&sjType=" + sjType + "&businessType=" + "<%=PlanMgmConstant.OtherCost_PLAN_MONTH%>" + "&unitId=" + unitID;
			var url3 = "Business/planMgm/investmentPlan/dept/investment.OtherCost.main.jsp?fromM=collect&businessType=" + "<%=PlanMgmConstant.OtherCost_PLAN_MONTH%>" + "&unitId=" + unitID;
			...
			var mainPanel = new Ext.Panel({
				...
				border:false,
				layout: 'border',
				tbar: [yearCombo, monthCombo, '->', viewReportBtn, '-', exportExcelBtn],
				items: [dataPanel]
			});
	    ...
	    function viewReportFun(){
			...
			var url = CONTEXT_PATH + "/Business/planMgm/collect/collect.main.jsp?sjFlag=M&businessFlag=" + businessFlag;
