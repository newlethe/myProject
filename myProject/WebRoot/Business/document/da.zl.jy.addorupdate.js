...
Ext.onReady(function() {
	...
	// 获取单位
	DWREngine.setAsync(false);
	baseMgm.getData('select unitid,unitname from sgcc_ini_unit',
			function(list) {
				for (i = 0; i < list.length; i++) {
					var temp = new Array();
					temp.push(list[i][0]);
	...
	with (gridTopBar) {
		add('-', selectJy, '-', '文件材料题名:', new Ext.form.TextField({
							id : 'searchText',
							emptyText : '输入文件名'
						}), queryBtn);
	}
	...
	function fileicon(value, metadata, record) {
		if (value != '') {
			...
			return "<center><a href='" + url
					+ "' target='about:blank'><img src='" + BASE_PATH
	...
	function doFormSave(dataArr) {
		...
		DWREngine.setAsync(false);
		zlMgm.insertdazl_jy(obj, function(dat) {
					if ("0" == dat) {
	...
	function queryFun() {
		...
		ds.baseParams.params = "  materialname like '%" + zl_q
				+ "%' and billstate=3"
		ds.load({
					params : {
						start : 0,
