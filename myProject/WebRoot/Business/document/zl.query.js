...
var orderColumn = "zltype"
var depttitle = USERORG.split(",");
var gridPanelTitle =  "资料综合查询"
var formPanelTitle = "资料综合查询"
...
//	 	}
	...
	var cm = new Ext.grid.ColumnModel([ // 创建列模型
			...
			sm, 
			{
				...
				renderer:function(value){
					if(value==2){return "已入库"}else if(value==3){return "已归档"}else if(value==1){return "申请移交"}else if(value==0){return "未移交"}
				}
			},{
				id : 'infoid',
...
//	                        	return data1;
	...
	ds = new Ext.data.Store({
		baseParams : {
			...
			business : business,
			method : listMethod,
			params : "  indexid in(select indexid from ZlTree) and  (billstate=2  or billstate=1 or billstate=0) " //or billstate=3
		},
		// 设置代理（保持默认）
...
//			forceFit : true,
	...
	ds.load({
		params : {
			start : 0,
			limit : PAGE_SIZE,
			params : "indexid in(select indexid from ZlTree) and  (billstate=2 or billstate=3 or billstate=1 or billstate=0) "
		}
	});
   ...
   ////////////////////////////////////////////////////////////	// //////////////////////////////////////////
	...
	treePanel.on('click', function(node) {
		...
		var where = "treeid" + SPLITB +selectedTreeData + SPLITA + "pid"+SPLITB+CURRENTAPPID+SPLITA+"billstate"+SPLITB+"query";
