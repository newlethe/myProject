...
var QueryPanel = new Ext.FormPanel({
	...
	function execQuery(){
		...
		if(treeData.trim().length != 0){
	        DWREngine.setAsync(false); 
	         zlMgm.querySort(treeData,function(value){ 
        		sqlStr =sqlStr+value+" and (billstate in ('0','1','2','3'))";
	 		}); 
	        DWREngine.setAsync(true);   
	     }
        ...
        if(wjcltm.trim().length != 0){
        	...
        	sqlStr += " materialname like '%"+wjcltm+"%'and (billstate in ('0','1','2','3'))";
        }
...
//   				baseParams.method = 'findwhereorderby';
                ...
                ds.baseParams.method  ='findwhereorderby';
	   			if((sqlStr + queStr)!=""){
	   				ds.baseParams.params = sqlStr + queStr;
	   			}else{
	   				ds.baseParams.params = " indexid in (select indexid from ZlTree) and (billstate='0' or billstate='1' or billstate='2' or billstate='3'))";
	   			}
   				ds.load({
   					params : {
						start : 0,
