		...
		if(treeData.trim().length != 0){
	        DWREngine.setAsync(false); 
	         zlMgm.querySort(treeData,function(value){ 
	         	if(value.length>0)
        		sqlStr =sqlStr+value+" and (billstate<>6 or billstate<>8)";
	 		}); 
...
//   				baseParams.method = 'findwhereorderby';
	   			...
	   			if(sqlStr == null || sqlStr == ""){
	   				switch(changeFlag){
	   				    case 'mydept':
	   				                  where +="responpeople='"+REALNAME+"'  and";
	   				               break;
	   				    case "dept" : break;           
	   				}
		   			where += " orgid='" + USERORGID + 
			                "'and indexid in (select indexid from ZlTree) and billstate<>'6' and billstate<>'8' "
	   			}else{
	   			   if(flag && USERORGID != ''){
			        	if(sqlStr.length > 0){
			        		sqlStr += " and "
