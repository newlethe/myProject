...
var bean = "com.sgepit.pmis.document.hbm.ZlInfoJy";
var business = "baseMgm"
var listMethod = "findwhereorderby"
var primaryKey = "uids"
var orderColumn = "jysj"
var depttitle = USERORG.split(",");
var gridPanelTitle = depttitle[0] + "--部门资料借阅"
var formPanelTitle = "部门资料借阅信息"
...
Ext.onReady(function() {
	...
	var fc = { // 创建编辑域配置
		...
		'jysj' : {name : 'jysj',fieldLabel : '借阅时间',width : 45,format : 'Y-m-d',anchor : '95%'},
	...
	var cm = new Ext.grid.ColumnModel([ // 创建列模型
		...
		{id:'infoid',header:fc['infoid'].fieldLabel,dataIndex:fc['infoid'].name,hidden:true},
	...
	PlantInt = {
		uids:'',infoid:'',jyr:'',deptid:'',jysj:new Date(),ghsj:'',xjsj:'',xjcs:0,memo:'',memo1:'',memo2:'',ghstate:''
	}; // 初始值
   ...
   ///////////////////////----------新增-------------/////////////////////////////////////
	...
	var currentInfoid="";currentjysj=""
	   ...
	   fs = selectRows[0].get("fs");                         //获取借阅份数，归还时置为0。
	...
	function formSave(){
    	...
    	DWREngine.setAsync(false);
    	zlMgm.insertzlinfo_gh(obj,fs,function(dat){
    		if("0"==dat){
	...
	function xjFun(){
	   ...
	   if(currentxjcs>1){Ext.MessageBox.alert('提示','最多只能续借2次,请及时归还!');return}
		//获取续借历史记录
    	DWREngine.setAsync(false);
    	zlMgm.getxj_history(currentuid,function(dat){
			his_xj=dat;
    	});
    	DWREngine.setAsync(true);
	...
	function formSavexj(){
    	...
    	DWREngine.setAsync(false);
    	zlMgm.update_xj(obj,function(dat){
    		if("0"==dat){
	...
	function deleteFun() {
		...
		for(var i=0;i<records.length;i++){
			if (records[i].get('ghsj')=="") {
				del_flag = "false";
				Ext.MessageBox.alert("提示!","资料必须在归还后才能删除!")
				return;
			}else{del_flag=true}
		}
		if(del_flag){
	...
	treePanel.on('click', function(node) {
		...
		ds.baseParams.params = " infoid in(select infoid from com.sgepit.frame.flow.hbm.ZlInfo where indexid like'"+selectedTreeData+"%')"
		ds.load({
			params : {
				start : 0,
				limit : PAGE_SIZE
