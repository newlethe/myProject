...
var gridPanelTitle = depttitle[0] + "--个人资料"
var formPanelTitle = "个人资料信息"
...
Ext.onReady(function() {
	...
	function doLoad() {
		...
		var state = record.get('billstate');
			...
			if(state==1||state==2||state==3){
				...
				ds.load({
					params : {
						start : 0,
						limit : PAGE_SIZE,
						arams : "responpeople='"+REALNAME+"' and orgid='" + USERORGID + 
			                    "'and indexid in (select indexid from ZlTree) and ( billstate<>'6' or billstate<>'8' ) "
					}
				});
		}
...
//	                        	return data1;
			...
			},
			{
				...
				renderer : function(v){
                      for(var i=0;i<indexidAry.length;i++){
                         if(v == indexidAry[i][0]){
                             return indexidAry[i][1];
                         }
                      }    
				}
			},{
	...
	ds = new Ext.data.Store({
		baseParams : {
			...
			/*params : " orgid='" + USERORGID + "' and indexid like '"
					+ selectedTreeData + "%' and billstate<>'6'"
		},*/
			  params : "responpeople='"+REALNAME+"' and orgid='" + USERORGID + "'and indexid in (select indexid from ZlTree) and ( billstate<>'6' or billstate<>'8') "
		},
		// 设置代理（保持默认）
...
//			forceFit : true,
	...
	ds.load({
		params : {
			start : 0,
			limit : PAGE_SIZE,
			params : "responpeople='"+REALNAME+"' and orgid='" + USERORGID + 
			         "'and indexid in (select indexid from ZlTree) and ( billstate<>'6' or billstate<>'8' ) order by STOCKDATE desc "
		}
	});
...
//		labelAlign : 'top',
   ...
   ////////////////////////////////////////////////////////////	// //////////////////////////////////////////
	...
	with (gridTopBar) {
		//add(update,'-', btnPrint, '-', btnQuery,'->',btnlcfj,'-',btnPayInfo,'-',btnback);
		add(update,'-', btnPrint, '-', btnQuery,'-',reset);
	}
	// 11. 事件绑定
...
//                                        ,hideLabel : true
   ...
   //////////////////////////////////////////////////////////
	function insertFun() {
        ...
        form.findField("orgid").setValue((selectorgid==null|| selectorgid == "")?USERORGID:selectorgid);
   ...
   function updatedept(){
				...
				fn: function(value){
					if ("yes" == value){
						...
						zlMgm.deleteinfo(ids, function(){
							...
							Ext.example.msg('删除成功！', '您成功删除了'+ids.length+'条个人资料信息！');
							refresh("no68",REALNAME);//no68的意思是在查询的时候billstate<>'6' and billstate<>'8'
							if (ds.getCount() > 0)
					    		sm.selectRow(0);
						});
					}
    ...
    var modName = "资料" + this.text
		...
		if (record != null) {
			...
			refresh("yes1",REALNAME);//yes1的意思是在查询的时候billState=1；
	...
	function refresh(str,REALNAME){
		...
		var where = "treeid" + SPLITB +selectTreeid + SPLITA + "pid"+SPLITB+CURRENTAPPID + SPLITA+
		            "orgid"+SPLITB+selectorgid+SPLITA+"billstate"+SPLITB+str+SPLITA+"responpeople"+SPLITB+REALNAME;
...
function reportOpened(CellWeb) {
	if(reportArgs1 && reportArgs1!=null) {
			...
			var sql = "select rownum,dh,mc,gdrq,zltype,billstate,ljr,bz,stockdate from "+"(select a.fileno as dh,a.materialname as mc," +
					...
					" decode(a.BILLSTATE, '0', '未移交', '1', '申请移交', '2', '已入库', '3', '已归档') as billstate, a.RESPONPEOPLE as ljr," +
					" a.REMARK as bz ,a.stockdate as stockdate from ZL_INFO a" +
					" where  a.ORGID ='"+reportArgs1.infyj+"' and  a.INDEXID ='"+reportArgs1.inftitle+"' and  a.responpeople= (select realname " +
					" from rock_user where userid='"+reportArgs1.infrealname+"') and  (a.BILLSTATE <> '6' or a.BILLSTATE <> '8') order by stockdate desc ,fileno asc ) " ;
			DWREngine.setAsync(false);
