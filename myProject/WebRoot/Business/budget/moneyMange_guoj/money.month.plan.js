...
var filterStr = " ifbl='0' and dept='"+USERDEPTID+"' and pid='"+CURRENTAPPID+"'";

var PCBdgMoneyReport = "PCBdgMoneyReport";
var ds;
var gridPanel;
Ext.onReady(function(){
	...
	//baseMgm.getData("select userid,realname from rock_user where DEPT_ID='"+USERDEPTID+"'",function(list){
 ...
 ///-------获取年份
	...
	var Columns = [
		...
		{name:'fj',type:'string'},   {name:'bh',type:'string'}, 
	...
	var PlantInt = {
		uids:'',  content:'',  bh:bh_flow,  planmoney:'',fj:'0',   enddate:'',sbsj:'',    fzr:'',   deptuser:'',
	...
	var cm = new Ext.grid.ColumnModel([
		...
		{id:'content'       , 	  header:fc['content'].fieldLabel       ,  dataIndex:fc['content'].name,type: 'string',width : 300,
     ...
     var comboFilter_y =  new Ext.form.ComboBox({
			...
			listeners:{select:function(combo,record,index){
				if(comboFilter_m.getValue()!=""){
		        	ds.baseParams.params = " ifbl='0' and dept='"+USERDEPTID+"' and pid='"+CURRENTAPPID+"' and to_char(sbsj,'yyyy-mm')='"+record.data.k+'-'+comboFilter_m.getValue()+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
    ...
    var comboFilter_m =  new Ext.form.ComboBox({
            ...
            listeners:{select:function(combo,record,index){
            	if(comboFilter_y.getValue()!=""){
	            	ds.baseParams.params = "ifbl='0' and dept='"+USERDEPTID+"' and pid='"+CURRENTAPPID+"' and to_char(sbsj,'yyyy-mm')='"+comboFilter_y.getValue()+'-'+record.data.id+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
	...
	gridPanel = new Ext.grid.EditorGridTbarPanel({
		...
		tbar : ['<font color=#15428b><B>费用计划表<B></font>','-',addBtn,'-',editBtn,'-',delBtn,'-',sbBtn,'-','按上报时间查询:',comboFilter_y,comboFilter_m,'-',transBtn,'->',queryBtn,'-','合计费用:',textTotalMoney],
	...
	ds.baseParams.params = "ifbl='0' and dept='"+USERDEPTID+"' and pid='"+CURRENTAPPID+"'";
	
	if(isFlwTask==true || isFlwView==true){
		ds.baseParams.params = "ifbl='0' and dept='"+USERDEPTID+"' and pid='"+CURRENTAPPID+"' and bh='"+bh_flow+"' ";
	}
    ...
    function insertFun_plan(){
    	var url = BASE_PATH+"Business/budget/moneyMange_guoj/money.month.plan.addorupdate.jsp?bh="+bh_flow;
		window.location.href = url;
    }
    function toEditHandler_plan(){
    	var records = sm.getSelections();var flag = false;
    	for(var i=0;i<records.length;i++){
    		if(records[i].get("jhzt")!=0){flag=true}
    	}
    	if(flag){
    		Ext.example.msg("提示","计划状态为‘新增’的数据才能编辑！");
    		return;
    	}
		var record = gridPanel.getSelectionModel().getSelected();
		var url = BASE_PATH+"Business/budget/moneyMange_guoj/money.month.plan.addorupdate.jsp?uids="+record.get('uids');
		window.location.href = url;
    }
    function DelFun(){
		var records = sm.getSelections();var flag = false;
    	...
    	if(flag){
    		Ext.example.msg("提示","计划状态为‘新增’的数据才能删除！");
    		return;
    	}
    ...
    function sbFun(){
    	var records = sm.getSelections();
    	var uids = "''";
    	if(records.length<1){Ext.example.msg("提示","请选择上报数据");return}
    	...
    	DWREngine.setAsync(false);
    	bdgMoneyMonthPlan.sbPlan(uids,function(dat){
    		if(dat==true){
    			Ext.example.msg("提示","上报审批数据成功");
    			ds.reload();
    			if (isFlwTask == true || isFlwView == true){
    				Ext.Msg.show({
					   title: '成功！',
					   msg: '成功上报资金计划！　　　<br>可以发送流程到下一步操作！',
					   buttons: Ext.Msg.OK,
...
//    		gridPanel_fw.defaultInsertHandler()
 ...
 //处理附件问题
    function fjFN(value, metadata, record,rowIndex, colIndex, store){
		...
		var count=0;
		DWREngine.setAsync(false);
        db2Json.selectData("select count(file_lsh) as num from sgcc_attach_list where transaction_id='"+record.data.uids+"' and transaction_type='"+PCBdgMoneyReport+"'", function (jsonData) {
...
function yjzls(filePk) {
		window.showModalDialog(
							CONTEXT_PATH
									+ "/Business/budget/moneyMange_guoj/money.month.fileTrans.jsp?type=PCBdgMoneyReport&fileId="
									+ filePk,
							null,
