...
var filterStr = " (jhzt='5' or jhzt='4') and pid='"+CURRENTAPPID+"'";
Ext.onReady(function(){
	var jhztArr = [['0','新增'],['1','已上报'],['2','汇总'],['4','审批完成'],['5','下达'],['6','上报中']];
 ...
 ///-------获取年份
     ...
     var comboFilter_y =  new Ext.form.ComboBox({
			...
			listeners:{select:function(combo,record,index){
				if(comboFilter_m.getValue()!=""){
		        	ds.baseParams.params = " jhzt='5' or jhzt='4' and pid='"+CURRENTAPPID+"' and to_char(sbsj,'yyyy-mm')='"+record.data.k+'-'+comboFilter_m.getValue()+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
    ...
    var comboFilter_m =  new Ext.form.ComboBox({
            ...
            listeners:{select:function(combo,record,index){
            	if(comboFilter_y.getValue()!=""){
	            	ds.baseParams.params = " jhzt='5' or jhzt='4' and pid='"+CURRENTAPPID+"' and to_char(sbsj,'yyyy-mm')='"+comboFilter_y.getValue()+'-'+record.data.id+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
	...
	gridPanel = new Ext.grid.EditorGridTbarPanel({
		...
		tbar : ['<font color=#15428b><B>费用计划表<B></font>','-',editBtn,'-',hzBtn,'-', '->',queryBtn,'合计费用:',textTotalMoney],
	...
	ds.baseParams.params = "  jhzt='5' or jhzt='4' and pid='"+CURRENTAPPID+"' ";
    ...
    function downFun(){
    	var records = sm.getSelections();
    	var uids = "''";
    	if(records.length<1){Ext.example.msg("提示","请选择下达数据");return}
    	...
    	DWREngine.setAsync(false);
    	bdgMoneyMonthPlan.downPlan(uids,function(dat){
    		if(dat==true){
    			Ext.example.msg("提示","下达成功");
    			ds.reload();
    		}else{
    			Ext.example.msg("提示","下达失败");
    		}
    	})
    	DWREngine.setAsync(true);
    }
   ...
   function toEditHandler_plan(){
		...
		var url = BASE_PATH+"Business/budget/moneyMange_guoj/down/money.month.plan.addorupdate.jsp?uids="+record.get('uids');
