...
Ext.onReady(function (){
	var btnReturn = new Ext.Button({
		...
		iconCls: 'returnTo',
		handler: function(){
			window.location.href = BASE_PATH + "Business/budget/bdg.generalInfo.input.jsp?conids="+CONIDS+"&optype="+OPTYPE+"&uids="+UIDS+"&dyView="+dyView;
		}
	});
	...
	var btnChange = new Ext.Button({
		...
		handler: function(){
			...
			if (id){
				var changeAppUrl;
			if ( hasNk == '0' ){
				changeAppUrl = "Business/budget/bdg.change.apportion.jsp";
			}
			else{
				changeAppUrl = "Business/budget/bdg.change.apportion.combine.jsp";
			}
			window.location.href = BASE_PATH + changeAppUrl + "?conid=" 
				+ id + "&conno=" + g_conno+ "&conname=" + encodeURIComponent(selectedConName) + "&chaid="+ record.get('chaid') + "&chano="+ record.get('chano')+"&uids="+UIDS+"&conids="+CONIDS+"&optype="+OPTYPE+"&dyView="+dyView;
								}
							}
	});
	...
	var btnProject = new Ext.Button({
		...
		handler: function(){
			if (sm.hasSelection()){
				var record = sm.getSelected();
				var id = record.get('conid');
				window.location.href = BASE_PATH + "Business/budget/bdg.project.apportion.jsp?conid=" 
					+ selectedConId + "&conname=" + selectedConName + "&conno="+g_conno+ "&conmoney="+conmoney +"&type=modify";
			}
		}
	});
	...
	var btnEqu = new Ext.Button({
		...
		handler: function(){
			if (sm.hasSelection()){
				var record = sm.getSelected();
				var id = record.get('conid');
				window.location.href = BASE_PATH + "Business/equipment/equ.equipment.input.js?conid=" 
					+ selectedConId + "&conname=" + selectedConName + "&conno="+g_conno+"&type='modify'";
			}
		}
	});
    ...
    bdgChangeMgm.isEquipment(selectedConId, function(flag){
    	if (flag == 1){
	...
	function toHandler(){
	    window.location.href = BASE_PATH + "Business/budget/bdg.change.apportion.jsp?conid=" 
				+ this.value.get("conid") + "&conname=" + selectedConName + "&chaid="+ this.value.get('chaid') + "&chano="+ this.value.get('chano') ;
	}
	...
	grid.getTopToolbar().add('->')
	grid.getTopToolbar().add(btnChange);
	grid.getTopToolbar().add('-'); 
	grid.getTopToolbar().add(btnProject); 
	grid.getTopToolbar().add('-'); 
	grid.getTopToolbar().add(btnEqu); 
	grid.getTopToolbar().add('-'); 
    ...
    function loadForm(){
     ...
     function renderchano(value, metadata, record){
		...
		output += 'Business/contract/cont.change.view.jsp?conid='+getChaid+'\'">'+ value+'</span>'*/
		//var selectedConId
		//var conno 
		//var selectedConName 
		return '<a href="'+BASE_PATH + 'Business/contract/cont.change.view.jsp?conid='+selectedConId + '&chaid='+getChaid+'&uids='+UIDS+'&conids='+CONIDS+'&optype='+OPTYPE+'&dyView=true">'+ value+'</a>'
