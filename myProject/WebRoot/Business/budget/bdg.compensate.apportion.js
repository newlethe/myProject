...
var gridPanelTitle = "合同：" + g_conname + "索赔号："+ g_clano + ", 所有索赔分摊";
var rootText = '索赔分摊';
...
Ext.onReady(function (){
	...
	var btnReturn = new Ext.Button({
		...
		handler: function(){
			//history.back();
			window.location.href = BASE_PATH + "Business/budget/bdg.compensate.jsp?conid="+g_conid+"&conname="+g_conname+"&conno="+conno+"&uids="+UIDS+"&conids="+CONIDS+"&optype="+OPTYPE+"&dyView="+dyView;
		}
	});
	...
	var btnSelect = new Ext.Button({
		...
		iconCls: 'option',
		handler: function(){
			window.location.href = BASE_PATH + "Business/budget/bdg.apportion.tree.jsp?type=compensate&conid="+g_conid+"&claid="+g_claid;
		}
	});
	...
	store = new Ext.ux.maximgb.treegrid.AdjacencyListStore({
				...
				baseParams : {
					ac : 'list',
					method : 'bdgCompensateTreeGrid',// 后台java代码的业务逻辑方法定义
					business : 'bdgCompensateMgm',// spring 管理的bean定义
				...
				reader : new Ext.data.JsonReader({
							...
							fields : ["bdgid", "bdgname", "bdgno", "pid","conid","bdgmoney","claappid",
									"conbdgappmoney","conappmoney","isleaf","parent","clamoney"]
						}),
				listeners : {
					'beforeload' : function(ds, options) {
						...
						ds.baseParams.params = 'conid' + SPLITB + g_conid
								+ ";parent" + SPLITB + parent+ ";claid" + SPLITB + g_claid;// 此处设置除第一次加载外的加载参数设置
					}
				}
			});	
	...
	treePanelNew =new Ext.ux.maximgb.treegrid.GridPanel({
		...
		columns:[{
            ...
            dataIndex: 'claappid',
            renderer: function(value){
            	return "<div id='claappid'>" + value + "</div>";
            }
        },{
            header: '概算编码',
            width: 120,
	...
	function toHandler(){
		...
		var state = this.text;
		var menu_nodeId = isRoot ? "0" : rec.data.claappid;
	...
	function delHandler(menu_isLeaf, nodeid, menu_parent){
		var rec = treePanelNew.getSelectionModel().getSelected();
		if (0 == menu_isLeaf){
			Ext.Msg.show({
			   title: '提示',
			...
			Ext.Msg.show({
					...
					fn: function(value){
						...
						if ("yes" == value){
				    		...
				    		bdgCompensateMgm.deleteCompensateChildNode(nodeid, function(flag){
				    			if ("0" == flag){
								...
								var oldFactMoneyStr = rec.data.clamoney+"";
...
function removeDeletedNode(){
	...
	if(rec){
		...
		menu_nodeId = rec.data.claappid;
		menu_bdgid = rec.data.bdgid;
		menu_isLeaf = rec.data.isleaf;
	    var formRecord = Ext.data.Record.create(Columns);
	    var loadFormRecord = null;
		tmpNodeRecord=rec;
		tmpLeaf = menu_isLeaf;		    
