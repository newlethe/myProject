...
Ext.onReady(function() {
	...
	var endSql = "select distinct sj_type as value, substr(sj_type, 0, 4)||'年'||substr(sj_type, 5, 2)||'月'||substr(sj_type, 7, 2)||'次' as text from hr_salary_master t where salary_type != 'BONUS'  and pid='"+CURRENTAPPID+"' order by sj_type asc";
...
//=========详情结束=====	
	...
	db2Json.selectSimpleData("select uids,name from hr_salary_type where state='1' and uids != 'BONUS'",
		function(dat){
...
//给是个人查询的部门下拉框和用户下拉框赋值
  	if(flagRole!=""&&flagRole=='self'){
  		Ext.getCmp("gridId").setTitle("个人工资查询");//panel的标题
		...
		Ext.getCmp("gridId").setTitle("工资统计查询");
  	}
...
//			execQuery()
		...
		//window.setTimeout("execQuery()", 5000)
		//时间设置完成        
	   //execQuery(); //页面渲染完成后调用  	
	...
	function execQuery() {
		...
		userIds = null;
		//区分是个人查询还是工资统计查询
	  	if(flagRole!=""&&flagRole=='self'){//个人工资查询时
	  		unitIds=new Array();
	  		userIds=new Array();
