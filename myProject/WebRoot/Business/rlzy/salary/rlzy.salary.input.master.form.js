...
Ext.onReady(function (){
	...
	DWREngine.setAsync(false);
	 baseMgm.getData("select zb_seqno,realname from sgcc_guideline_info where parentid='005' order by zb_seqno ",function(list){
	...
	DWREngine.setAsync(false);
	rlzyXcglMgm.getSjTypeListFromSalaryMaster(UNITID,"",CURRENTAPPID,function(list){
	...
	//工资单类型
	var salaryTypeSt = new Ext.data.SimpleStore({
		fields: ['value','text']
	})
	
	//var sql = "select uids as value, name as text from hr_salary_type where state = '1'";
	var sql = "select uids as value, name as text from hr_salary_type where uids != 'BONUS' ";
	db2Json.selectSimpleData(sql, function(dat){
		if(dat){
			salaryTypeSt.loadData(eval(dat))
		}
	});
	...
	var sql = "select uids as value, template_name as text from hr_salary_template where state = '1'";
	db2Json.selectSimpleData(sql, function(dat){
		if(dat){
	...
	templateCombo.onTriggerClick = function(){
		var typeUids = salaryTypeCombo.getValue();
		if(typeUids == null || typeUids == ""){
			Ext.example.msg('提示','请先选择工资单类型！');
		}else{
			templateCombo.resumeEvents(); 
			if(USERDEPTID == '103090106'){
				accDs.baseParams.params = "salary_type = '"+typeUids+"' and pid = '"+CURRENTAPPID+"'";
			}else{
				accDs.baseParams.params = "salary_type = '"+typeUids+"' and pid = '"+CURRENTAPPID+"' and templateDept like '%,"+USERDEPTID+",%'";//and templateDept like '%,"+USERDEPTID+",%'
			}
			accDs.load();
			accWin.show();
		}
	...
	infoForm = new Ext.FormPanel({
		...
		buttons: [{
            ...
            text: '关闭',
            handler: function(){
            	window.location.href = BASE_PATH + "Business/rlzy/salary/rlzy.salary.input.jsp";
            }
        }]
 ...
 /////////////////////////////////////////工资模板////////////////////////////////////////////////   
    ...
    var fc = {
		...
		'itemId':{name:'itemId',fieldLabel:'科目名称',hidden:true,hideLabel:true},
		'templateName':{name:'templateName',fieldLabel:'模板名称'},
		'xgridTitle':{name:'xgridTitle',fieldLabel:'表头'},
	...
	accSm = new Ext.grid.CheckboxSelectionModel({singleSelect:true});
	accCm = new Ext.grid.ColumnModel([
		new Ext.grid.RowNumberer(),
		accSm,
		{id:'uids',header:fc['uids'].fieldLabel,dataIndex:fc['uids'].name,hidden:true},
		{id:'xgridTitle',header:fc['xgridTitle'].fieldLabel,dataIndex:fc['xgridTitle'].name,hidden:true},
		{id:'templateName',header:fc['templateName'].fieldLabel,dataIndex:fc['templateName'].name,width:100},
		{id:'itemId',header:fc['itemId'].fieldLabel,dataIndex:fc['itemId'].name,width:80,hidden:true,renderer:function(value){
		...
		{id:'formula',header:fc['formula'].fieldLabel,dataIndex:fc['formula'].name,width:180, renderer:function(value){
		DWREngine.setAsync(false);
		var formula= "";
	...
	accDs = new Ext.data.Store({
			baseParams:{
				...
				business:business,
				method:listMethod,
				params:"state = '1' and salaryType != 'BONUS' and pid = '"+CURRENTAPPID+"' and templateDept like '%,"+USERDEPTID+",%'"
			},
	...
	kmTypeBox = new Ext.form.MultiSelect({
		...
		db2Json.selectSimpleData("select zb_seqno,realname from sgcc_guideline_info where parentid='005' order by zb_seqno",
			function(dat){
    ...
    KmPanel = new Ext.FormPanel({
    	...
    	items:[{
            ...
            labelWidth:36,
            items: [
            	{layout:'form',border:false,columnWidth:.3,labelWidth:37,items:[kmSel_com]},
            	{layout:'form',border:false,columnWidth:.3,labelWidth:37,items:[param_com]},
            	{layout:'form',border:false,columnWidth:.3,labelWidth:37,items:[symbol_com]}
            ]},{
	            layout:'form',border:false,labelWidth:36,
	            items:[{xtype:'textarea',id:'formula',fieldLabel:'公式',height:110,width:460}]
            }
    	],
    	buttons: [BUTTON_CONFIG['SELECT'],BUTTON_CONFIG['SAVE'], BUTTON_CONFIG['RESET']]
	});
...
function doFormSave(rtn){
	...
	FormulaUtil.getFormulaByText(textArea,function(str){
		if(str == record.get("formula")){
			Ext.Msg.alert("提示","没有修改公式，不需要保存!");
			...
			rlzyXcglMgm.saveTempFormula(uids,str,function(str){
				...
				if(arr[0]=="0"){
					...
					Ext.Msg.alert("操作成功","当前编辑的模板已经被使用过，编辑后的公式将保存为新的模板！");
				}else if(arr[0]=="2"){
