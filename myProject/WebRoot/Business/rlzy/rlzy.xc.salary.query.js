...
Ext.onReady(function (){
	...
	rlzyMgm.findSjListForXcSalaryD(USERID,function(list){
...
*/
	...
	function openCell(){
		...
		*/
		...
		cellUrl += "&p_where=V_HR_XC_SALARY_D`bill_status=1"
	...
	function findXcSalaryM(type){
		var rtn = false;
		DWREngine.setAsync(false);
		rlzyMgm.makeXcSalary(nowSj,deptId,type,function(obj){
			var status = '1'
			if(obj!=null && obj.lsh!=null){
				xcSalary = obj
				//Ext.getCmp('text_memo').setValue(obj.memo==null?'':obj.memo)
				document.getElementById("toolbarStatus").value = (obj.latestDate==null?'工资未发放':'工资发放时间：' + obj.latestDate.toLocaleString()) + (obj.memo==null?'':'    发放说明：' + obj.memo)
	...
	function doDelete(btn){
		if(btn=="yes"){
			DWREngine.setAsync(false);
			rlzyMgm.deleteXcSalary(nowSj,deptId,function(obj){
				if(obj||obj=='true'){
				    //openCell()
					location.reload() 
				}
