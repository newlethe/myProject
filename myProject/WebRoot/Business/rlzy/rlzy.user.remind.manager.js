...
Ext.onReady(function(){
	...
	function changeDate(loadsort){
		...
		var days = solarDays(parseInt(yy),(parseInt(mm)-1))+100;
		daysStore.removeAll();
		for(var i=101;i<=days;i++)	daysStore.add(new CommonRecord({v:i.toString().substring(1)}))
		if(mm=="02"&&(parseInt(dd)+100>days))  solarDDCombo.setValue(days.toString().substring(1));
		
		var lunar = new Lunar(new Date(yy,(parseInt(mm)-1),parseInt(solarDDCombo.getValue())));
	...
	function reloadData(loadsort){
		...
		if(pgrid&&sortgrid&&cbox){
			...
			var y = parseInt(solarYYCombo.getValue());
			var m = parseInt(solarMMCombo.getValue())-1;
			var d = parseInt(solarDDCombo.getValue());
...
//人员具体信息列表
var PersonGrid = Ext.extend(Ext.grid.GridPanel ,{
	border: false,
