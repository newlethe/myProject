...
Ext.onReady(function(){
	...
	var fc = {
		...
		'salaryType':{name:'salaryType',fieldLabel:'奖金单类别',allowBlank:false},
		'formula':{name:'formula',fieldLabel:'使用的公式'},
		'xgridTitle':{name:'xgridTitle',fieldLabel:'奖金单表头'},
	...
	gridPanel = new Ext.grid.EditorGridTbarPanel({
		...
		sm:sm,
		cm:cm,
		tbar: ['<font color=#15428b><b>奖金模版维护</b></font>','-'],
        region: 'center',
        border: false,
	...
	function saveTemplateHandler(){
		...
		if(!boolSj){
			Ext.Msg.show({
				title : '时间格式错误！',
				msg : "时间格式为8为数字，前四位为当前年费，如2011，中间两位为当前月份，如06，最后两位为奖金发放次数。<br><br><p align='center'>请检查后重新保存！</p>",
				buttons : Ext.Msg.OK,
				width : 400,
...
//		bbar:[symbol_com,'-','公式：',formulaTextArea,formulaHidden]
	});
    ...
    treePanel.on('click', function(node, e){
		...
		var paramStrCur = "posid"+SPLITB+node.id+SPLITA+"templateId"+SPLITB+selectedTemplateUids;
		
		if (selectedOrgId=="10000000000000")
			paramStrCur = "templateId"+SPLITB+selectedTemplateUids;
...
//	    enableDragDrop : true,
	...
	function setUserFun(){
		...
		userDs.baseParams.params = "templateId"+SPLITB+selectedTemplateUids;
		userDs.load({params:{start: 0,limit: 20}});
