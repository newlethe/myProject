...
Ext.onReady(function(){
		...
		items:[{
			...
			listeners:{
				...
				beforeload : function(node,e){
					...
					this.loader.dataUrl = treeNodeUrl+"&parentId="+node.id+"&treeName=HrManOrgTree";
				}
			}
		}]
	});
	...
	function exportDataFile(){
		var openUrl = CONTEXT_PATH + "/servlet/RlzyServlet?ac=exportData&businessType=userTransfer&params=" + encodeURI(exportQueryStr);
		document.all.formAc.action = openUrl
		document.all.formAc.submit();
	}
    Ext.getCmp("transfergrid").getTopToolbar().add(exportExcelBtn);
});
//组织结构树
    ...
    initComponent: function(){
		...
		,{
			...
			handler:function(){
				...
				this.store.reload();		
				exportQueryStr = this.store.baseParams.params; 
			}
		},'->',{
					...
					listeners:{
						beforerender :function(combo){
							...
							baseDao.findByWhere2("com.sgepit.frame.sysman.hbm.SgccIniUnit",
												"unit_type_id = '8' order by view_order_num asc",
								function(list){
