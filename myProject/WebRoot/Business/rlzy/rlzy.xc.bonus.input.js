...
*/
	...
	function findXcBonusM(type){
		var rtn = false;
		DWREngine.setAsync(false);
		rlzyMgm.makeXcBonus(nowSj,defaultOrgRootId,type,function(obj){
			var status = '1'
			if(obj!=null && obj.lsh!=null){
				xcBonus = obj
				Ext.getCmp('text_memo').setValue(obj.memo==null?'':obj.memo)
				document.getElementById("toolbarStatus").value = obj.latestDate==null?'奖金未发放':'奖金发放时间：' + obj.latestDate.toLocaleString()
	...
	function doDelete(btn){
		if(btn=="yes"){
			DWREngine.setAsync(false);
			rlzyMgm.deleteXcBonus(nowSj,defaultOrgRootId,function(obj){
				if(obj||obj=='true'){
				    //openCell()
					location.reload() 
				}
