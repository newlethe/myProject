...
Ext.onReady(function (){
	...
	rlzyMgm.findSjListForXcSalary(deptId,function(list){
...
//自定方法--------------------------------------------------------------------------------------------------
/*	function createFromCopyXcSalary(selQh,selSj,createSj){
		var rtn = false;
		DWREngine.setAsync(false);
		rlzyMgm.createFromCopyXcSalary(selQh,selSj,createSj,function(obj){
		});
	    DWREngine.setAsync(true);
	    openCell()
	    return rtn
...
*/
	...
	function openCell(){
		...
		*/
		...
		cellSaveable = xcSalary!=null&&xcSalary.status!=1&&xcSalary.billStatus!=1?true:false
		reportIdTemp = xcSalary!=null?xcSalary.lsh:''
		REMARKTABLE = ''
		REMARKCOL = ''
		...
		cellUrl += "&p_where=V_HR_XC_SALARY_D`bill_status=1|p_where=V_HR_XC_SALARYTJ_D`bill_status=1"
			
		document.all.cellFrame.src = cellUrl;
	}

	function findXcSalaryM(type){
		var rtn = false;
		DWREngine.setAsync(false);
		rlzyMgm.makeXcSalary(nowSj,defaultOrgRootId,type,function(obj){
			var status = '1'
	...
	function doDelete(btn){
		if(btn=="yes"){
			DWREngine.setAsync(false);
			rlzyMgm.deleteXcSalary(nowSj,defaultOrgRootId,function(obj){
				if(obj||obj=='true'){
				    //openCell()
					location.reload() 
				}
	...
	function modify(){
		var oldMemo = xcSalary.memo
		var newMemo =Ext.getCmp('text_memo').getValue()
		if(newMemo!=oldMemo){
			xcSalary.memo = newMemo
	    	DWREngine.setAsync(false);
			rlzyMgm.updateXcSalaryM(xcSalary,function(obj){
				if(obj){
					alert('更新成功！')
				}else{
					xcSalary.memo = oldMemo
	...
	function grant(){
		xcSalary.latestDate = new Date()
		xcSalary.userId = USERID
		xcSalary.billStatus = '1'
		xcSalary.status = '1'
    	DWREngine.setAsync(false);
		rlzyMgm.updateXcSalaryM(xcSalary,function(obj){
			if(obj){
				document.getElementById("toolbarStatus").value = xcSalary.latestDate==null?'奖金未发放':'奖金发放时间：' + xcSalary.latestDate.toLocaleString()
				alert('发放成功！')
				checkButton(xcSalary.status)
			}else{
				alert('发放失败！')
			}
		});
	...
	function afterCellSaved(CellWeb,callback){
	    DWREngine.setAsync(true);
		rlzyMgm.calcXcSalaryTjData(xcSalary,function(rtn){
			if(rtn){
