...
var treeLoaderURL = BASE_PATH+'servlet/RlzyServlet'+'?ac=getBonusGridTree'+ '&sjType='+nowSj;
var treeLoader
...
Ext.onReady(function (){
	...
	rlzyMgm.findSjListForXcBonus(deptId,function(list){
	...
	var label_month = new Ext.Toolbar.TextItem({text:"奖金月份："});
    ...
    combox_month.on('select', function(obj, record, idx ){
	    ...
	    if(!findXcBonusM("query")){
			...
			if(xcBonus.status!=null)
	    	setStatus(xcBonus.status)
	    	loadTreeGrid()
...
//        autoHeight: true,  
        ...
        columns:[{
            ...
            renderer: function(vl, n, a){
            	...
            	if(a['deptId'] != defaultOrgRootID && a['status'] != null && a['status'] != '1' && a['status'] != '3'){
            		return vl;
            	}else{
            ...
            ,renderer:function(value, n, a){
				if(value == null || value == '0')
					//return '<font color=gray>未上报</font>&nbsp;&nbsp;<u onclick=openCell(\''+ a['lsh'] +'\',\''+ a['deptId'] +'\') style=\'color:green\'>直接编制</u>&nbsp;&nbsp;<u onclick=uploadById(\''+ a['lsh'] +'\',\'3\') style=\'color:blue\'>批准</u>'
					return '<font color=gray>未上报</font>'
				else if(value == '1')
					//return '<B><font color=green>已上报</font></B>&nbsp;&nbsp;<u onclick=uploadById(\''+ a['lsh'] +'\',\'0\')  style=\'color:red\'>退回</u>&nbsp;&nbsp;<u onclick=permit(\''+ a['lsh'] +'\',\'3\') style=\'color:blue\'>批准</u>'
					...
					//return '<B><font color=red>申请重报</font></B>&nbsp;&nbsp;<u onclick=uploadById(\''+ a['lsh'] +'\',\'0\')  style=\'color:red\'>退回</u>&nbsp;&nbsp;<u onclick=permit(\''+ a['lsh'] +'\',\'3\') style=\'color:blue\'>批准</u>'
...
//			openCell()
	...
	function loadTreeGrid(){
		treeLoaderURL = BASE_PATH+'servlet/RlzyServlet'+'?ac=getBonusGridTree'+ '&sjType='+nowSj;
...
//				Ext.getCmp('text_memo').setValue(obj.memo==null?'':obj.memo)
				//document.getElementById("toolbarStatus").value = '状态：' + statusDesc//obj.latestDate==null?'奖金未发放':'奖金发放时间：' + obj.latestDate.toLocaleString()
...
//			setStatus(status)
	...
	function uploadById(lsh,status){
		var rtn = false
		DWREngine.setAsync(false);
		rlzyMgm.updateXcBonusMById(lsh,status,function(obj){
			if(obj){
				setStatus(status)
				treeLoader.load(treePanel.getRootNode(), function(){
					while (root.childNodes.length > 1) {
	...
	function permit(lsh,status){
		Ext.Msg.confirm('提示', '一旦批准无法修改，确认批准吗？', function(btn, text){
		    if (btn == 'ok' || btn == 'yes'){
				...
				rlzyMgm.updateXcBonusM(xcBonus,function(rtn){
					if(rtn){
						//将批准过的数据计算汇总数据
						calcXcBonusTjData()
						setStatus(xcBonus.status)
...
//            value: nowSj,
				...
				buttons: [{
					...
					handler: function() {
					    ...
					    if(!findXcBonusM("create")){
				    		...
				    		loadTreeGrid()
				    		if(contentPanel.getActiveTab().getId()!='tab_list')
								openCell(xcBonus.lsh,xcBonus.deptId)
				    	}
					    //checkButton()
					}
				}, {
	...
	function setStatus(status){
		...
		document.all.toolbarStatus.value = deptId == defaultOrgRootID?'':('状态：' + statusDesc)
		try{
	...
	function setLabels(unitname,username,status){
		...
		document.all.toolbarUserName.value = deptId == defaultOrgRootID?'':("填写人：" + username)
	}
	function openCell(_lsh,_deptId){
		if(_lsh == null || _deptId == null)
			return
//		if(contentPanel.getActiveTab().getId()=='tab_list')
//			return
		lsh = _lsh
		...
		*/
		...
		corp = defaultOrgRootId + '/' + deptId//(USERORGID==""?UNITID:USERORGID)
		GUIDELINEIDS = 'zb_seqno'
		cellSaveable = false//xcBonus!=null&&xcBonus.status!=3&&xcBonus.billStatus!=1&&xcBonus.unitId!=defaultOrgRootID?true:false
//		reportIdTemp = lsh//xcBonus!=null?xcBonus.lsh:''
		reportIdTemp = xcBonus!=null?xcBonus.lsh:(lsh!=null?lsh:'')
		REMARKTABLE = ''
		REMARKCOL = ''
		...
		cellUrl += "&p_where=V_HR_XC_BONUS_D`bill_status=1|V_HR_XC_BONUSTJ_D`bill_status=1"
	...
	function afterCellSaved(CellWeb,callback){
	    /*DWREngine.setAsync(false);
			rlzyMgm.calcXcBonusTjData(xcBonus,function(rtn){
				if(rtn){
	...
	function calcXcBonusTjData(CellWeb,callback){
		var rtn = false
		rlzyMgm.calcXcBonusTjData(xcBonus,function(obj){
			if(obj){
				rtn = true
			}
		});
