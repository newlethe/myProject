...
var btn_create,btn_update,btn_delete,btn_grant,btn_modify,btn_upload
var deptId = ""
...
//    		,'-',btn_grant
    		,'-',label_user
    		,'->',label_state,'-','说明：',text_memo//,btn_modify
    		,btn_upload
		]
	});
...
*/
	function checkButton(status){
		...
		if(allButtonPermission=='true' || status!='3'){
			btn_upload.enable()
		}else{
			btn_upload.disable()
		}
	...
	function setStatus(status){
		...
		document.all.toolbarStatus.value = '状态：' + statusDesc//xcBonus.latestDate==null?'奖金未发放':'奖金发放时间：' + xcBonus.latestDate.toLocaleString()//
		try{
	...
	function openCell(){
		...
		*/
		...
		cellSaveable = xcBonus!=null&&(xcBonus.status==null||xcBonus.status==0||xcBonus.status==4)?true:false
...
/*	function afterCellSaved(CellWeb,callback){
		...
		rlzyMgm.calcXcBonusTjData(xcBonus,function(rtn){
			if(rtn){
				...
				cellXML.db2xml( def,'21', deptId, nowSj, deptId, 'zb_seqno','',callback);
			}
		});
	}*/
