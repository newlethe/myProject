...
Ext.onReady(function (){
       ...
       treeLoaderURL = BASE_PATH+'servlet/RlzyServlet'+'?ac=getKqGridTree'+ '&sjType='+nowSj+'&userBelongUnitid='+upunit+'&deptId='+unitid;
	var fm = Ext.form;			// 包名简写（缩写）
	DWREngine.setAsync(false);
	rlzyKqglMgm.findSjListForKqDeptByDeptId('',function(list){
...
//			frame: true,
	...
	cellPanel = new Ext.Panel({
        ...
        //region:'center',
			frame: true,
			html: '<iframe name="cellFrame" src="/'+ROOT_CELL+'/cell/eReport.jsp?openCellType=iframe" frameborder=0 style="width:100%;height:100%;"></iframe>'
			//renderTo: "center",
			//contentEl: "cell"
	});
...
/*	function upload(lsh,status){
	...
	function uploadById(lsh,status){
		DWREngine.setAsync(false);
		rlzyKqglMgm.updateKqDaysDeptZbById(lsh,status,function(rtn){
			if(rtn){
				setStatus(status)
				treeLoader.load(treePanel.getRootNode(), function(){
					//while (root.childNodes.length > 1) {
		    ...
		    if (btn == 'ok' || btn == 'yes'){
				...
				rlzyKqglMgm.updateKqDaysDeptZb(kqDaysDeptZb,function(rtn){
					if(rtn){
						...
						rlzyKqglMgm.makeKqAnnualleave(kqDaysDeptZb.sjType.substr(0, 4),kqDaysDeptZb.unitId,"unit",function(rtn){
							if(!rtn){
								//alert('计算年休假有错误')
							}
						});
...
//            value: nowSj,
	...
	function afterCellSaved(CellWeb,callback){
	    DWREngine.setAsync(false);
			rlzyKqglMgm.calcKqDaysTjDataByKey(lsh,function(rtn){
				if(rtn){
