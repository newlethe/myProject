...
function update(currAppObj) {
	...
	Ext.Ajax.request({
				...
				success : function(result, request) {
					tempNode.attributes.chgMoney = currAppObj.chgMoney;
					refreshNodeColumns(tempNode);
					if (tempNode.parentNode) {
						sumChgAppValue(tempNode.parentNode);
					}
					mask.hide();
...
function sumChgAppValue(node) {
	...
	for (var i = 0; i < childNodes.length; i++) {
		sum += parseFloat(childNodes[i].attributes.chgMoney);

	}
	node.attributes.chgMoney = sum;

	refreshNodeColumns(node);
	sumChgAppValue(node.parentNode);
}
