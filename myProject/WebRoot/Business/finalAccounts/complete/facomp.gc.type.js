...
var beanName = 'com.sgepit.pmis.finalAccounts.complete.hbm.FACompGcType';
var selectedPath;
...
Ext.onReady(function() {
	...
	store = new Ext.ux.maximgb.treegrid.AdjacencyListStore({
				...
				baseParams : {
					ac : 'list',
					method : 'gcTypeTree',// 后台java代码的业务逻辑方法定义
				...
				reader : new Ext.data.JsonReader({
							...
							fields : ['uids', 'pid', 'treeid', 'gcTypeName',
									'gcTypeBm', 'remark', 'parentid', 'isleaf']
						}),
				listeners : {
	...
	store.setDefaultSort('gcTypeBm', 'asc');

	treeGrid = new Ext.ux.maximgb.treegrid.GridPanel({
				id : 'gctype-tree-panel',
				iconCls : 'icon-by-category',
				store : store,
				master_column_id : 'gcTypeName',// 此项为列的id，定义设置哪一个数据项为展开定义
				autoScroll : true,
				region : 'center',
				viewConfig : {
					forceFit : true,
