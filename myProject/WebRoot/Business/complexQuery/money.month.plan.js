...
var filterStr = " ifbl='0' and dept='"+USERDEPTID+"'";
...
Ext.onReady(function(){
 ...
 ///-------获取年份
     ...
     var comboFilter_y =  new Ext.form.ComboBox({
			...
			listeners:{select:function(combo,record,index){
				if(comboFilter_m.getValue()!=""){
		        	ds.baseParams.params = " ifbl='0' and dept='"+USERDEPTID+"' and to_char(sbsj,'yyyy-mm')='"+record.data.k+'-'+comboFilter_m.getValue()+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
    ...
    var comboFilter_m =  new Ext.form.ComboBox({
            ...
            listeners:{select:function(combo,record,index){
            	if(comboFilter_y.getValue()!=""){
	            	ds.baseParams.params = "ifbl='0' and dept='"+USERDEPTID+"' and to_char(sbsj,'yyyy-mm')='"+comboFilter_y.getValue()+'-'+record.data.id+"'";
					ds.load({
						params : {
							start : 0,
							limit : 20
	...
	ds.baseParams.params = "ifbl='0' and dept='"+USERDEPTID+"'";
    ...
    function insertFun_plan(){
    	var url = BASE_PATH+"Business/budget/moneyMange/money.month.plan.addorupdate.jsp?bh="+bh_flow;
	...
	sm.on('rowselect',function(sm,rowIndex,record){
		...
		if(record.get('jhzt')!=0){
			...
			//gridPanel_fw.getTopToolbar().enable();
		}
	})
