...
Ext.onReady(function() {
	//是否禁用添加/修改/删除按钮
		...
		sczbJjbMgm.initMendJjb(currentPid, function(data) {
			
		});

		DWREngine.setAsync(true);

		var gridfilter = "PID = '" + currentPid + "' and recordState='2'";
		var fm = Ext.form;
		var fcPB = {
			'uids' : {
				name : 'uids',
		 ...
		 smPB = new Ext.grid.CheckboxSelectionModel( {
			singleSelect : true
		})
		var cmPB = new Ext.grid.ColumnModel( [ // 创建列模型
				{
					...
					width : 50,
					//hidden : true,
					editor : new fm.TextField(fcPB['uids']),
					renderer : function(data) {
						return "<a href='javascript:void(0)' onclick='showInfo()'><img  src='" + CONTEXT_PATH
								+ "/jsp/res/images/icons/calculator_edit.png'></img></a>";
				}
					...
					id : 'rq',
					header : fcPB['rq'].fieldLabel,
					dataIndex : fcPB['rq'].name,
					editor : new fm.TextField(fcPB['rq']),
					...
					id : 'zbTime',
					header : fcPB['zbTime'].fieldLabel,
					dataIndex : fcPB['zbTime'].name,
					width : 150,
					//editor : new fm.TextField(fcPB['zbTime']),
					hidden : false
				}, {
					id : 'nameJie',
					header : fcPB['nameJie'].fieldLabel,
					dataIndex : fcPB['nameJie'].name,
					width : 150
				//editor: new fm.TextField(fcPB['isUsed']),
						}, {
							id : 'nameJiao',
							header : fcPB['nameJiao'].fieldLabel,
							dataIndex : fcPB['nameJiao'].name,
							width : 150
						//editor: new fm.TextField(fcPB['isUsed']),
						}, {
							id : 'jbDate',
							header : fcPB['jbDate'].fieldLabel,
							dataIndex : fcPB['jbDate'].name,
							width : 150,
							hidden : true
						//editor: new fm.TextField(fcPB['isUsed']),
						}, {
							id : 'getbDate',
							header : fcPB['getbDate'].fieldLabel,
							dataIndex : fcPB['getbDate'].name,
							width : 150,
							hidden : true
						//editor: new fm.TextField(fcPB['isUsed']),
						}, {
							id : 'recordState',
							header : fcPB['recordState'].fieldLabel,
							dataIndex : fcPB['recordState'].name,
							width : 150,
							hidden : true
						//editor: new fm.TextField(fcPB['isUsed']),
						}, {
							id : 'userUnitid',
							header : fcPB['userUnitid'].fieldLabel,
							dataIndex : fcPB['userUnitid'].name,
							width : 150,
							hidden : true
						//editor: new fm.TextField(fcPB['isUsed']),
						} ]);
		...
		// 3. 定义记录集
		 ...
		 PlantPB = Ext.data.Record.create(ColumnsPB); //定义记录集
...
//		});
		...
		smPB.on('rowselect', popPartbDet);
		//gridPB.on('rowdblclick',showInfo);
		...
		smPB.on('selectionchange', function(zbjlSmPB){ // grid 行选择事件
		...
		function selectData(){
			...
			var selectSql=" rq between to_date('"+beginStr+"','yyyy-mm-dd') and to_date('"+endStr+"','yyyy-mm-dd')";
			dsPB.baseParams.params = selectSql+" and pid='"+currentPid+"' and recordState='2'";
			dsPB.load({
				params:{
					start: 0,
					limit: PAGE_SIZE
		...
		function loadFormPB() {
			...
			if (smPB.getSelected() != null) {
				var gridRecod = smPB.getSelected()
				var ids = smPB.getSelected().get('uids');
				var rq=smPB.getSelected().get('rq');
				var bcName=smPB.getSelected().get('bcName');
				baseMgm.findById(
								"com.sgepit.pmis.sczb.hbm.SczbJjb",
								ids,
								function(rtn) {
									if (rtn == null) {
										Ext.MessageBox.show( {
									...
									for ( var i = 0; i < ColumnsPB.length; i++) {
										if (!obj[ColumnsPB[i].name]) {
											obj[ColumnsPB[i].name] = gridRecod
													.get(ColumnsPB[i].name)
										}
									}
									var record = new PlantPB(obj)
									form.loadRecord(record)
									formPanelPB.buttons[0].enable()
									formPanelPB.isNew = false
								})
				//DWREngine.setAsync(false);
				sczbPbQueryMgm.getzcAndzbTime(rq.getFullYear()+"-"+(rq.getMonth()+1)+"-"+rq.getDate(),bcName,currentPid,function(data){
					//alert("data: "+data.length);
					if(data!=null&&data.length>0){
						form.findField("zcName").setValue(data[0]);
						form.findField("zbTime").setValue(data[1]);
					}
				});
				//DWREngine.setAsync(true);
