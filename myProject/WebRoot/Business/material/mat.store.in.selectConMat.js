...
Ext.onReady(function(){
   ...
   btnReturn = new Ext.Button({
	...
	gridPanelMat = new Ext.grid.EditorGridTbarPanel({
		...
		region: 'center',
		title:'合同物资清单',
		tbar:['<font color=#15428b><b>&nbsp;从采购合同选择物资</b></font>','->',{
                    text : '查询',
                    iconCls : 'option',
                    handler : function(){
                        showWindow(gridPanelMat);
                    }
                },
                '-',btnConfirm],
		header : false,
...
function confirmChoose(){
	...
	var chooseCgArr = new Array();
	if(recArr.length >0){
		for(var i=0;i<recArr.length;i++){
			chooseMatArr.push(recArr[i].data.bm);
			chooseCgArr.push(recArr[i].data.cgjhbh)
		}
		matStoreMgm.saveStoreInByCon(appid,chooseMatArr,chooseCgArr,function(dat){
			if(dat){  
