...
var dsB = new Ext.data.GroupingStore({ // 分组
	baseParams : {
		...
		business : businessB,
		method : listMethodB,
		params : " bh in (select bh from com.sgepit.pmis.wzgl.hbm.WzCjhpb w where w.billState='1' and w.pid = '"+CURRENTAPPID+"' ) " +
                    //" and uids not in (select matId from com.sgepit.pmis.material.hbm.MatStoreInsub where pid = '"+CURRENTAPPID+"' ) " +
                    " and hzsl > (select nvl(sum(inNum),0) from com.sgepit.pmis.material.hbm.MatStoreInsub s where s.catNo = bm and s.formId = bh and s.pid = '"+CURRENTAPPID+"') " +
                    " and bm not in (select catNo from com.sgepit.pmis.material.hbm.MatStoreInsub s where s.inId='" + inId + "' and s.pid = '"+CURRENTAPPID+"') " +
                    " and bm not in (select bm from com.sgepit.pmis.wzgl.hbm.ConMat c where bh=c.cgjhbh and c.pid = '"+CURRENTAPPID+"' ) " +
...
cmB.defaultSortable = true;	
	...
	var gridPanelB = new Ext.grid.GridPanel({
		...
		cm : cmB,
		sm : smB,
		tbar:['<font color=#15428b><b>&nbsp;从采购计划选择物资</b></font>','->',{
                    text : '查询',
                    iconCls : 'option',
                    handler : function(){
