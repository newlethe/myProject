...
//			heigth: 30,
			...
			name : 'appState',
			fieldLabel : '<font color=blue>申请状态</font>',
			valueField:'k',
			displayField: 'v',
			mode: 'local',
            typeAhead: true,
			...
			name : 'frameId',
			fieldLabel : '<font color=blue>所在分类</font>',  
			readOnly: true,
			anchor : '95%'
		},'catNo' : {  
			name : 'catNo',
			...
			name : 'warehouse',
			fieldLabel : '<font color=blue>仓库名</font>',  
			readOnly: true,
			anchor : '95%'
		},'wareNo' : {  
			name : 'wareNo',
			fieldLabel : '<font color=blue>货位号</font>',  
			readOnly: true,
			anchor : '95%'
		}
	}
...
//        autoWidth : true,
    	...
    	items: [
                ...
                items:[{
    					...
    					items:[
    							...
    							new fm.TextField(fc['price']),
    							new fm.TextField(fc['appMan']),
    							new fm.ComboBox(fc['appState']),
    ...
    function doFormSave(isNew, dataArr){
    	...
    	if (obj.uuid == '' || obj.uuid == null){
	   		...
	   		//proLinkMgm.insertProLink(obj, function(){
    		maAppMgm.updateMcapp(obj, function(){
   			...
   			/*proLinkMgm.updateProLink(obj, function(){
	   				Ext.example.msg('保存成功！', '您成功修改了一条信息！');
	   				history.back();
	   		});*/
   		}
