...
Ext.onReady(function(){
   ...
   wz_root = new Ext.tree.AsyncTreeNode({
	...
	function onClick(node,e){
		...
		wz_ds.baseParams.params = " flbm like'"+node.getUI().elNode.all('bm').innerText+"%' and bm_state = '1' and  bm not in (select catNo from  MatStoreInsub where inId ='"+inId+"' and "+pidWhereString+" ) and "+pidWhereString+" "
		wz_ds.load({
			params : {
				start : 0,
				limit : 20
...
//-------------------------(已启用)物资编码GRID----------------------------------
	...
	wz_sm =  new Ext.grid.CheckboxSelectionModel({singleSelect:false});
	...
	wz_ds.baseParams.params = "  bm_state = '1' and bm not in(select catNo from MatStoreInsub where inId ='"+inId+"' and inType='清单' and "+pidWhereString+" ) and "+pidWhereString+" "
	...
	function confirmChoose(){
		...
		if(recArr.length >0){
			for(var i=0;i<recArr.length;i++){
	  		...
	  		DWREngine.setAsync(false);   
	    	matStoreMgm.saveMatFrameTree(inId,chooseUidsArr, function(){  
