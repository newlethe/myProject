...
Ext.onReady(function(){
	...
	var billStateArr = [['0','未完成'],['1','已完成'],['-1','处理中'],['2','退回']];
	//--用户userid:realname
	...
	var fc = {
		...
		'reUser':{name:'reUser',fieldLabel:'报销人',anchor:'95%'},
		'reDept':{name:'reDept',fieldLabel:'报销部门',anchor:'95%'},
	...
	var cm = new Ext.grid.ColumnModel([
		...
		{id:'title'       , 	  header:fc['title'].fieldLabel       ,  dataIndex:fc['title'].name},
		...
		{id:'reDept'      ,  	  header:fc['reDept'].fieldLabel      ,  dataIndex:fc['reDept'].name,
			renderer:function(value){
		...
		{id:'reZje'       , 	  header:fc['reZje'].fieldLabel       ,  dataIndex:fc['reZje'].name,type: 'string'},
		{id:'jhje'     , 	  header:fc['jhje'].fieldLabel     ,  dataIndex:fc['jhje'].name,type: 'float'},
		{id:'relj'      , 	  header:fc['relj'].fieldLabel     ,  dataIndex:fc['relj'].name,type: 'float'},
		{id:'reDate'      , 	  header:fc['reDate'].fieldLabel     ,  dataIndex:fc['reDate'].name,renderer: formatDate,type: 'date'},
		...
		{id:'memo1'   , 	  header:fc['memo1'].fieldLabel   ,  dataIndex:fc['memo1'].name,renderer:function(value,cellmeta,record){
			var editflag=false;
			if(record.data.billState==0){editflag=true}
			return "<u style='cursor:hand' onclick=showFileWin('bussiness_deptreimburse','"+record.data.memo+"',"+editflag+",'','')><font color=green>明细附件</font></u>";
		}},
		//'bussiness_deptreimburse',"+record.data.uids+",true,'',new Date()
		{id:'billState'   , 	  header:fc['billState'].fieldLabel   ,  dataIndex:fc['billState'].name,renderer:function(value){
	...
	gridPanel = new Ext.grid.EditorGridTbarPanel({
		...
		tbar : ['<font color=#15428b><B>部门费用报销<B></font>','-',addBtn,editBtn,delBtn],
    ...
    function insertFun(){
    	var url = BASE_PATH+"Business/reimburse/re_info_addorupdate.jsp?isTask=" + isFlwTask + "&bh="+bh_flow;
		window.location.href = url;
    }
    function toEditHandler(){
    	var records = sm.getSelections();var flag = true;
    	if(records.length==0){Ext.example.msg("提示","请选择一条数据！"); return;}
    	for(var i=0;i<records.length;i++){
    		if(records[i].get("billState")==0 || records[i].get("billState")==2){flag=false}
    	}
    	if(flag){
    		Ext.example.msg("提示","流程状态为'新增'或'退回'的数据才能编辑！");
    		return;
    	}
		var record = gridPanel.getSelectionModel().getSelected();
		var url = BASE_PATH+"Business/reimburse/re_info_addorupdate.jsp?isTask=true&uids="+record.get('uids');
		window.location.href = url;
    }
    function DelFun(){
		var records = sm.getSelections();var flag = true;
    	...
    	if(flag){
    		Ext.example.msg("提示","流程状态为'新增'或'退回'的数据才能删除！");
    		return;
    	}
