...
var ds;
var formWin;
var selectorgid;var v_node;var v_id=""
var flag = true;
var dazjWin;
var operate;
var bb=true;
...
Ext.onReady(function() {
	...
	sm, // 第0列，checkbox,行选择器
			 ...
			 }),// 计算行数
			{
				...
				id : 'isremove',
				header : fc['isremove'].fieldLabel,
				dataIndex : fc['isremove'].name,
				renderer:function(value){
					if(value=="1"){return "<font color=red>已移交</font>"}else{return "未移交"}
				}
			},{
				id : 'bzr',
...
//            	history.back();
   ...
   */
	...
	function deleteFun(){
		...
		designMgm.getRowCount(id,function(cn){
			if (0 < cn){
					Ext.Msg.show({
					   title: '提示',
					   msg: '该文件下还有资料,不能删除！',
	...
	function yjzlFun(){
		...
		for(var i=0;i<selectRows.length;i++){
			if(selectRows[i].get('isremove')=="1"){v_flag=true}
		}
		if(v_flag){
			Ext.Msg.show({
					title: '警告',
	...
	function sureDevolve(){
		var selectRows = sm.getSelections();
		for(var i=0;i<selectRows.length;i++){
			v_id+=selectRows[i].get(primaryKey)+"==";
		}
		if(v_node!=""){
	 		DWREngine.setAsync(false);
			designMgm.ZlHandoverDesInfozlOk(v_id,v_node,USERORGID);
			Ext.example.msg('移交成功！', '您成功移交了' + selectRows.length+ '条资料信息！');
			devolveWin.hide();
	 		v_node=""
			ds.load({
				params: {
	...
	function doFormSave(dataArr){
    	...
    	if (obj.infoid == '' || obj.infoid == null){
	   		designMgm.savedesigninfogl(obj, function(){
   			...
   			designMgm.updatedesigninfogl(obj, function(){
