...
Ext.onReady(function() {
	...
	baseMgm.getData("select userid,realname from rock_user where unitid = '"
				...
				var tempCur= new Array();
			    tempCur.push(USERID);
				tempCur.push(REALNAME);
				userArr.push(tempCur); 
	...
	gridPanel = new Ext.grid.EditorGridTbarPanel({
				...
				tbar : ['<font color=#15428b><B>编码申请信息<B></font>', '-'],
	...
	if (ModuleLVL < 3) {
		gridPanel.getTopToolbar().add(editBtn, '-', reBtn);
	}
	...
	function returnFalse() {
		...
		if (record.get('qr') == "-1") {
			...
			Ext.MessageBox.alert("提示", "请选择新申请数据!");
		}
	}
	function returnBM(value) {
		if ("yes" == value) {
			var bh = gridPanel.getSelectionModel().getSelected().get('bh');
			wzbaseinfoMgm.updateWzbmConfirmReturn(bh, function(state) {
						if (state) {
	...
	function toEditHandler() {
		...
		if (record) {
			...
			if (record && record.get('qr') == "-1") {
				var url = BASE_PATH
						+ "Business/wzgl/baseinfo/wz.baseinfo.wzbm.bmwh.confirm.addorupdate.jsp?bh="
						+ record.get('bh');
