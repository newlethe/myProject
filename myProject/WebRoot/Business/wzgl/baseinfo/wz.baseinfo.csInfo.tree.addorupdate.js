...
var pid = new Ext.form.TextField({name:'pid',fieldLabel:'PID',hidden:true,hideLabel:true,value:CURRENTAPPID});
//PID查询条件
...
function toHandlerAdd(){
	...
	if(isRoot){
    fbmt.setValue(rootParent)
	fpmt.setValue(rootText)   
	}else{
...
function saveTree(){
	...
	baseMgm.getData("select bm from wz_csb_type where bm='"+obj['bm']+"' and "+pidWhereString+" ", function(obj) {
		if(""!=obj){
			chekArr[0] = obj[0]
		}else{
			chekArr[0] = "";
