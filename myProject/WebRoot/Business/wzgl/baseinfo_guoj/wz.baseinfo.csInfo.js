...
//-------------------------(启用)供应商GRID----------------------------------
	...
	cm = new Ext.grid.ColumnModel([		// 创建列模型
    	...
    	{id : 'property',header : fc['property'].fieldLabel,dataIndex : fc['property'].name,width: 50,hidden:true}
    	])
    ...
    gridPanel = new Ext.grid.EditorGridTbarPanel({
		...
		tbar : ['<font color=#15428b><B>供应商信息<B></font>','-',addBtn,'-',editBtn,'-'],
	...
	function selFun(){
		var pro = Ext.getCmp('wzpro').getValue();
		ds.baseParams.params =" isused ='1' and  property='"+pro+"'"
		ds.load({
			params : {
				start : 0,
				limit : PAGE_SIZE
   ...
   gridPanel_jy = new Ext.grid.EditorGridTbarPanel({
		...
		saveBtn:false,
		autoScroll : true, // 自动出现滚动条
		tbar : ['<font color=#15428b><B>禁用供应商信息<B></font>','-',addBtn_jy,'-',editBtn_jy,'-'],
		animCollapse : false, // 折叠时显示动画
		loadMask : true, // 加载时是否显示进度
		stripeRows: true,
		viewConfig : {
    ...
    gridPanel.getTopToolbar().add(jyBtn,'-','专业分类：',procob);
    gridPanel_jy.getTopToolbar().add(qyBtn);
    ...
    function insertFun(){
    	if(selectedTreeData==null){
    		...
    		var url = BASE_PATH+"Business/wzgl/baseinfo_guoj/wz.baseinfo.csInfo.addorupdate.jsp?flbm="+selectedTreeData+"&&flmc="+selectedTreeData_text;
    ...
    function toEditHandler(){
    	...
    	if(record){
    		var url = BASE_PATH+"Business/wzgl/baseinfo_guoj/wz.baseinfo.csInfo.addorupdate.jsp?flbm="+selectedTreeData+"&&bm="+record.get('bm')+"&&uids="+record.get('uids');
			window.location.href = url;
    	}
    }
...
/*		if(gridPanel_jy.getSelectionModel().getSelected()){
	    	Ext.MessageBox.confirm("提示","确定启用?",
	    		function(btn){
	    			if(btn == "yes"){
	    				toQy()
...
/*    	var record_jy = gridPanel_jy.getSelectionModel().getSelected();
    	if(record_jy){
    		var uids = record_jy.get('uids');
    		DWREngine.setAsync(false);
    		wzbaseinfoMgm.updateWzCsStateChange(uids,"1",function(state){
    			if(state){
    				Ext.example.msg('提示！', '状态修改成功！');
    				ds_jy.reload();
    				ds.reload();
...
/*    	var record = gridPanel.getSelectionModel().getSelected();
    	if(record){
    		...
    		DWREngine.setAsync(false);
    		wzbaseinfoMgm.updateWzCsStateChange(uids,"0",function(state){
    			if(state){
    				Ext.example.msg('提示！', '状态修改成功！');
    				ds.reload();
    				ds_jy.reload();
