...
//-------------------------(已启用)物资编码GRID----------------------------------
    ...
    gridPanel = new Ext.grid.EditorGridTbarPanel({
		...
		tbar : ['<font color=#15428b><B>物资编码信息<B></font>','-',jyBtn,{
					id: 'query',
					text: '查询',
					tooltip: '查询',
					iconCls: 'option',
...
//-------------------------(禁用)物资编码GRID----------------------------------	
    ...
    gridPanel_check = new Ext.grid.EditorGridTbarPanel({
		...
		saveBtn:false,
		autoScroll : true, // 自动出现滚动条
		tbar : ['<font color=#15428b><B>物资编码信息<B></font>','-',qyBtn_check,'-',addBtn_check,editBtn_check],
		animCollapse : false, // 折叠时显示动画
		loadMask : true, // 加载时是否显示进度
		stripeRows: true,
		viewConfig : {
    ...
    function insertFun(){
    	...
    	if(selectedTreeData==null){
    		...
    		var urls = BASE_PATH+"Business/wzgl/baseinfo_guoj/wz.baseinfo.wzbm.addorupdate.jsp?flbm="+selectedTreeData+"&&bm="+selectedTreeBm+"&&flmc="+selectedTreeData_text;
    ...
    function toEditHandler(){
    	...
    	if(record){
    		var urls = BASE_PATH+"Business/wzgl/baseinfo_guoj/wz.baseinfo.wzbm.addorupdate.jsp?flbm="+selectedTreeData+"&&bm="+record.get('bm')+"&&uids="+record.get('uids');
			Ext.getCmp('outputPanel').html='<iframe name=content src="'+urls+'" frameborder=0 style=width:100%;height:100%;></iframe>';
			openWin.show()
    		//window.location.href = url;
    	}
    ...
    function toEditHandler_check(){
    	...
    	if(record_check){
    		var urls = BASE_PATH+"Business/wzgl/baseinfo_guoj/wz.baseinfo.wzbm.addorupdate.jsp?flbm="+selectedTreeData+"&&bm="+record_check.get('bm')+"&&uids="+record_check.get('uids');
			Ext.getCmp('outputPanel').html='<iframe name=content src="'+urls+'" frameborder=0 style=width:100%;height:100%;></iframe>';
			openWin.show()
    		//window.location.href = url;
    	}
