...
var bm_ = new Ext.form.TextField({fieldLabel: '编码',name: 'bm'});
...
function toHandlerAdd(){
	...
	baseMgm.getData("select max(bm) from wz_ckclb where parentbm='"+f_bmArr[0]+"' and "+pidWhereString+" ", function(obj) {
		if(""!=obj){
			sonkArr[0] = obj[0]
		}else{
			sonkArr[0] = "";
	...
	if(isRoot){
		fbmt.setValue(0)
		fpmt.setValue("物资分类")
		bm_.setValue(0+parseInt(sonkArr[0])+01)
		pm_.setValue("")
	}else{
...
function saveTree(){
	...
	DWREngine.setAsync(false);
	baseMgm.getData("select bm from wz_ckclb where bm='"+obj['bm']+"'", function(obj) {
		if(""!=obj){
			chekArr[0] = obj[0]
		}else{
			chekArr[0] = "";
