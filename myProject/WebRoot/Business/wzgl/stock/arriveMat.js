...
smB, {
	...
	align:"center",
	editor : new Ext.form.NumberField(fcB['sjdj']),
	width :80,
	...
	align:"center",
	editor : new Ext.form.NumberField(fcB['sv']),
	width :40,
	renderer:function(value,cell,record,rowIndex,columnIndex,store)
			{ 
				if(record.get("billType") =="到货"  && record.get("cgbh") == "计划外"){
	...
	editor : new Ext.form.NumberField(fcB['sqsl']),
	align:"center",
	renderer:function(value,cell,record,rowIndex,columnIndex,store)
			{ 
				if(record.get("billType") =="到货"  && record.get("cgbh") == "计划外"){
...
//如果flag=1 标识为最后一条入库数据	
	...
	var re = window.showModalDialog(BASE_PATH + 'Business/wzgl/stock/arriveInput.jsp',obj,sfeature)
	if(re==0){
		if(isFlwTask==true){
		   Ext.Msg.show({
			   title: '保存成功！',
