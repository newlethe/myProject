...
//-------------------------(已启用)物资编码GRID----------------------------------
	...
	wz_cm = new Ext.grid.ColumnModel([		// 创建列模型
    	...
    	{id:'bm',header:wz_fc['bm'].fieldLabel,dataIndex:wz_fc['bm'].name,type:'string',width:60},
    	{id:'pm',header:wz_fc['pm'].fieldLabel,dataIndex:wz_fc['pm'].name,type:'string',width:150},
    	{id:'gg',header:wz_fc['gg'].fieldLabel,dataIndex:wz_fc['gg'].name,type:'string',width:200},
    ...
    wz_gridPanel = new Ext.grid.EditorGridTbarPanel({
		...
		tbar : ['<font color=#15428b><B>库存物资清单<B></font>','-',queryWz,'-'],
  ...
  function selectStorageFun(){
  	...
  	if(selectRows.length==0){
  		...
  		if(g_type=="Arrive"){  	
			DWREngine.setAsync(false);
  			stockMgm.createArriveMatFromStorage(g_conId,wzArr,"",USERID,function(state){
	  			if(state){
	  				Ext.example.msg('提示！', '选择成功！');
