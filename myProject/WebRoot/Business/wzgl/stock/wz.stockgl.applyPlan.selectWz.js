...
Ext.onReady(function(){
   ...
   wz_root = new Ext.tree.AsyncTreeNode({
	...
	function onClick(node,e){
		...
		wz_ds.baseParams.params = " flbm='"+node.getUI().elNode.all('bm').innerText+"' and bm_state = '1' and  bm not in(select bm from  WzCjsxb where bh ='"+selectedData+"' and "+pidWhereString+" ) and "+pidWhereString+" "
		wz_ds.load({
			params : {
				start : 0,
				limit : PAGE_SIZE
...
//-------------------------(已启用)物资编码GRID----------------------------------
	...
	wz_cm = new Ext.grid.ColumnModel([		// 创建列模型
    	...
    	{id:'bmState',header:wz_fc['bmState'].fieldLabel,type:'string',dataIndex:wz_fc['bmState'].name,width:50,hidden:true,
    		renderer:function(value){
    			if("1"==value){
    				return "启用";
    			}else if("-1"==value){
    	...
    	{id:'bm',header:wz_fc['bm'].fieldLabel,type:'string',dataIndex:wz_fc['bm'].name,width:50},
    	{id:'pm',header:wz_fc['pm'].fieldLabel,type:'string',dataIndex:wz_fc['pm'].name,width:50},
    	{id:'gg',header:wz_fc['gg'].fieldLabel,type:'string',dataIndex:wz_fc['gg'].name,width:50},
    	{id:'dw',header:wz_fc['dw'].fieldLabel,type:'string',dataIndex:wz_fc['dw'].name,width:50},
    	...
    	{id:'bz',header:wz_fc['bz'].fieldLabel,dataIndex:wz_fc['bz'].name,width:50},
    	{id:'pid',header:wz_fc['pid'].fieldLabel,dataIndex:wz_fc['pid'].name,value:CURRENTAPPID,hidden:true}
    	])
	...
	wz_ds.baseParams.params = "  bm_state = '1' and bm not in(select bm from  WzCjsxb where bh ='"+selectedData+"' and "+pidWhereString+" ) and "+pidWhereString+" "
	wz_ds.load({
		params : {
			start : 0,
			limit : PAGE_SIZE
  ...
  function showWindow_(){showWindow(wz_gridPanel)};
  ...
  function confirmChoose(){
  	var selectRows = collectionToRecords();
  	if(selectRows.length==0){
  		...
  		stockMgm.wzcjsxbSelectWz(selectedData,wzArr,function(state){
  			if(state){
    ...
    function insertFun(){
        ...
        if(wz_selectedTreeData==null){
            ...
            var urls = CONTEXT_PATH+"/Business/wzgl/baseinfo/wz.baseinfo.wzbm.addorupdate.jsp?flbm="+wz_selectedTreeData+"&flmc="+wz_selectedTreeData_text;
