...
var dsParams = " 1=1 ";
var BillStateDoc = new Array();
var formWin, uploadWin, queryWin;
var INDEXID = '-1', INFOID = '-1';
var addAdjunct,  editAdjunct,  delAdjunct,  btnQuery, btndevolve
var gridPanelDoc
...
Ext.onReady(function(){	
	...
	addAdjunct = new Ext.Button({
		text: '新增',
		iconCls: 'add',
		handler: doAddAdjunct
	});
	
	editAdjunct = new Ext.Button({
		text: '修改',
		iconCls: 'btn',
		handler: doEditAdjunct
	});
	
	delAdjunct = new Ext.Button({
		text: '删除',
		iconCls: 'remove',
		handler: doDelAdjunct
	});
	
	btnQuery = new Ext.Button({
		text: '查询',
		iconCls: 'option',
		hidden:true,
...
//			xtype: 'htmleditor',
	...
	var ds = new Ext.data.Store({
		baseParams: {
			...
			params: dsParams +" and "+pidWhereString
		},
	...
	gridPanelDoc = new Ext.grid.GridPanel({
	...
	if(_isflowView =="flow"){
		...
		delAdjunct.setDisabled(true)
	}
	
	dsParams = " modtabid='"+selectedConId+"' and "+pidWhereString+" ";
	...
	gridPanelDoc.on('celldblclick', function(grid, rowIndex, columnIndex){
	...
	function refreshForm(){
		with (adjunctForm.getForm()){
			...
			findField("pid").setValue(CURRENTAPPID);
	...
	function doDevolve(){
		if (sm.getSelected()&&gridPanelDoc.getSelectionModel().getSelected().data["billstate"]==0){
	...
	var v_node="";
	function sureDevolve(){
		var ids = gridPanelDoc.getSelectionModel().getSelected().data["infoid"];
		newtreePanel.on('click', function(node){
			v_node=node.attributes.indexid
	 });
	 	if(v_node!=""){
	 		DWREngine.setAsync(false);
	 		alert(ids+"=="+v_node+"=="+zlMgm+"=="+zlMgm.ZlHandoverContractOk)
			zlMgm.ZlHandoverContractOk(ids,v_node);
			DWREngine.setAsync(true);
