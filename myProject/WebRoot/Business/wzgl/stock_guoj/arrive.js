...
Ext.onReady(function() {
	...
	*/
	...
	var viewport = new Ext.Viewport({
		...
		listeners: {
			afterlayout: function(){
				if (isFlwView == true){
					gridPanelMain.getTopToolbar().disable();
					gridPanelMat.getTopToolbar().disable();
			    }else if(isFlwTask){
					with(gridPanelMain.getTopToolbar().items){
						get('add').disable();
				    	get('del').disable();
				    }
				}
	...
	if (isFlwTask == true || isFlwView == true){
		ds.baseParams.params = " bh='"+Flowuids+"' and pid='"+CURRENTAPPID+"' ";
    }else{
		ds.baseParams.params = " jhr ='" + USERID + "' and pid='"+CURRENTAPPID+"' ";
    }
	...
	function collectStockFun(){
		...
		var rtn = window.showModalDialog(BASE_PATH + 'Business/wzgl/stock_guoj/selectStock.jsp',obj,"dialogWidth:1024px;dialogHeight:768px;center:yes;resizable:yes;")
	...
	function selectMatFun(btn){
		...
		if(btn.id == "btnSelectCon"){
			...
			/*if(csdm == null || csdm ==""){
				var rtn = window.showModalDialog(BASE_PATH + 'Business/wzgl/stock_guoj/selectStockCon.jsp',csdm,"dialogWidth:1024px;dialogHeight:600px;center:yes;resizable:yes;")
			}*/
			...
			obj.funType = "Arrive"		
			var rtn = window.showModalDialog(BASE_PATH + 'Business/wzgl/stock_guoj/stockConSelectStorage.jsp',obj,"dialogWidth:1024px;dialogHeight:600px;center:yes;resizable:yes;")
			dsB.load({ params:{start: 0, limit: PAGE_SIZE }});
		}
