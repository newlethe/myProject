...
var whereStr = " bill_state='1' and comid not in (select cgbh || ':' || bm from wz_input where pbbh='" + g_arriveBh+ "' and pid = '"+CURRENTAPPID+"')";
	...
	function doSelect(dataArr, mrc, dhbh,fph, handler) {
		...
		Ext.Ajax.request({
			...
			params : {
				...
				dhbh: dhbh,
				fph: fph,
				username: username + new Date().format('ym')
			},
			method : "POST",
			xmlData : dataArr,
