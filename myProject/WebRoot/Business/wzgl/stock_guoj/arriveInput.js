...
function saveForm(valueMonthod){
	...
	zj = form.findField("sqsl").getValue()*form.findField("sjdj").getValue()
	form.findField("jhzj").setValue(zj)
	...
	if(cgbh != "计划外"){	
		...
		for (var i=0;i<store.getTotalCount();i++){
			...
			var curSl = rec.get("sl")*1;
			var curDhsl = rec.get("curDhsl")==null?0: rec.get("curDhsl")*1;
			var curFtsl = rec.get("ftsl")*1;
			
			if(curDhsl + curFtsl > curSl){
				Ext.Msg.alert("提示", "已分摊数量不能大于申请数量！");
				return;
			}
		}
