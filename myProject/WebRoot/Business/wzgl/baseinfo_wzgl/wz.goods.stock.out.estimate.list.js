...
//                temp.push("建管仓库")
    ...
    if(edit_flagLayout == ''){
       whereSqlss = " and judgment_flag='noBody'";
    }else{
       whereSqlss = " and judgment_flag='body'";
    }    
	if(editFlag == 'zgrk' ||  editFlag == 'chrk'){
			var whereSql = " conid='"+conidStr+"' and  pid='" + pid+ "'";
			var whereStr = ""
			if(editFlag == 'zgrk'){
			   whereStr  = " and  out_no not in(select r.out_estimate_no from WZ_GOODS_STOCK_OUT r where "+whereSql+""+whereSqlss+" and out_estimate_no is not null)" 
			}
			var endSql ="select t.uids from WZ_GOODS_OUT_ESTIMATE t where t.finished='1' and t.is_installation='0' "+whereSqlss+" and "+ whereSql+whereStr ;
	...
	if(uidsStr == ""){
		if(editFlag =="zgrk" && equUids != ""){
		    whereDsOut = " and uids in "+equUids + whereSqls;
		    whereDsOutSub = " and out_id in "+equUids;
		}else if(editFlag =="zgrk" && equUids == ""){
            whereDsOut = " and conid ='"+conidStr+"' and finished='1' and out_no not in(select  outEstimateNo from WzGoodsStockOut t )"+whereSqls;
		    whereDsOutSub = " and outId in (select uids from WzGoodsOutEstimate where finished='1' "+whereSqls+" and out_no not in(select  outEstimateNo from WzGoodsStockOut t ))";		
		}else{
		    whereDsOut = " and conid='"+conidStr+"' and finished='1'" +
		    		     " and isInstallation='0' and outNo not in (select outNo from  WzGoodsOutBack )"+whereSqls;
		    var sql = "(select  uids from WzGoodsOutEstimate r where conid='"+conidStr+"' "+whereSqls+" and   finished='1' and isInstallation='0' and outNo not in (select outNo from WzGoodsOutBack)"+")";               
		    whereDsOutSub = " and out_id in "+sql;
		}
	}else{
	     whereDsOut = " and uids='"+uidsStr+"'"+whereSqls;
	...
	dsOut = new Ext.data.Store({
		baseParams: {
	    	...
	    	params: "pid='"+CURRENTAPPID+"'"+whereDsOut+ whereSqls
		},
	...
	var gridPanelOut = new Ext.grid.GridPanel({
		...
		sm : smOut,
		title : '暂估出库单信息',
		tbar : ['<font color=#15428b><B>暂估出库单信息<B></font>','->',estimateBtn,sureBtn,chooseBtn],
		header: false,
	    border: false,
	    //layout: 'fit',
	    region: 'center',
		...
		if (rec == null || rec == "") {
			...
			Ext.MessageBox.confirm('确认', '冲回出库操作将不可恢复，确认要进行吗？', function(btn,text) {
                if(btn == 'yes'){
		            ...
		            DWREngine.setAsync(false);
					wzbaseinfoMgm.resetWzGoodsOutEstimate(uids,function(str){
					     if(str == 'success'){
		    ...
		    Ext.MessageBox.confirm('确认', '冲回出库操作将不可恢复，确认要进行吗？', function(btn,text) {
                if(btn == 'yes'){
					...
					equMgm.getEquNewDhNo(CURRENTAPPID, warehouseNoNo, "out_out_no",
							"wz_goods_out_back", null, function(str) {
								newCkNo = str;
							});
					DWREngine.setAsync(true)
                    wzbaseinfoMgm.insertWzGoodsOutBack(uids,newCkNo,function(str){
                         if(str == "success"){
                              smOut.clearSelections(false);
	                          parent.dsOut.load();
	                          parent.dsOutSub.load();
	                          parent.selectWin.hide(); 
                         }
                    })
                }
	...
	function ChooseFn(){
		    ...
		    if(rec == null || rec==""){
		        ...
		        for(var i=0;i<rec.length;i++){
			        ...
			        var newCkNo = "";
			        if(edit_flagLayout ==''){
			        	newCkNo = prefix +"-"+conmoneyno.replace(/^\n+|\n+$/g,"") + "-CK-";
			        	DWREngine.setAsync(false);
						...
						DWREngine.setAsync(true);
						   ...
						   equMgm.getEquNewDhNoToSb(CURRENTAPPID, newCkNo, "out_No",
									   "wz_goods_stock_out", null,"judgment_flag='body'", function(str) {
										newCkNo = rec[i].get("outNo").substring(0,2)+str;
									});
						DWREngine.setAsync(true);			
			        }
					DWREngine.setAsync(false);
					wzbaseinfoMgm.insertWzFromOutEstimateToOutStock(rec[i].get("uids"),newCkNo,function(str){
					    if(str=='success'){
					    	 parent.ds.baseParams.params = "pid='"+CURRENTAPPID+"' and conid='"+conidStr+"' and treeuids ='"+treeuidsStr+"'";
					         parent.ds.reload();
							 parent.dsOut.reload();
							 parent.selectWin.hide();
					    }else{
