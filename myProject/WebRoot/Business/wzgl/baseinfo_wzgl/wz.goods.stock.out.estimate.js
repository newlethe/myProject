...
Ext.onReady(function(){
    ...
    var fc = {
		...
		'equPartName' : {name : 'equPartName',fieldLabel : '材料名称'},
...
//		}),
	...
	ds = new Ext.data.Store({
		baseParams: {
	    	...
	    	business: business,
	    	method: listMethod,
	    	params: "pid='"+CURRENTAPPID+"'"+ whereSql+ " and makeType='暂估入库'"
		},
...
//	})
    ...
    var gridPanel = new Ext.grid.GridPanel({
		...
		tbar: ['<font color=#15428b><B>库存信息<B></font>','-',
			...
			//'<font color=#15428b>材料名称：</font>', equPartName, '-', 
...
//		}
    ...
    function doPrint(){
        ...
        var filePrintType = "WzStockOutEsView";
   ...
   var  chooseRow = new Ext.form.MultiSelect({
	...
	var cmOutSub = new Ext.grid.ColumnModel([
		...
		,{
			...
			align : 'center',
			hidden : edit_flagLayout !=null?false:true,
			width : 80
		}, {
			id : 'kksNo',
	...
	treePanel.on('click',function(node){
	    if(node != null || node !=''){
	    	  ...
	    	  var whereSqls = "";
	    	  if(edit_flagLayout == ''){
	    	      whereSqls = " and judgment_flag ='noBody'"
	    	  }else{
	    	      whereSqls = " and judgment_flag ='body'"
	    	  }
	          DWREngine.setAsync(false);
		      var sql="select t.uids from wz_goods_out_estimate t where "+sqlWhere + whereSqls;
...
//		}
	...
	function EditHandler(){
		var btnId = this.id;
		var record = smOut.getSelected();
		var url = BASE_PATH+"Business/wzgl/baseinfo_wzgl/wz.goods.stock.out.estimate.addorupdate.jsp"
		...
		if(btnId=="doSelect"){
    		...
    		if (changeRecord==null || changeRecord=="") {
				...
				equMgm.getEquNewDhNo(CURRENTAPPID, newCkNo, "out_No",
						"wz_goods_out_estimate", null, function(str) {
							newCkNo = str;
						});
				...
				obj.uids=ckuids
	   			DWREngine.setAsync(false);
	   			wzbaseinfoMgm.insertWzEstimateOutSubFromStock(OutSubUids,ckuids,newCkNo,function(str){
	    			if(str == "1"){
	    				Ext.example.msg('提示信息','暂估出库单材料选择成功！');
	    				url += "?conid="+selectConid+"&treeuids="+selectUuid+"&uids="+ckuids+"&flagLayout="+edit_flagLayout;
	    			}else{
	    				Ext.example.msg('提示信息','暂估出库单材料选择失败！');
	    			}
	    		});
	    		DWREngine.setAsync(true);
			}else{//修改
				DWREngine.setAsync(false);
	   			wzbaseinfoMgm.insertWzOutEsSubFromStock(OutSubUids,changeRecord.get('uids'),changeRecord.get('outNo'),function(str){
	    			if(str == "1"){
	    				Ext.example.msg('提示信息','暂估出库单材料选择成功！');
	    				url += "?conid="+selectConid+"&treeuids="+selectUuid+"&uids="+changeRecord.get('uids')+"&flagLayout="+edit_flagLayout;
	    			}else{
	    				Ext.example.msg('提示信息','暂估出库单材料选择失败！');
	    			}
	    		});
			...
			url += "?conid="+record.get("conid")+"&treeuids="+record.get("treeuids")+"&uids="+record.get("uids")+"&flag=edit"+"&flagLayout="+edit_flagLayout;
		}
		if(selectWin){
	    	selectWin.destroy();
	    }
		selectWin = new Ext.Window({
			...
			listeners : {
				'close' : function(p){
					...
					var subSql="select s.stock_id,s.equ_type from wz_goods_out_estimate_sub s where s.out_id='"+changeRecord.get('uids')+"'";
					...
					if(cktreeuids!=""){
					    ckStockIds=" and uids not in ("+cktreeuids+")";
					}
					selectUuid=changeRecord.get('treeuids');//根据出库单获得的材料合同分类树主键
					selectConid=changeRecord.get('conid');//根据出库单获得的材料合同主键
					ds.baseParams.params = "pid='"+CURRENTAPPID+"' and conid='"+selectConid+"'" + whereSql;
					ds.reload();
					dsOut.reload();
		...
		Ext.Msg.show({
				...
				fn : function(value) {
					if ("yes" == value) {
						...
						wzbaseinfoMgm.deleteWzEstimateOutAndOutSub(record.get('uids'), function(flag) {
							if ("0" == flag) {
...
function finishOut(uids,finished){
	var record = smOut.getSelected();
	if(record == null || record == ""){
	    Ext.example.msg('提示信息','请选择您要完结的出库单记录！');
	    return;
	}
	if(record.get('outNo') == null || record.get('outNo') == ""){
	    Ext.example.msg('提示信息','出库单记录不完整，不能完结！');
...
//    }
	Ext.Msg.show({
		...
		fn : function(value) {
			if ("yes" == value) {
				DWREngine.setAsync(false);
				wzbaseinfoMgm.wzOutEstimateFinished(uids,function(str){
					if(str == "1"){
