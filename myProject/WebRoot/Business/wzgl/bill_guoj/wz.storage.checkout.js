...
var gridPanel_output,conoveWindow
var insertFlag = 0;
...
//----------------------领料出库单wz_output----------------------------//
	...
	var fc = {
		...
		'kysl':{name:'kysl',fieldLabel:'可用数量'},
		'sqsl':{name:'sqsl',fieldLabel:'领用数量'},
		//'sl':{name:'sl',fieldLabel:'领用数量'},
	...
	var Columns_output = [
		...
		{name:'cz',type:'string'},      {name:'dw',type:'string'},     		{name:'sqsl',type:'float'}, {name:'kysl',type:'float'}, 	{name:'jhdj',type:'float'},         {name:'jhzj',type:'float'}, 
		...
		{name:'sqbm',type:'string'}, {name:'jhdj_sl',type:'float'},
	...
	var cm_output = new Ext.grid.ColumnModel([
		...
		{	id:'billState', header:fc['billState'].fieldLabel,   dataIndex:fc['billState'].name ,
			renderer:function(value){
				if("N"==value){return"审批中"}else if("Y"==value||"1"==value){return "已处理"}
			} },
		...
		{	id:'hth',   	header:fc['hth'].fieldLabel,      	dataIndex:fc['hth'].name,
	   	renderer:function(value,cellmeta,record){
	   		if(record.get("billState")=="Y"||record.get("billState")=="1"){
	   			return value;
	   		}else{
		...
		{	id:'bm',   		header:fc['bm'].fieldLabel,      	dataIndex:fc['bm'].name,readOnly:true,editor:new Ext.form.TextField(fc['pm']),
			width:200,renderer:function(value,cellmeta,record){
				if(record.get("billState")=="Y"||record.get("billState")=="1"){
	   				return value;
	   			}else{
		...
		//{	id:'sqsl',   	header:fc['sqsl'].fieldLabel,      	dataIndex:fc['sqsl'].name,hidden: true},
		{	id:'sqsl',   		header:fc['sqsl'].fieldLabel,      	dataIndex:fc['sqsl'].name,editor:new Ext.form.TextField(fc['sqsl'])},
		{	id:'kysl',   	header:fc['kysl'].fieldLabel,      	renderer:function(value,cellmeta,record,rowIndex,columnIndex,store){
			DWREngine.setAsync(false);
	  		var sql="select nvl(a.kcsl-b.sqsls+c.sqsl-d.zysl-y.sl,0)as kysl from "
					+"(select max(sl)kcsl from wz_bm where bm='"+record.get('bm')+"')a,"
					+"(select nvl(sum(sqsl),0)sqsls from wz_cjsxb where jhbh is null and bh in(select bh from wz_cjspb where bill_state='1') and bm='"+record.get('bm')+"')b,"
					+"(select nvl(sum(sl),0)sqsl from wz_input where bm='"+record.get('bm')+"' and bill_state='N')c,"
					+"(select nvl(sum(sl),0)zysl from wz_kc_zy where bm='"+record.get('bm')+"' and if_zy='1')d," 
					+"(select nvl(sum(z.sl), 0) sl from " +
					"((select * from wz_output where bill_state = '0') union all (select * from wz_output where bill_state = 'N'))z  where z.bm='"+record.data.bm+"')y"
			var returnV=0;
			baseMgm.getData(sql, function(list) {
				if(list<0){returnV=0;}
					else{returnV=list[0]-record.data.sqsl}	
			});
			record.data.kysl = returnV;
			return "<div id='kysl'>"+returnV+"</div>";
			DWREngine.setAsync(true);
		 }
		},
		{	id:'billname',  header:fc['billname'].fieldLabel,   dataIndex:fc['billname'].name,hidden: true  },
		//{	id:'sl',   		header:fc['sl'].fieldLabel,      	dataIndex:fc['sl'].name,editor:new Ext.form.TextField(fc['sl'])},
		...
		{	id:'jhdj_sl',   header:fc['jhdj_sl'].fieldLabel,	dataIndex:fc['jhdj_sl'].name,
			renderer:function(value,cell,record,rowIndex,columnIndex,store){
				record.data.jhdj_sl=record.data.jhdj*record.data.sqsl
				return  record.data.jhdj*record.data.sqsl
			}
		},
	...
	gridPanel_output = new Ext.grid.EditorGridTbarPanel({
		...
		listeners: {
			beforeedit:function(e){
	            ...
	            if (currRecord.get("billState")=="1"||currRecord.get("billState")=="Y")   
	                e.cancel = true;   
	        }
		}
	});
    if (isFlwTask == true || isFlwView == true){
    	...
    	ds.baseParams.params =" bh='"+Flowuids+"' and jhbh='计划外' and  billName='领料出库单' and pid='"+CURRENTAPPID+"' ";
    }else{
		ds.baseParams.params = "lyr='"+USERID+"' and jhbh='计划外' and  billName='领料出库单'  and pid='"+CURRENTAPPID+"' ";
    }
    ...
    sm_output.on("rowselect",function(sm,rowIndex,record){
    ...
    if (isFlwTask == true){
    	...
    	gridPanel_output.getTopToolbar().disable();
    }
...
function saveData(){
	...
	for(var i=0;i<records.length;i++){
		var m_sqsl = records[i].data.sqsl;
		var m_kysl = records[i].data.kysl;
		if(m_kysl<0){flag=true;}
	}
	if(flag==true){
		Ext.MessageBox.alert("提示","申请数量不能大于可用库存数量！");
		return;
	}
	DWREngine.setAsync(true);
