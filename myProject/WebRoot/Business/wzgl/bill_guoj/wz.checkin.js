...
Ext.onReady(function(){
	...
	if(isFlwTask==true ){
		dsB.baseParams.params = " bh='"+isFlwUids+"' and bill_type='计划入库' and bill_state ='N' and pid='"+CURRENTAPPID+"'";
		//dsB.baseParams.params = " bh='"+isFlwUids+"' and bill_state ='N'";
	}else{
		dsB.baseParams.params = " bill_type='计划入库' and bill_state ='N' and pid='"+CURRENTAPPID+"' ";
	}
...
//---------------------------【审批中】---------------------
	...
	if(isFlwView==true){
		dsB_spz.baseParams.params = "bh='"+isFlwUids+"' and bill_type='计划入库' and (bill_state ='Y' or bill_state='-1') and pid='"+CURRENTAPPID+"' ";
		dsB_spz.load({ params:{start: 0, limit: PAGE_SIZE }});
	}
	else{
	dsB_spz.baseParams.params = " bill_type='计划入库' and (bill_state ='Y' or bill_state='-1') and pid='"+CURRENTAPPID+"' ";
	dsB_spz.load({ params:{start: 0, limit: PAGE_SIZE }});
	}
//---------------------------【已入库】---------------------
	...
	if(isFlwView==true){
		dsB_ysp.baseParams.params = "bh='"+isFlwUids+"' and bill_type='计划入库' and bill_state ='1' and pid='"+CURRENTAPPID+"' ";
		dsB_ysp.load({ params:{start: 0, limit: PAGE_SIZE }});
		}
	else{
	dsB_ysp.baseParams.params = " bill_type='计划入库' and bill_state ='1' and pid='"+CURRENTAPPID+"'";
	dsB_ysp.load({ params:{start: 0, limit: PAGE_SIZE }});
	}
//-------------------------
    ...
    if(isFlwView==true){ 	
    	tabs.add(gridPanel_spz)
    	gridPanel_output.disable()
    	//gridPanel_ysp.disable();
    }else{
    	tabs.add(gridPanel_output,gridPanel_spz,gridPanel_ysp)
    }
