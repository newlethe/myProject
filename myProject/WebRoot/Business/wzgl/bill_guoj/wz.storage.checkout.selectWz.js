...
Ext.onReady(function(){
   ...
   wz_root = new Ext.tree.AsyncTreeNode({
	...
	function onClick(node,e){
		...
		baseMgm.getData("select bm,pm ,leaf from wz_ckclb where uids='"+node.getUI().elNode.all('uids').innerText+"' and pid='"+CURRENTAPPID+"' ", function(obj) {
		...
		wz_ds.baseParams.params = " flbm='"+node.getUI().elNode.all('bm').innerText+"' and bm_state = '1' and  bm not in(select bm from  WzCjsxb where bh ='"+selectedData+"') and pid='"+CURRENTAPPID+"' "
		wz_ds.load({
			params : {
				start : 0,
				limit : PAGE_SIZE
...
//-------------------------(已启用)物资编码GRID----------------------------------
	...
	wz_ds.baseParams.params = "  bm_state = '1' and bm not in(select bm from  WzCjsxb where bh ='"+selectedData+"') and pid='"+CURRENTAPPID+"' "
	wz_ds.load({
		params : {
			start : 0,
			limit : PAGE_SIZE
...
/*  		for(var i = 0;i<selectRows.length;i++){
  		...
  		gridPanel_output.getStore().getAt(num_h).set("bm",selectRows.get('bm')); 
  		gridPanel_output.getStore().getAt(num_h).set("gg",selectRows.get('gg')); 
  		gridPanel_output.getStore().getAt(num_h).set("pm",selectRows.get('pm')); 
  		gridPanel_output.getStore().getAt(num_h).set("dw",selectRows.get('dw')); 
  		gridPanel_output.getStore().getAt(num_h).set("jhdj",selectRows.get('jhdj')); 
  		
  		//可以数量
  		DWREngine.setAsync(false);
  		var sql="select nvl(a.kcsl-b.sqsls+c.sqsl-d.zysl,0)kysl from "
				+"(select max(sl)kcsl from wz_bm where bm='"+selectRows.get('bm')+"')a,"
				+"(select nvl(sum(sl),0)sqsls from wz_cjsxb where jhbh is null and bh in(select bh from wz_cjspb where bill_state='1') and bm='"+selectRows.get('bm')+"')b,"
				+"(select nvl(sum(sqsl),0)sqsl from wz_input where bm='"+selectRows.get('bm')+"' and bill_state='N')c,"
				+"(select nvl(sum(sl),0)zysl from wz_kc_zy where bm='"+selectRows.get('bm')+"' and if_zy='1')d";
				
		baseMgm.getData(sql, function(list) {
			if(list<0){
				list=0;
			}
			gridPanel_output.getStore().getAt(num_h).set("kysl",list[0]); 
		});
		DWREngine.setAsync(true);
