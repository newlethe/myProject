/*Ext.onReady(function(){
	var container = new Ext.Panel({
		...
		html: '<iframe name="content1" src="Business/wzgl_guoj/wz.storage.storageOutGrid.jsp" frameborder=0 style="width:100%;height:100%;"></iframe>'
	});
...
var insertFlag = 0;
var jhbh_output;
var PlantInt_fw, selectedData,ds_ysp,ds_fw,bh_fw,gridPanel_output,conoveWindow;
Ext.onReady(function(){
	//--用户userid:realname
	...
	var cjxbdsArr = new Array();
 	...
 	if(isFlwView==true){
 	DWREngine.setAsync(false);
 	storageMgmImpl.getJhbh(flwbh,function(value){  
				jhbh_output=value
    });
 	DWREngine.setAsync(true);
	...
	ds_ysp.baseParams.params = " bill_state=1 and sqr='"+USERID+"' and pid='"+CURRENTAPPID+"' ";
	ds_ysp.load({
		params : {
			start : 0,
			limit : 20
    ...
    if(isFlwView==true){
    ds_ysp.baseParams.params = " bh='"+jhbh_output+"' and pid='"+CURRENTAPPID+"' ";
	ds_ysp.load({
		params : {
			start : 0,
			limit : 20
	...
	var cm_fw = new Ext.grid.ColumnModel([
		...
		{	id:'sqsl_dj',    header:fc_fw['sqsl_dj'].fieldLabel,    dataIndex:fc_fw['sqsl_dj'].name,  width:90, 
			renderer:function(value,cellmeta,record,rowIndex,columnIndex,store){
				record.data.sqsl_dj=record.data.sqsl*record.data.dj;
    ...
    ds_fw = new Ext.data.Store({
		baseParams:{
			...
			business:business,
			method: listMethod,
			params: " bh='"+bh_fw+"' and pid='"+CURRENTAPPID+"' "
		},
		proxy: new Ext.data.HttpProxy({
            method: 'GET',
            url: MAIN_SERVLET
	...
	var fc_output = {
		...
		'rq':{name:'rq',fieldLabel:'日期',format: 'Y-m-d',anchor:'95%',hidden:true,hideLabel:true},
		'bz':{name:'bz',fieldLabel:'备注',anchor:'95%'},
		...
		'gg':{name:'gg',fieldLabel:'规格',anchor:'95%'},
		'cz':{name:'cz',fieldLabel:'材质',anchor:'95%'},
		'dw':{name:'dw',fieldLabel:'计量单位',anchor:'95%',hidden:true,hideLabel:true},
		'sqsl':{name:'sqsl',fieldLabel:'申请数量',anchor:'95%',hidden:true,hideLabel:true},
		'sl':{name:'sl',fieldLabel:'实际领用数量',anchor:'95%'},
		'jhdj':{name:'jhdj',fieldLabel:'单价',anchor:'95%',hidden:true,hideLabel:true},
		'jhzj':{name:'jhzj',fieldLabel:'总价',anchor:'95%',hidden:true,hideLabel:true},
		'jhdj_sl':{name:'jhdj_sl',fieldLabel:'总价',anchor:'95%',hidden:true,hideLabel:true},
		'jhzj':{name:'jhzj',fieldLabel:'总金额',anchor:'95%',hidden:true,hideLabel:true},
		...
		'glfl':{name:'glfl',fieldLabel:'管理费率',anchor:'95%',hidden:true,hideLabel:true},
		'glf':{name:'glf',fieldLabel:'管理费',anchor:'95%',hidden:true,hideLabel:true},
		'stage':{name:'stage',fieldLabel:'期别',anchor:'95%',hidden:true,hideLabel:true},
		'projectId':{name:'projectId',fieldLabel:'工程项目',anchor:'95%',hidden:true,hideLabel:true},
		'khh':{name:'khh',fieldLabel:'开户行',anchor:'95%'},
		'zh':{name:'zh',fieldLabel:'账号',anchor:'95%'},
		'address':{name:'address',fieldLabel:'地址',anchor:'95%',hidden:true,hideLabel:true},
		'dbyj':{name:'dbyj',fieldLabel:'调拨依据',anchor:'95%',hidden:true,hideLabel:true},
		'dh':{name:'dh',fieldLabel:'电话',anchor:'95%'},
		'jsje':{name:'jsje',fieldLabel:'确认出库时的库存金额',anchor:'95%'},
		'stocks':{name:'stocks',fieldLabel:'确认出库时的库存数量',anchor:'95%',hidden:true,hideLabel:true},
		'projectLb':{name:'projectLb',fieldLabel:'项目类别',anchor:'95%',hidden:true,hideLabel:true},
		'sjdj':{name:'sjdj',fieldLabel:'实际单价',anchor:'95%',hidden:true,hideLabel:true},
		'sjzj':{name:'sjzj',fieldLabel:'实际总价',anchor:'95%',hidden:true,hideLabel:true},
		'wonum':{name:'wonum',fieldLabel:'工单编号',anchor:'95%',hidden:true,hideLabel:true},
		'bgdid':{name:'bgdid',fieldLabel:'概算编号',anchor:'95%',hidden:true,hideLabel:true},
		'klsl_btn':{name:'klsl_btn',fieldLabel:'领料操作',anchor:'95%',hidden:true,hideLabel:true},
	...
	var Columns_output = [
		...
		{name:'jhdj_sl',type:'float'},{name:'sl',type:'float'},    	{name:'jhdj',type:'float'},         {name:'jhzj',type:'float'}, 
		...
		{name:'projectLb',type:'string'},        {name:'sjdj',type:'float'}, 
	...
	PlantInt_output = {
		...
		hth:'',jjr:'',bmzg:'',bgr:'',wzzg:'',jhr:'',
		tl:'',ckbh:'',rkbh:'',jhbh:'',bm:'',
		pm:'',gg:'',cz:'',dw:'',
		sqsl:'',sl:'',jhdj:'',jhzj:'',
		ckh:'',billState:'',zdrq:'',
		qrrq:'',lyr:'',sqbm:'',tyfs:'',jhdj_sl:'',
		glfl:'',glf:'',stage:'',projectId:'',
		khh:'',zh:'',address:'',
		dbyj:'',dh:'',jsje:'',stocks:'',
		projectLb:'',sjdj:'',sjzj:'',jhzj:'',jhdj_sl:'',
		wonum:'',bgdid:'',
		pid:CURRENTAPPID
	}
	...
	var cm_output = new Ext.grid.ColumnModel([
		...
		{   id:'bmzg',      header:fc_output['bmzg'].fieldLabel,        dataIndex:fc_output['bmzg'].name,          hidden: true},
		...
		{id:'bgdid',header:fc_output['bgdid'].fieldLabel,dataIndex:fc_output['bgdid'].name,hidden:true},
		{id:'wonum',header:fc_output['wonum'].fieldLabel,dataIndex:fc_output['wonum'].name,hidden:true},
		{id:'sjzj',header:fc_output['sjzj'].fieldLabel,dataIndex:fc_output['sjzj'].name,hidden:true},
		{id:'sjdj',header:fc_output['sjdj'].fieldLabel,dataIndex:fc_output['sjdj'].name,hidden:true},
		{id:'stocks',header:fc_output['stocks'].fieldLabel,dataIndex:fc_output['stocks'].name,hidden:true},
		{id:'jsje',header:fc_output['jsje'].fieldLabel,dataIndex:fc_output['jsje'].name,hidden:true},
		{id:'dh',header:fc_output['dh'].fieldLabel,dataIndex:fc_output['dh'].name,hidden:true},
		{id:'lyr',header:fc_output['lyr'].fieldLabel,dataIndex:fc_output['lyr'].name,hidden:true},
		{id:'projectLb',header:fc_output['projectLb'].fieldLabel,dataIndex:fc_output['projectLb'].name,hidden:true},
		{id:'dbyj',header:fc_output['dbyj'].fieldLabel,dataIndex:fc_output['dbyj'].name,hidden:true},
		{id:'zh',header:fc_output['zh'].fieldLabel,dataIndex:fc_output['zh'].name,hidden:true},
		{id:'khh',header:fc_output['khh'].fieldLabel,dataIndex:fc_output['khh'].name,hidden:true},
		{id:'projectId',header:fc_output['projectId'].fieldLabel,dataIndex:fc_output['projectId'].name,hidden:true},
		{id:'ckh',header:fc_output['ckh'].fieldLabel,dataIndex:fc_output['ckh'].name,hidden:true},
		{id:'cz',header:fc_output['cz'].fieldLabel,dataIndex:fc_output['cz'].name,hidden:true},
		{id:'ckh',header:fc_output['ckh'].fieldLabel,dataIndex:fc_output['ckh'].name,hidden:true},
		{id:'rkbh',header:fc_output['rkbh'].fieldLabel,dataIndex:fc_output['rkbh'].name,hidden:true},
		{id:'ckbh',header:fc_output['ckbh'].fieldLabel,dataIndex:fc_output['ckbh'].name,hidden:true},
		{id:'glfl',header:fc_output['glfl'].fieldLabel,dataIndex:fc_output['glfl'].name,hidden:true},
		{id:'glf',header:fc_output['glf'].fieldLabel,dataIndex:fc_output['glf'].name,hidden:true},
		{id:'stage',header:fc_output['stage'].fieldLabel,dataIndex:fc_output['stage'].name,hidden:true},
		{id:'tyfs',header:fc_output['tyfs'].fieldLabel,dataIndex:fc_output['tyfs'].name,hidden:true},
		{id:'tl',header:fc_output['tl'].fieldLabel,dataIndex:fc_output['tl'].name},
		{ id:'rq',header:fc_output['rq'].fieldLabel,dataIndex:fc_output['rq'].name,hidden:true},
		{ id:'jjr',header:fc_output['jjr'].fieldLabel,dataIndex:fc_output['jjr'].name,hidden:true},
		{ id:'address',header:fc_output['address'].fieldLabel,dataIndex:fc_output['address'].name,hidden:true},
		{	id:'hth',   	header:fc_output['hth'].fieldLabel,      	dataIndex:fc_output['hth'].name,
	   	renderer:function(value){
		...
		{	id:'bz',   		header:fc_output['bz'].fieldLabel,      	dataIndex:fc_output['bz'].name,hidden:true },
		{	id:'jhr', 		header:fc_output['jhr'].fieldLabel,			dataIndex:fc_output['jhr'].name,hidden:true},
		{	id:'bgr',   	header:fc_output['bgr'].fieldLabel,      	dataIndex:fc_output['bgr'].name ,hidden:true},
		{	id:'wzzg',   	header:fc_output['wzzg'].fieldLabel,      	dataIndex:fc_output['wzzg'].name,hidden:true },
		{	id:'bm',   		header:fc_output['bm'].fieldLabel,      	dataIndex:fc_output['bm'].name,hidden:true },
		{	id:'pm',   		header:fc_output['pm'].fieldLabel,      	dataIndex:fc_output['pm'].name,hidden:true },
		{	id:'gg',   		header:fc_output['gg'].fieldLabel,      	dataIndex:fc_output['gg'].name ,hidden:true},
		{	id:'dw',   		header:fc_output['dw'].fieldLabel,      	dataIndex:fc_output['dw'].name,hidden:true},
		{	id:'sqsl',   	header:fc_output['sqsl'].fieldLabel,      	dataIndex:fc_output['sqsl'].name,hidden:true},
		{	id:'billname',  header:fc_output['billname'].fieldLabel,    dataIndex:fc_output['billname'].name,hidden: true  },
		{	id:'sl',   		header:fc_output['sl'].fieldLabel,      	dataIndex:fc_output['sl'].name},
		{	id:'jhdj',   	header:fc_output['jhdj'].fieldLabel,      	dataIndex:fc_output['jhdj'].name},
		{	id:'jhbh',   	header:fc_output['jhbh'].fieldLabel,      	dataIndex:fc_output['jhbh'].name},
		{	id:'jhzj',   	header:fc_output['jhzj'].fieldLabel,      	dataIndex:fc_output['jhzj'].name,
		renderer: function(value,cellmeta,record,rowIndex,columnIndex,store){
					record.data.jhzj=record.data.sl*record.data.jhdj;
					return record.data.sl*record.data.jhdj;
			},hidden: true
		},
		{	id:'jhdj_sl',   	header:fc_output['jhdj_sl'].fieldLabel,      	dataIndex:fc_output['jhdj_sl'].name,
		renderer: function(value,cellmeta,record,rowIndex,columnIndex,store){
					record.data.jhdj_sl=record.data.sl*record.data.jhdj;
					return record.data.sl*record.data.jhdj;
			}
		},
		{	id:'qrrq',   	header:fc_output['qrrq'].fieldLabel,      	dataIndex:fc_output['qrrq'].name,  renderer: formatDate,hidden: true  },
		{	id:'klsl_btn',   header:fc_fw['klsl_btn'].fieldLabel,       dataIndex:fc_fw['sl'].name,  width:90,renderer:showBtn_output    },
		{	id:'pid',   	header:fc_output['pid'].fieldLabel,      	dataIndex:fc_output['pid'].name,hidden:true}
	]);
    ...
    if(isFlwView==true){
    	ds_output.baseParams.params="bh='"+flwbh+"' and pid='"+CURRENTAPPID+"' "
	...
	sm_ysp.on('rowselect',function(sm,rowIndex,record){
	    bh_fw = record.get('bh');
		ds_fw.baseParams.params = " bh='"+bh_fw+"' and pid='"+CURRENTAPPID+"' ";
		...
		if(flwbh!=""){
			...
			ds_output.baseParams.params = "jhbh='"+bh_fw+"' and  billName='领料出库单' and bill_state<>'-1' and bill_state<>'1' and pid='"+CURRENTAPPID+"' ";
			ds_output.load({params:{start:0,limit:PAGE_SIZE}});
		}else{
			ds_output.baseParams.params = "jhbh='"+bh_fw+"' and  billName='领料出库单' and bill_state<>'-1' and bill_state<>'1' and pid='"+CURRENTAPPID+"' ";
			ds_output.load({params:{start:0,limit:PAGE_SIZE}});
			...
			if(btn=='yes'){
				storageMgmImpl.deleteGoods(bh,bm,function(dat){
					if(dat){
						...
						ds_output.baseParams.params = " jhbh='"+bh_fw+"'and billName='领料出库单' and bill_state='N' and pid='"+CURRENTAPPID+"' ";
						ds_output.load({params:{start:0,limit:PAGE_SIZE}});
					}
				});
			}else{return}
...
function getGoodsConfirm(value,bh,bm,flag){
		...
		if(dat){
			...
			ds_output.baseParams.params = " jhbh='"+bh_fw+"' and bm='"+bm_wz+"' and billName='领料出库单' and pid='"+CURRENTAPPID+"' ";
			ds_output.load({params:{start:0,limit:PAGE_SIZE}});
			
			
			ds_ysp.baseParams.params = " bh='"+bh_fw+"' and pid='"+CURRENTAPPID+"' ";
...
function saveFun(){
		var Records = ds_output.getModifiedRecords();
    	for (var i = 0; i < Records.length; i++) {
    		var record = Records[i];
    		var hth_ = record.get('hth');
    		if ("" == hth_){
    			Ext.example.msg('提示', '必填项： 合同 未填写！')
    			return;
    		}
    		else{gridPanel_output.defaultSaveHandler()}
    	}
...
//------------------选择合同
	...
	dsConove.baseParams.params="billstate=2 and pid='"+CURRENTAPPID+"' ";
		...
		conoveWindow = new Ext.Window({
			...
			tbar: [
				{text: '选择', iconCls: 'save', handler: function(){
						...
						if (sm.getSelected()){
							var record = gridPanel_output.getSelectionModel().getSelected();
							var num_h = gridPanel_output.getStore().indexOf(record);
							gridPanel_output.getStore().getAt(num_h).set("hth",sm.getSelected().get('conno')); 
							conoveWindow.hide();
						} else {
