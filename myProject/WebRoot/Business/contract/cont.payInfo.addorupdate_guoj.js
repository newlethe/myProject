...
Ext.onReady(function (){
    		...
    		new Ext.form.FieldSet({
                ...
                items:[{
	   					...
	   					items:[
    						...
    						new fm.TextField(fc['fundsPlanId']),
	   						new fm.ComboBox(fc['cashuse']),
    ...
    var viewport = new Ext.Viewport({
        ...
        items: [
        	formPanel, {
        		region: 'south', height: 180, layout: 'border', 
...
//    realname.getEl().up('.x-form-item').setDisplayed(true); 
    ...
    function getParamsFromList(){
	    ...
	    if(rtn){
		    ...
		    Ext.getCmp('fundsPlanId').setValue(rtn.split("||")[2])
	    }   	
    }
    ...
    function doFormSave(dataArr){
    	...
    	for(var i=0; i<Columns.length; i++) {
    		...
    		if (field) {
	    		...
	    		if(n=="paymoney"){obj[n]=appm-dem;continue}
    ...
    function doFormSave1(dataArr){
    	...
    	for(var i=0; i<Columns.length; i++) {
    		...
    		if (field) {
	    		...
	    		if(n=="planmoney"){planm=field.getValue();;continue}
	    		if(n=="demoney"){dem=field.getValue();continue}
	    		if(n=="paymoney"){obj[n]=appm-dem;paym=appm-dem;continue}
    			obj[n] = field.getValue();
    		}
    	}
    	obj['appmoney']=appm;
    	obj['paytype'] = form.findField('paytype').getRawValue();
    	if(conObj.conpay+obj.appmoney>conObj.convaluemoney){
    	    Ext.Msg.alert('提示信息','合同申请金额与累计已付款之和不能超出合同总金额');
    	    return ;
    	}
    	if(conObj.conpay+obj.passmoney>conObj.convaluemoney){
    	    Ext.Msg.alert('提示信息','合同批准付款金额与累计已付款之和不能超出合同总金额');
    	    return ;
    	}
    	if(conObj.conpay+obj.paymoney>conObj.convaluemoney){
    ...
    function processResult(value){
    	if ("yes" == value){
    		var url = BASE_PATH+"Business/contract/cont.payInfo.addorupdate.jsp?conid="+g_conid+"";
    		//国锦项目单独付款新增页面
    		if(CURRENTAPPID=='1030603') {
	    		var url = BASE_PATH+"Business/contract/cont.payInfo.addorupdate_guoj.jsp?conid="+g_conid+"";
    		}
    ...
    // 下拉列表中 k v 的mapping 
   	...
   	function loadPayTotal(){
		...
		baseDao.findByWhere5(bean, "conid='"+_conid+"'",null,null,null, function(list){
			...
			Ext.getCmp('appmoneyTotal').setValue(cnMoneyToPrec(_total_appmoney));
			Ext.getCmp('passmoneyTotal').setValue(cnMoneyToPrec(_total_passmoney));
			Ext.getCmp('paymoneyTotal').setValue(cnMoneyToPrec(_total_paymoney));
			Ext.getCmp('invoicemoneyTotal').setValue(cnMoneyToPrec(_total_invoicemoney));
		});
	}
...
///////////////////////////////////////////////合同扣款项
    ...
    var conPayCm = new Ext.grid.ColumnModel([
        //conPaySm,
        {
            ...
            align : 'center',
            renderer : function (v){
                for(var i=0;i<deMonStore.getCount();i++){
                    if(deMonStore.getAt(i).get('k')==v){
                        return deMonStore.getAt(i).get('v');
                    }
                }
            },
            width : 250,
    ...
    function selectConPay(){
         conPayTotalDs.reload({callback:function(){
            	Ext.getCmp('demoney').setValue(conPayTotalDs.sum('chargemoney'));
