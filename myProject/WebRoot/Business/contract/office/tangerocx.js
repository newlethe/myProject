 ...
 * xiaos update 带上文件主键
 */
function TANGER_OCX_OpenDoc(url, fileid)
{
	TANGER_OCX_OBJ = document.all('TANGER_OCX');
	if(fileid != "")
	{	
		//alert(url+"&fileid=" + fileid);
	 	try
		{
			TANGER_OCX_OBJ.OpenFromURL(url+"&fileid=" + fileid, false);
		}
		catch(err){
			TANGER_OCX_OBJ.OpenFromURL(url+"&fileid=" + fileid, false);
		}
...
function TANGER_OCX_SetInfo()
{
	...
	try
	{
		...
		//alert("TANGER_OCX_attachURL:=" + TANGER_OCX_attachURL);
