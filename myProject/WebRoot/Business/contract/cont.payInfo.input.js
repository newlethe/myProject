...
var gridPanelTitle = "合同：" + selectedConName + "(合同总金额：￥" + selectconmoney + ")，所有付款记录";
...
//			    		}
	...
	var tbar = [titleBar, '->', btnAdd, '-', btnEdit, '-', btnReturn];
	if(pageType =='showDetail'){//合同结算查看付款明细
		tbar=[];
	}
...
//		}
	...
	if(pageType =='showDetail'){//合同结算查看付款明细
		ds.baseParams.params="conid='"+selectedConId+"'";
	}
	ds.load();
	...
	function renderPayno(value, metadata, record) {
		if(pageType != 'showDetail'){
			...
			output += 'onclick="Ext.get(\'loading\').show();Ext.get(\'loading-mask\').show();';
			output += 'window.location.href=\'' + BASE_PATH
			output += 'Business/contract/cont.payInfo.view.jsp?payid=' + getPayid;
			output += '&conname=' + selectedConName + '&conno=' + selectedConNo
					+ '&conid=' + selectedConId +'&uids='+UIDS+'&conids='+CONIDS+'&optype='+OPTYPE+'&dyView='+dyView+'\'">' + value + '</span>'
			return output;
		}else{
			return value;
		}
	...
	function billTypeRender(value, metadata, record) { // 单据状态类型
		...
		baseDao.findByWhere2(
				...
				"paramvalues like '%payno:" + _payno + "%'", function(
						list) {
					if (list.length > 0) {
						returnStr='<a href="javascript:showFlow(\''
								+ list[0].insid + '\' )">' + str +'</a>'				
		...
		baseDao.findByWhere5(bean, "conid='" + selectedConId + "'", null, null,
				null, function(list) {
					...
					if (selectconmoney == 0) {
						...
						_conPer = (_value_finish / selectconmoney * 100).toFixed(2) + '%';
					}
					if(pageType !='showDetail'){
