...
//    Ext.form.Field.prototype.msgTarget = 'side';
	...
	var fc={
		...
		,'mobile':{name:'mobile',fieldLabel:'手机',allowBlank:false}
...
//						var record = sm.getSelected();
	...
	var cm = new Ext.grid.ColumnModel([		// 创建列模型
    	...
    	, {id:'state',header:fc['state'].fieldLabel,dataIndex:fc['state'].name,align:'center',
	...
	var addBackBtn = new Ext.Button({
		...
		handler : function(){
			...
			if(record){
				var uids = record.data.uids;
				addBackWin.showBackWin(uids,true);
	...
	var tbarArr  = [addBtn,'-',editBtn,'-',delBtn,'-'];
	if(hideBtn){
		tbarArr  = [addBtn,'-'];
	}
	tbarArr.push(addBackBtn,'->','添加时间：',dateField_begin,'至',dateField_end,'-',keyText,keyBtn)
	...
	var formPanel = new Ext.form.FormPanel({
		...
		items : [{
			...
			items : [{
				...
				columnWidth : .5,
				border : false,
				items : [new Ext.form.TextField(fc['mobile'])
