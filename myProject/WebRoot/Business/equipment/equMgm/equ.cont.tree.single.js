...
Ext.onReady(function(){
	...
	var sql = "select distinct t.uids from Equ_Con_Ove_Tree_View t start with t.treeid='01' connect by prior t.treeid =t.parentid";
	DWREngine.setAsync(false);
	baseDao.getData(sql,function(list){
		for(i = 0; i < list.length; i++) {
			notreeuuidstr += ",'"+list[i]+"'";		
		}
	});	
	DWREngine.setAsync(true);
	notreeuuidstr = notreeuuidstr.substring(1);
	...
	treeLoader = new Ext.tree.TreeLoader({
		...
		baseParams: {
			ac:"columntree", 
			treeName:"newEquTypeTreeListSingle", 
			businessName:"equMgm",
			parent:"0",
			pid: CURRENTAPPID,
			conid:""
	...
	function onClick(node, e){
		...
		if(selectParentid == "0"){
            if((typeof isFlwTask != "undefined" || typeof isFlwView != "undefined")&&(isFlwTask == true || isFlwView == true) && flowid != ""){
                ds.baseParams.params = "pid='"+CURRENTAPPID+"' and treeuids not in ("+notreeuuidstr+") and conid in "
                		+"(select conid from EquConOveTreeView  where parentid = '"+selectTreeid+"') and flowid='"+flowid+"' ";
            }else{
                ds.baseParams.params = "pid='"+CURRENTAPPID+"' and treeuids not in ("+notreeuuidstr+") and conid in "
                		+"(select conid from EquConOveTreeView  where parentid = '"+selectTreeid+"' ) ";
            }
