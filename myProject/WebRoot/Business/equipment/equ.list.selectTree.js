...
Ext.onReady(function (){
	...
	if("openbox" == argments){
		baseMgm.getData("select * from equ_sbdh_arr where conid = '"+ conid +"'",function(_list){
			parentList = _list[0].PARENTID;
			rootText = _list[0].SBMC;
		})
	}else{
    ...
    //var zsTitle='<font color=red><b>&nbsp; 总到货数量：' + sumzs(parentList) + '</b></font>';
	...
	function confirmChoose(){
    	...
    	if("getgoods" == argments){
			...
			equlistMgm.saveSelectEquArr(conid,ggid, data,partB, function(){
				treePanel.getEl().unmask();
			});     	    		
    	} else if("openbox" == argments){
			equlistMgm.saveSelectOpen(conid,ggid, data, function(){
				treePanel.getEl().unmask();
			});     	    		
    	}
