   ...
   root = new Ext.tree.AsyncTreeNode({
	...
	function onClick(node,e){
		...
		if(node.text=="中国"){
			...
			ds_jy.baseParams.params = " (flbm='"+bm_+"' or flbm in (select bm from WzCsbType where parentbm='"+bm_+"')) and isused = '0' "
		}else{
			uids = node.getUI().elNode.all('uids').innerText;
			bm_ = node.getUI().elNode.all('bm').innerText
			ds.baseParams.params = " flbm='"+bm_+"' and isused = '1'  and pid='" + CURRENTAPPID + "'"
			ds_jy.baseParams.params = " flbm='"+bm_+"' and isused='0' "
		}
...
//-------------------------(启用)供应商GRID----------------------------------
	...
	cm = new Ext.grid.ColumnModel([		// 创建列模型
    	...
    	{id:'wzorsb',header:fc['wzorsb'].fieldLabel,dataIndex:fc['wzorsb'].name,width:50},
   ...
   gridPanel_jy = new Ext.grid.EditorGridTbarPanel({
    ...
    if (ModuleLVL < 3){
		...
		gridPanel_jy.getTopToolbar().add('-',jyBtn,'-',addBtn_jy,'-',editBtn_jy,'-');
	}	
    ...
    function insertFun(){
    	if(selectedTreeData==null){
    		...
    		var url = BASE_PATH+"Business/equipment/csInfo/wz.baseinfo.csInfo.addorupdate.jsp?flbm="+selectedTreeData+"&&flmc="+selectedTreeData_text;
    ...
    function toEditHandler(){
    	...
    	if(record){
    		var url = BASE_PATH+"Business/equipment/csInfo/wz.baseinfo.csInfo.addorupdate.jsp?flbm="+selectedTreeData+"&&bm="+record.get('bm')+"&&uids="+record.get('uids');
			window.location.href = url;
    	}
    }
    ...
    function toQy(){
    	...
    	DWREngine.setAsync(false);
		equsbcsop.updateWzCsStateChange(uidsStr_qy,"1",function(state){
			if(state){
    ...
    function toJy(value){
    	...
    	if(selectRows.length==0){
	    	...
	    	DWREngine.setAsync(false);
    		equsbcsop.updateWzCsStateChange(uidsStr,"0",function(state){
    			if(state){
    				Ext.example.msg('提示！', '状态修改成功！');
    				ds.reload();
    				ds_jy.reload();
