var beanBack = "com.sgepit.pmis.equipment.hbm.EquGoodsStock"
var business = "baseMgm"
var listMethod = "findWhereOrderby"
var primaryKey = "uids"
var orderColumn = "uids"

var beanOutBack = "com.sgepit.pmis.equipment.hbm.EquGoodsOutBack"
var businessOutBack = "baseMgm"
var listMethodOut = "findWhereOrderby"
var primaryKeyOutBack = "uids"
var orderColumnOutBack = "uids"

var beanOutSubBack = "com.sgepit.pmis.equipment.hbm.EquGoodsOutBackSub"
var businessOutSubBack = "baseMgm"
var listMethodOutSubBack = "findWhereOrderby"
var primaryKeyOutSub = "uids"
var orderColumnOutSub = "uids"

var beanConBack = "com.sgepit.pmis.contract.hbm.ConOve"

var selectTreeid = "";
var selectUuid = "";
var selectConid = "";
var selectParentid = "";
var fileWin;
var selectWin;
var outContentPanel2;
var gridPanelOutBack;
var gridPanelOutBackSub;

var equTypeArr = new Array();
var unitArr = new Array();
var dsBack;
var dsOutBack;
var smOutBack;
var tabPanel;
var dsOutSubBack;
var changeRecord;//更改部件时选中的出库单
var ckStockIds="";//设备主键   用于在库存中过滤出库单已选中的设备
var loadFormRecord = null;//新增出库单  用于选中新增的出库单

var flagaddorupdate=true;
var selTreeuids=new Array();
var equWareArr = new Array();
var pid = CURRENTAPPID;
var businessType = "zlMaterial";
var fcOutBack
var fcOutBackSub

var bdgArr = new Array();

var cmArrayOutBack = [['selectAll','全部']];
var cmHideOutBack = new Array();
var store1OutBack;
var chooseRowOutBack;

var cmArrayOutBackSub = [['selectAll','全部']];
var cmHideOutBackSub = new Array();
var store1OutBackSub;
var chooseRowOutBackSub;

var treeIds = ""

//判断当前用户是否是财务部
//var isFinance = (USERDEPTID == '102010105') ? true : false;

Ext.onReady(function(){
	
		//处理设备仓库下拉框
	    DWREngine.setAsync(false);
	    var typeArr = new Array();
	    baseMgm.getData("select wareno,waretype from equ_warehouse where pid='" + CURRENTAPPID
	                    + "' and parent='01' order by equid ", function(list){
	              for (var i = 0; i < list.length; i++) {
	            var temp = new Array();
	            temp.push(list[i][0]);
	            temp.push(list[i][1]);
	            typeArr.push(temp);
	        }
	    }); 
	    baseMgm.getData("select uids,equid,wareno,waretype from equ_warehouse where pid='" + CURRENTAPPID
	                    + "' order by equid ", function(list) {
	        for (var i = 0; i < list.length; i++) {
	            var temp = new Array();
	            temp.push(list[i][0]);
	            temp.push(list[i][1]);
	            temp.push(list[i][2]);
	            for(var j=0;j<typeArr.length;j++){
	                if(list[i][3] == typeArr[j][1]){
	                    temp.push(typeArr[j][0]);
	                }
	            }
	//            if(list[i][3]=="SBCK")
	//                temp.push("设备仓库");
	//            else if(list[i][3]=="CLCK")
	//                temp.push("材料仓库")
	//            else if(list[i][3]=="JGCK")
	//                temp.push("建管仓库")
		          equWareArr.push(temp);
	           
	        }
	    });
	    
	//设备类型equTypeArr
	appMgm.getCodeValue("设备合同树分类",function(list){
		for(i = 0; i < list.length; i++) {
			if(list[i].propertyCode == "4")
				continue;
			var temp = new Array();
			temp.push(list[i].propertyCode);	
			temp.push(list[i].propertyName);		
			equTypeArr.push(temp);			
		}
	});
	//领用单位
    appMgm.getCodeValue("领用单位",function(list){
        for(i = 0; i < list.length; i++) {
            var temp = new Array();
            temp.push(list[i].propertyCode); 
            temp.push(list[i].propertyName);            
            unitArr.push(temp);         
        }
    });

    baseMgm.getData("select bdgid,bdgname from bdg_info where pid='" + CURRENTAPPID
                    + "' order by bdgid ", function(list) {
        for (var i = 0; i < list.length; i++) {
            var temp = new Array();
            temp.push(list[i][0]);
            temp.push(list[i][1]+" - "+list[i][0]);
            bdgArr.push(temp);
        }
    })
    
    //点击树节点时查询树子节点
    var  treeSql = "select a.uids from (select t.* from equ_con_ove_tree_view t" +
    		" where t.conid = '"+edit_conid+"') a start with a.treeid =" +
    		" (SELECT t.treeid from equ_con_ove_tree_view t where t.uids = '"+edit_treeUids+"'  " +
    		" and a.conid = '"+edit_conid+"') connect by PRIOR a.treeid = a.parentid"
    baseMgm.getData(treeSql,function(str){
        if(str.length ==1){
          treeIds = " and treeuids='"+str+"'"
        }else if(str.length>1){
              treeIds = " and treeuids in ("
             for(var i=0;i<str.length;i++){
                if(i==0){
                   treeIds +="'"+str[i]+"'";
                }else{
                   treeIds +=",'"+str[i]+"'";
                }
             }
             treeIds += ")"
        }
    });     
    
	DWREngine.setAsync(true);
    // 设备仓库系统编码下来框
    var getEquid = new Ext.data.SimpleStore({
        fields : ['k', 'v'],
        data : equWareArr
    });
    //查询表单
    var boxNo = new Ext.form.TextField({
		id: 'boxNo', name: 'boxNo',width : 100
	});
	var equPartName = new Ext.form.TextField({
		id: 'equPartName', name: 'equPartName',width : 100
	});
	var ggxh = new Ext.form.TextField({
		id: 'ggxh', name: 'ggxh',width : 100
	});
	var storage = new Ext.form.TextField({
		id: 'storage', name: 'storage',width : 100
	});
	var doQuery = new Ext.Button({
		text: '查询',
		iconCls: 'btn',
		handler: queHandler
	});
	var doSelect = new Ext.Button({
		id:"doSelect",
		text: '新增',
		iconCls: 'btn',
		handler: EditHandler
	});
//	var editBtn = new Ext.Button({
//		id : 'editBtn',
//		text : '修改',
//		iconCls : 'btn',
//		handler : EditHandler
//	});
	var delBtn = new Ext.Button({
		text : '删除',
		iconCls : 'remove',
		handler : deleHandler
	});
	
	/*******************************库存start************************************************/
    var fc = {
		'uids' : {name : 'uids',fieldLabel : '主键'},
		'pid' : {name : 'pid',fieldLabel : 'PID'},
		'conid' : {name : 'conid',fieldLabel : '合同主键'},
		'treeuids' : {name : 'treeuids',fieldLabel : '设备合同分类树主键'},
		'boxNo' : {name : 'boxNo',fieldLabel : '箱件号'},
		'equType' : {name : 'equType',fieldLabel : '物资类型'},
		'equPartName' : {name : 'equPartName',fieldLabel : '设备名称'},
		'ggxh' : {name : 'ggxh',fieldLabel : '规格型号'},
		'graphNo' : {name : 'graphNo',fieldLabel : '图号'},
		'unit' : {name : 'unit',fieldLabel : '单位'},
		'stockNum' : {name : 'stockNum',fieldLabel : '库存数量',decimalPrecision:4},
		'weight' : {name : 'weight',fieldLabel : '重量（KG）',decimalPrecision:4},
		'storage' : {name : 'storage',fieldLabel : '存放库位'}
	};
	var sm = new Ext.grid.CheckboxSelectionModel();
	var cm = new Ext.grid.ColumnModel([
//		new Ext.grid.RowNumberer({
//			header : '序号',
//			width : 35
//		}),
		sm,
		{
			id : 'uids',
			header : fc['uids'].fieldLabel,
			dataIndex : fc['uids'].name,
		    sortable : true,
			hidden : true
		},{
			id : 'pid',
			header : fc['pid'].fieldLabel,
			dataIndex : fc['pid'].name,
		    sortable : true,
			hidden : true
		},{
			id : 'conid',
			header : fc['conid'].fieldLabel,
			dataIndex : fc['conid'].name,
		    sortable : true,
			hidden : true
		},{
			id : 'treeuids',
			header : fc['treeuids'].fieldLabel,
			dataIndex : fc['treeuids'].name,
		    sortable : true,
			hidden : true
		},{
			id : 'boxNo',
			header : fc['boxNo'].fieldLabel,
			dataIndex : fc['boxNo'].name,
		    sortable : true,
			align : 'center',
			width : 100
		},{
			id : 'equType',
			header : fc['equType'].fieldLabel,
			dataIndex : fc['equType'].name,
		    sortable : true,
			renderer : function(v,m,r){
				var equ = "";
				for(var i=0;i<equTypeArr.length;i++){
					if(v == equTypeArr[i][0])
						equ = equTypeArr[i][1];
				}
				return equ;
			},
			align : 'center',
			width : 100
		},{
			id : 'equPartName',
			header : fc['equPartName'].fieldLabel,
			dataIndex : fc['equPartName'].name,
		    sortable : true,
			align : 'center',
			width : 180
		},{
			id : 'ggxh',
			header : fc['ggxh'].fieldLabel,
			dataIndex : fc['ggxh'].name,
		    sortable : true,
			align : 'center',
			width : 100
		},{
			id : 'graphNo',
			header : fc['graphNo'].fieldLabel,
			dataIndex : fc['graphNo'].name,
		    sortable : true,
			align : 'center',
            hidden : true,
			width : 100
		},{
			id : 'unit',
			header : fc['unit'].fieldLabel,
			dataIndex : fc['unit'].name,
		    sortable : true,
			width : 80
		},{
			id : 'stockNum',
			header : fc['stockNum'].fieldLabel,
			dataIndex : fc['stockNum'].name,
		    sortable : true,
			align : 'right',
			width : 80
		},{
			id : 'weight',
			header : fc['weight'].fieldLabel,
			dataIndex : fc['weight'].name,
		    sortable : true,
			align : 'right',
			width : 80
		},{
			id : 'storage',
			header : fc['storage'].fieldLabel,
			dataIndex : fc['storage'].name,
		    sortable : true,
			renderer : function(v,m,r){
				var storage = "";
				for(var i=0;i<equWareArr.length;i++){
					if(v == equWareArr[i][0])
						storage = equWareArr[i][3]+"-"+equWareArr[i][2];
				}
				return storage;
			},
			align : 'center',
			hidden : true,
			width : 80
		}
	]);
	var Columns = [
		{name:'uids', type:'string'},
		{name:'pid', type:'string'},
		{name:'conid', type:'string'},
		{name:'treeuids', type:'string'},
		{name:'boxNo', type:'string'},
		{name:'equType', type:'string'},
		{name:'equPartName', type:'string'},
		{name:'ggxh', type:'string'},
		{name:'graphNo', type:'string'},
		{name:'unit', type:'string'},
		{name:'stockNum', type:'float'},
		{name:'weight', type:'float'},
		{name:'storage', type:'string'}
	];
	dsBack = new Ext.data.Store({
		baseParams: {
	    	ac: 'list',
	    	bean: beanBack,
	    	business: business,
	    	method: listMethod,
	    	params: "pid='"+CURRENTAPPID+"'"
		},
        proxy: new Ext.data.HttpProxy({
            method: 'GET',
            url: MAIN_SERVLET
        }),
        reader: new Ext.data.JsonReader({
            root: 'topics',
            totalProperty: 'totalCount',
            id: primaryKey
        }, Columns),
        remoteSort: true,
        pruneModifiedRecords: true	
    });
    dsBack.setDefaultSort(orderColumn, 'desc');	//设置默认排序列
//    ds.on('load', function(s, r, o) {
//		s.each(function(rec) {
//			if (collection.get(rec.get("uids"))!=null) {
//				sm.selectRecords([rec], true);
//			}
//		});
//		collection.clear();
//	})
    dsBack.on("beforeload",function(){
    	if(ckStockIds!=""){//过滤出库单已经选中的设备
    		dsBack.baseParams.params+=ckStockIds;
    	}
    	dsBack.baseParams.params+=" and stockNum>=0"
    })
    var gridPanel = new Ext.grid.GridPanel({
    	id:"stock",
		ds: dsBack,
		cm: cm,
		sm:sm,
		title:"库存",
		border: false,
		tbar: ['<font color=#15428b><B>库存信息<B></font>','-',
			'<font color=#15428b>箱件号：</font>', boxNo, '-',
			'<font color=#15428b>设备名称：</font>', equPartName, '-', 
			'<font color=#15428b>规格型号：</font>', ggxh, '-', 
			//'<font color=#15428b>库位：</font>', storage, '-',
			doQuery, '-'],//,doSelect,'-'
		region: 'center',
		header: false, 
		stripeRows: true,
		loadMask: true,
		viewConfig: {
			forceFit: false,
			ignoreAdd: true
		},
		bbar: new Ext.PagingToolbar({
            pageSize: PAGE_SIZE,
            store: dsBack,
            displayInfo: true,
            displayMsg: ' {0} - {1} / {2}',
            emptyMsg: "无记录。"
        })
	});
	/*******************************库存end************************************************/
	/*******************************出库单基础信息start************************************************/
   fcOutBack = {
		'uids' : {	name : 'uids',	fieldLabel : '主键'},
		'pid' : {	name : 'pid',	fieldLabel : 'PID'},
		'conid' : {	name : 'conid',	fieldLabel : '合同主键'},
		'treeuids' : {name : 'treeuids',fieldLabel : '设备合同分类树主键'},
		'finished' : {name : 'finished',fieldLabel : '完结'},
		'isInstallation' : {name : 'isInstallation',fieldLabel : '已安装'},
		'outNo' : {name : 'outNo',fieldLabel : '暂估出库单号'},
		'outOutNo' : {name : 'outOutNo',fieldLabel : '冲回出库单号'},
		'type' : {name : 'type',fieldLabel : '出库类型'},
		'outDate' : {name : 'outDate',fieldLabel : '出库日期'},
		'recipientsUnit' : {name : 'recipientsUnit',fieldLabel : '领用单位'},
		'grantDesc' : {name : 'grantDesc',fieldLabel : '发放描述'},
		'recipientsUser' : {name : 'recipientsUser',fieldLabel : '领用人'},
		'recipientsUnitManager' : {name : 'recipientsUnitManager',fieldLabel : '领用单位负责人'},
		'handPerson' : {name : 'handPerson',fieldLabel : '经手人'},
		'shipperNo' : {name : 'shipperNo',fieldLabel : '出门证编号'},
		'proUse' : {name : 'proUse',fieldLabel : '工程部位（工程项目或用途）'},
		'remark' : {name : 'remark',fieldLabel : '备注'},
        
        'equid' : {name : 'equid', fieldLabel : '仓库号'},
        'fileid' : {name : 'fileid',fieldLabel : '附件'},
        'using' : {name : 'using',fieldLabel : '领料用途'},
        'dataType' : {name : 'dataType',fieldLabel : '领料用途'},
        'equname' : {name : 'equname',fieldLabel : '设备名称'}
	}
	
	smOutBack = new Ext.grid.CheckboxSelectionModel({singleSelect: true});
	
	var cmOut = new Ext.grid.ColumnModel([
		//sm,
		{
			id:'uids',
			header: fcOutBack['uids'].fieldLabel,
			dataIndex: fcOutBack['uids'].name,
		    sortable : true,
			hidden: true
		},{
			id:'pid',
			header: fcOutBack['pid'].fieldLabel,
			dataIndex: fcOutBack['pid'].name,
		    sortable : true,
			hidden: true
		},{
			id:'conid',
			header: fcOutBack['conid'].fieldLabel,
			dataIndex: fcOutBack['conid'].name,
		    sortable : true,
			hidden: true
		},{
			id:'treeuids',
			header: fcOutBack['treeuids'].fieldLabel,
			dataIndex: fcOutBack['treeuids'].name,
		    sortable : true,
			hidden: true
		},{
			id:'finished',
			header: fcOutBack['finished'].fieldLabel,
			dataIndex: fcOutBack['finished'].name,
		    sortable : true,
			renderer : function(v,m,r){
				var o = r.get("isInstallation");
				var str = "<input type='checkbox' "+(v==1?" disabled checked title='已完结' ":"title='未完结'")+" onclick='finishOutBack(\""+r.get("uids")+"\",this)'>"
				return str;
			},
			width : 40
		},{
			id:'isInstallation',
			header:fcOutBack['isInstallation'].fieldLabel,
			dataIndex: fcOutBack['isInstallation'].name,
		    sortable : true,
			hidden: true
		},{
			id:'outNo',
			header: fcOutBack['outNo'].fieldLabel,
			dataIndex: fcOutBack['outNo'].name,
		    sortable : true,
			hidden: true,
			width : 180
		}, {
			id:'outOutNo',
			header: fcOutBack['outOutNo'].fieldLabel,
			dataIndex: fcOutBack['outOutNo'].name,
		    sortable : true,
			width : 250
		}, {
			id:'type',
			header: fcOutBack['type'].fieldLabel,
			dataIndex: fcOutBack['type'].name,
		    sortable : true,
			hidden: true,
			width : 180		
		}, {
			id:'outDate',
			header: fcOutBack['outDate'].fieldLabel,
			dataIndex: fcOutBack['outDate'].name,
		    sortable : true,
			renderer : formatDate,
			align : 'center',
			width : 80
		},{
			id:'recipientsUnit',
			header: fcOutBack['recipientsUnit'].fieldLabel,
			dataIndex: fcOutBack['recipientsUnit'].name,
		    sortable : true,
			renderer : function(v){
				var unit = "";
				for(var i=0;i<unitArr.length;i++){
					if(v == unitArr[i][0])
						unit = unitArr[i][1];
				}
				return unit;
			},
			width : 180
        },{
            id : 'equname',
            header : fcOutBack['equname'].fieldLabel,
            dataIndex : fcOutBack['equname'].name,
		    sortable : true,
            hidden: true,
            width : 180
        }, {
            id:'using',
            header: fcOutBack['using'].fieldLabel,
            dataIndex: fcOutBack['using'].name,
		    sortable : true,
            renderer : function(v){
                var using = "";
                for (var i = 0; i < bdgArr.length; i++) {
                    if (v == bdgArr[i][0])
                        using = bdgArr[i][1];
                }
                return using;
            },
            align : 'center',
            width : 220
		},{
	        id : 'equid',
	        header : fcOutBack['equid'].fieldLabel,
	        dataIndex : fcOutBack['equid'].name,
		    sortable : true,
	        renderer : function(v){
	            var equid = "";
	            for (var i = 0; i < equWareArr.length; i++) {
	                if (v == equWareArr[i][1])
	                    equid = equWareArr[i][3]+" - "+equWareArr[i][2];
	            }
	            return equid;
	        },
	        align : 'center',
	        width : 180
	    },{
	        id : 'fileid',
	        header : fcOutBack['fileid'].fieldLabel,
	        dataIndex : fcOutBack['fileid'].name,
		    sortable : true,
	        renderer : filelistFn,
	        align : 'center',
	        width : 100
	    }, {
			id:'grantDesc',
			header: fcOutBack['grantDesc'].fieldLabel,
			dataIndex: fcOutBack['grantDesc'].name,
		    sortable : true,
            hidden: true,
			width : 180
		},{
			id:'recipientsUser',
			header: fcOutBack['recipientsUser'].fieldLabel,
			dataIndex: fcOutBack['recipientsUser'].name,
		    sortable : true,
            hidden: true,
			width : 160
		},{
			id:'recipientsUnitManager',
			header: fcOutBack['recipientsUnitManager'].fieldLabel,
			dataIndex: fcOutBack['recipientsUnitManager'].name,
		    sortable : true,
            hidden: true,
			width : 160
		},{
			id:'handPerson',
			header: fcOutBack['handPerson'].fieldLabel,
			dataIndex: fcOutBack['handPerson'].name,
		    sortable : true,
            hidden: true,
			width : 160
		},{
			id:'shipperNo',
			header: fcOutBack['shipperNo'].fieldLabel,
			dataIndex: fcOutBack['shipperNo'].name,
		    sortable : true,
            hidden: true,
			width : 160
		},{
			id:'proUse',
			header: fcOutBack['proUse'].fieldLabel,
			dataIndex: fcOutBack['proUse'].name,
		    sortable : true,
            hidden: true,
			width : 160
		},{
			id:'remark',
			header: fcOutBack['remark'].fieldLabel,
			dataIndex: fcOutBack['remark'].name,
		    sortable : true,
            hidden: true,
			width : 180
		},{
			id:'dataType',
			header: fcOutBack['dataType'].fieldLabel,
			dataIndex: fcOutBack['dataType'].name,
		    sortable : true,
            hidden: true,
			width : 180
		}
	]);
	
	var ColumnsOut = [
		{name : 'uids', type : 'string'},
		{name : 'pid', type : 'string'},
		{name : 'conid', type : 'string'},
		{name : 'treeuids', type : 'string'},
		{name : 'finished', type : 'float'},
		{name : 'isInstallation', type : 'float'},
		{name : 'outNo', type : 'string'},
		{name : 'outOutNo', type : 'string'},
		{name : 'type', type : 'string'},
		{name : 'outDate', type : 'date', dateFormat: 'Y-m-d H:i:s'},
		{name : 'recipientsUnit', type : 'string'},
		{name : 'grantDesc', type : 'string'},
		{name : 'recipientsUser', type : 'string'},
		{name : 'recipientsUnitManager', type : 'string'},
		{name : 'handPerson', type : 'string'},
		{name : 'shipperNo', type : 'string'},
		{name : 'proUse', type : 'string'},
		{name : 'remark', type : 'string'}, 
        {name : 'equid', type : 'string'},
        {name : 'fileid', type : 'string'},
        {name : 'using', type : 'string'},
        {name : 'dataType', type : 'string'},
        {name : 'equname', type : 'string'}
	];

	dsOutBack = new Ext.data.Store({
		baseParams: {
	    	ac: 'list',
	    	bean: beanOutBack,				
	    	business: businessOutBack,
	    	method: listMethodOut,
	    	//params: "conid='"+edit_conid+"'"
	    	params: "dataType='"+DATA_TYPE+"' and pid='"+CURRENTAPPID+"' and conid='"+edit_conid+"'"+treeIds
		},
        proxy: new Ext.data.HttpProxy({
            method: 'GET',
            url: MAIN_SERVLET
        }),
        reader: new Ext.data.JsonReader({
            root: 'topics',
            totalProperty: 'totalCount',
            id: primaryKeyOutBack
        }, ColumnsOut),
        remoteSort: true,
        pruneModifiedRecords: true	
    });
    dsOutBack.setDefaultSort(orderColumnOutBack, 'asc');
    
    var printBtn = new Ext.Button({
        text : '打印',
        iconCls : 'print',
        handler : doPrint
    });
    
    function doPrint(){
        var fileid = "";
        var uids = ""
        var modetype = "SB";
        var record = smOutBack.getSelected();
        if(record != null){
            uids = record.get("uids");
        }else{
            Ext.example.msg('提示信息', '请先选择要打印的记录！');
            return;
        }
        //模板参数，固定值，在 系统管理  -> office模板 中配置
        var filePrintType = "EquGoodsOutBackView";
        var sql = "select t.fileid from APP_TEMPLATE  t where t.templatecode='"+filePrintType+"'";
        DWREngine.setAsync(false);
        baseMgm.getData(sql,function(str){
            fileid = str;
        });
        DWREngine.setAsync(true);
        if(fileid == null || fileid == ""){
            Ext.MessageBox.alert("文档打印错误","文档打印模板不存在，请先在系统管理中添加！");
            return;
        }else{
            var docUrl = BASE_PATH + "Business/equipment/equMgm/equ.file.print.jsp?fileid="+fileid;
            docUrl += "&filetype="+filePrintType
            docUrl += "&uids="+uids
            docUrl += "&modetype="+modetype
//            window.open(docUrl)
            window.showModalDialog(docUrl,"","dialogWidth:"+screen.availWidth+"px;dialogHeight:"+screen.availHeight+"px;status:no;center:yes;resizable:yes;Minimize:no;Maximize:yes");
        }
    }

  store1OutBack = new Ext.data.SimpleStore({  
          fields : ['k', 'v'],  
          data : cmArrayOutBack
	}); 
  chooseRowOutBack = new Ext.form.MultiSelect({
         id:   'chooserow6',
         width:  150,
         store : store1OutBack,
         readOnly : true,
         displayField:'v',
         valueField:'k',
         emptyText: '显示更多信息',
         mode: 'local',
         triggerAction : 'all',
         onSelect : function(r,i){
         	var colModel = gridPanelOutBack.getColumnModel();
	    	if(i==0){
		        if(r.get(this.checkField)){
		            chooseRowOutBack.setValue(cmHideOutBack);
		            cmSelectById(colModel,cmHideOutBack);
		        }else{
		            this.selectAll();
		            cmSelectById(colModel,this.getCheckedValue());
		        }
		    }else{
		        r.set(this.checkField, !r.get(this.checkField));
                chooseRowOutBack.setValue(this.getCheckedValue());
                cmSelectById(colModel,this.getCheckedValue());
		    }
		}
  });
 
  function cmSelectById(colModel,str){
    	var cmHide = str.toString().split(',');
    	var lockedCol = colModel.getLockedCount()
        for(var i=lockedCol; i<colModel.getColumnCount();i++){
            for(var j=0;j<cmHide.length;j++){
                if(colModel.getDataIndex(i) == cmHide[j]){
                    colModel.setHidden(i,false);
                    break;
                }else{
                    colModel.setHidden(i,true);
                }
            }
        }
	}   
	      
   gridPanelOutBack = new Ext.grid.GridPanel({
		ds : dsOutBack,
		cm : cmOut,
		sm : smOutBack,
		title : '冲回出库单信息',
		tbar : ['<font color=#15428b><B>冲回出库单信息<B></font>','-',doSelect,'-',delBtn,'-',printBtn,'->',chooseRowOutBack],
		enableHdMenu : false,
		header: false,
	    border: false,
	    //layout: 'fit',
	    region: 'center',
        stripeRows:true,
        loadMask : true,
	    viewConfig: {
	        forceFit: false,
	        ignoreAdd: true
	    },
		bbar: new Ext.PagingToolbar({//在底部工具栏上添加分页导航
            pageSize: PAGE_SIZE,
            store: dsOutBack,
            displayInfo: true,
            displayMsg: ' {0} - {1} / {2}',
            emptyMsg: "无记录。"
        })
	});
	/*******************************出库单基础信息end************************************************/
	/*******************************出库单明细信息start************************************************/
   fcOutBackSub = {
		'uids' : {name : 'uids',fieldLabel : '主键'},
		'pid' : {name : 'pid',fieldLabel : 'PID'},
		'stockId' : {name : 'stockId',fieldLabel : '设备库存主键'},
		'outId' : {name : 'outId',fieldLabel : '出库单主键'},
		'outNo' : {name : 'outNo',fieldLabel : '出库单号'},
		'boxNo' : {name : 'boxNo',fieldLabel : '物资编码'},
		'equType' : {name : 'equType',fieldLabel : '物资类型'},
		'equPartName' : {name : 'equPartName',fieldLabel : '物资名称'},
		'ggxh' : {name : 'ggxh',fieldLabel : '规格型号'},
		'graphNo' : {name : 'graphNo',fieldLabel : '图号'},
		'unit' : {name : 'unit',fieldLabel : '单位'},
		'outNum' : {name : 'outNum',fieldLabel : '出库数量', decimalPrecision : 4},
		'storage' : {name : 'storage',fieldLabel : '存放库位'},
        'price' : {name : 'price', fieldLabel : '单价', allowBlank : false},
        'amount' : {name : 'amount', fieldLabel : '金额', allowBlank : false}
	};
	var smOutSub = new Ext.grid.CheckboxSelectionModel();
	var cmOutSub = new Ext.grid.ColumnModel([
		new Ext.grid.RowNumberer({
			header : '序号',
			width : 35
		}),
		{
			id : 'uids',
			header : fcOutBackSub['uids'].fieldLabel,
			dataIndex : fcOutBackSub['uids'].name,
		    sortable : true,
			hidden : true
		},{
			id : 'pid',
			header : fcOutBackSub['pid'].fieldLabel,
			dataIndex : fcOutBackSub['pid'].name,
		    sortable : true,
			hidden : true
		},{
			id : 'stockId',
			header : fcOutBackSub['stockId'].fieldLabel,
			dataIndex : fcOutBackSub['stockId'].name,
		    sortable : true,
			hidden : true
		},{
			id : 'outId',
			header : fcOutBackSub['outId'].fieldLabel,
			dataIndex : fcOutBackSub['outId'].name,
			hidden : true
		},{
			id : 'outNo',
			header : fcOutBackSub['outNo'].fieldLabel,
			dataIndex : fcOutBackSub['outNo'].name,
		    sortable : true,
			hidden : true
		},{
			id : 'boxNo',
			header : fcOutBackSub['boxNo'].fieldLabel,
			dataIndex : fcOutBackSub['boxNo'].name,
		    sortable : true,
			align : 'center',
			width : 100
		},{
			id : 'equType',
			header : fcOutBackSub['equType'].fieldLabel,
			dataIndex : fcOutBackSub['equType'].name,
		    sortable : true,
			renderer : function(v,m,r){
				var equ = "";
				for(var i=0;i<equTypeArr.length;i++){
					if(v == equTypeArr[i][0])
						equ = equTypeArr[i][1];
				}
				return equ;
			},
			align : 'center',
            hidden : true,
			width : 100
		},{
			id : 'equPartName',
			header : fcOutBackSub['equPartName'].fieldLabel,
			dataIndex : fcOutBackSub['equPartName'].name,
		    sortable : true,
			align : 'center',
			width : 180
		},{
			id : 'ggxh',
			header : fcOutBackSub['ggxh'].fieldLabel,
			dataIndex : fcOutBackSub['ggxh'].name,
		    sortable : true,
			align : 'center',
			width : 100
		},{
			id : 'graphNo',
			header : fcOutBackSub['graphNo'].fieldLabel,
			dataIndex : fcOutBackSub['graphNo'].name,
		    sortable : true,
			align : 'center',
            hidden : true,
			width : 100
		},{
			id : 'unit',
			header : fcOutBackSub['unit'].fieldLabel,
			dataIndex : fcOutBackSub['unit'].name,
		    sortable : true,
            align : 'center',
			width : 80
		},{
			id : 'outNum',
			header : fcOutBackSub['outNum'].fieldLabel,
			dataIndex : fcOutBackSub['outNum'].name,
		    sortable : true,
			align : 'right',
			width : 80
        },{
            id : 'price',
            header : fcOutBackSub['price'].fieldLabel,
            dataIndex : fcOutBackSub['price'].name,
		    sortable : true,
            align : 'right',
            width : 80
        },{
            id : 'amount',
            header : fcOutBackSub['amount'].fieldLabel,
            dataIndex : fcOutBackSub['amount'].name,
		    sortable : true,
            align : 'right',
            width : 80
        },
			{
			id : 'stockNum',
			header : "库存数量",
			dataIndex:'stockNum',
			align : 'right',
			renderer:function(value,cell,record){
				var stocknum="";
				DWREngine.setAsync(false);
				equMgm.getStockNumFromStock(record.get('stockId'),function(num){
					stocknum=num;
				});
				DWREngine.setAsync(true);
    			return stocknum;
			},
		    sortable : true,
			width : 80
		}
		,{
			id : 'storage',
			header : fcOutBackSub['storage'].fieldLabel,
			dataIndex : fcOutBackSub['storage'].name,
		    sortable : true,
			renderer : function(v,m,r){
				var storage = "";
				for(var i=0;i<equWareArr.length;i++){
					if(v == equWareArr[i][0])
						storage = equWareArr[i][3]+"-"+equWareArr[i][2];
				}
				return storage;
			},
			align : 'center',
			hidden : true,
			width : 80
		}
	]);
	var ColumnsOutSub = [
		{name:'uids', type:'string'},
		{name:'pid', type:'string'},
		{name:'stockId', type:'string'},
		{name:'outId', type:'string'},
		{name:'outNo', type:'string'},
		{name:'boxNo', type:'string'},
		{name:'equType', type:'string'},
		{name:'equPartName', type:'string'},
		{name:'ggxh', type:'string'},
		{name:'graphNo', type:'string'},
		{name:'unit', type:'string'},
		{name:'outNum', type:'float'},
        {name:'price', type:'float'},
        {name:'amount', type:'float'},
		{name:'storage', type:'string'}
	];
	dsOutSubBack = new Ext.data.Store({
		baseParams: {
	    	ac: 'list',
	    	bean: beanOutSubBack,
	    	business: businessOutSubBack,
	    	method: listMethodOutSubBack,
	    	params:  ' 1=2 '  //"pid='"+CURRENTAPPID+"' and out_id in (select uids from  EquGoodsStockOut where conid='"+edit_conid+"' and treeuids='"+edit_treeUids+"')"
		},
        proxy: new Ext.data.HttpProxy({
            method: 'GET',
            url: MAIN_SERVLET
        }),
        reader: new Ext.data.JsonReader({
            root: 'topics',
            totalProperty: 'totalCount',
            id: primaryKeyOutSub
        }, ColumnsOutSub),
        remoteSort: true,
        pruneModifiedRecords: true	
    });
    dsOutSubBack.setDefaultSort(orderColumnOutSub, 'desc');	//设置默认排序列
        dsOutBack.on('load', function(s, r, o) {
		s.each(function(rec) {
			if(loadFormRecord!=null){//选中新增的出库单
				if (rec.get("uids")==loadFormRecord.get('uids')) {
					smOutBack.selectRecords([rec], true);
				}
		    }
		});
	})
 
  store1OutBackSub = new Ext.data.SimpleStore({  
          fields : ['k', 'v'],  
          data : cmArrayOutBackSub
	}); 
  chooseRowOutBackSub = new Ext.form.MultiSelect({
         id:   'chooserow7',
         width:  150,
         store : store1OutBackSub,
         readOnly : true,
         displayField:'v',
         valueField:'k',
         emptyText: '显示更多信息',
         mode: 'local',
         triggerAction : 'all',
         onSelect : function(r,i){
         	var colModel = gridPanelOutBackSub.getColumnModel();
	    	if(i==0){
		        if(r.get(this.checkField)){
		            chooseRowOutBackSub.setValue(cmHideOutBackSub);
		            cmSelectByIdSub(colModel,cmHideOutBackSub);
		        }else{
		            this.selectAll();
		            cmSelectByIdSub(colModel,this.getCheckedValue());
		        }
		    }else{
		        r.set(this.checkField, !r.get(this.checkField));
                chooseRowOutBackSub.setValue(this.getCheckedValue());
                cmSelectByIdSub(colModel,this.getCheckedValue());
		    }
		}
  });
 
  function cmSelectByIdSub(colModel,str){
    	var cmHide = str.toString().split(',');
    	var lockedCol = colModel.getLockedCount()
        for(var i=lockedCol+1; i<colModel.getColumnCount();i++){
            for(var j=0;j<cmHide.length;j++){
                if(colModel.getDataIndex(i) == cmHide[j]){
                    colModel.setHidden(i,false);
                    break;
                }else{
                    colModel.setHidden(i,true);
                }
            }
        }
	}   
	
   gridPanelOutBackSub = new Ext.grid.GridPanel({
		ds: dsOutSubBack,
		cm: cmOutSub,
		sm:smOutSub,
		title:"出库单详细信息",
		tbar : ['->',chooseRowOutBackSub],
		enableHdMenu : false,
		border: false,
		region: 'south',
		header: false, 
		height : document.body.clientHeight*0.4,
		stripeRows: true,
		loadMask: true,
		viewConfig: {
			forceFit: true,
			ignoreAdd: true
		},
		bbar: new Ext.PagingToolbar({
            pageSize: PAGE_SIZE,
            store: dsOutSubBack,
            displayInfo: true,
            displayMsg: ' {0} - {1} / {2}',
            emptyMsg: "无记录。"
        })
	});
	/*******************************出库单明细信息end************************************************/
	var gridOutPanel = new Ext.Panel({
		id:"stockOutBack",
		layout:'border',
		region: 'center',
		items : [gridPanelOutBack,gridPanelOutBackSub]
	});

	
	outContentPanel2 = new Ext.Panel({
		id: 'outContentPanel2',
		layout:'border',
		region: 'center',
		title:'冲回出库',
		items : [gridOutPanel]
	});

	if(USERDEPTID == "102010103"){
		outContentPanel2.disable();
	}	
//	ds.load({params:{start:0,limit:PAGE_SIZE}});
//	dsOutBack.load({params:{start:0,limit:PAGE_SIZE}});
	
//	tabPanel.on('tabchange',function(t,tab){
//		if(t.activeTab.id=="stock"){
//			sm.clearSelections();
//		}
//	});
	if(ModuleLVL>=3){
		   delBtn.setDisabled(true);	
		   doSelect.setDisabled(true);
		}	     
	smOutBack.on('rowselect',function(){
		var record = gridPanelOutBack.getSelectionModel().getSelected();
		if(ModuleLVL>=3){
		   delBtn.setDisabled(true);	
		   doSelect.setDisabled(true);
		}else{
			if(record.get('finished') == 1){
	//			editBtn.setDisabled(true);
				delBtn.setDisabled(true);
	            printBtn.setDisabled(false);
			}else{
	//			editBtn.setDisabled(false);
				delBtn.setDisabled(false);
	            printBtn.setDisabled(false);
			}			
		}
		dsOutSubBack.baseParams.params = "outId = '"+record.get('uids')+"'";
		dsOutSubBack.load({params:{start:0,limit:PAGE_SIZE}});
	});
	
	//模糊查询
	function queHandler(){
		var querywheres="";
		var qboxNo = boxNo.getValue();
		var qequPartName = equPartName.getValue();
		var qggxh = ggxh.getValue();
		var qstorage = storage.getValue();
		if ('' != qboxNo){
			querywheres += " and boxNo like '%"+qboxNo+"%'";
		}
		if ('' != qequPartName){
			querywheres += " and equPartName like '%"+qequPartName+"%'";
		}
		if ('' != qggxh){
			querywheres += " and ggxh like '%"+qggxh+"%'";
		}
		if ('' != qstorage){
			querywheres += " and storage like '%"+qstorage+"%'";
		}
		if(selectParentid!=null&&selectParentid!=""){
			if(selectParentid == "0"){
				dsBack.baseParams.params = "pid='"+CURRENTAPPID+"' and conid='"+selectConid+"'"+querywheres;
				dsBack.reload();
			}else{
					//查询当前选中节点的所有子节点主键。
					var sql = "select a.uuid from ( select t.* from equ_type_tree t " +
							" where t.conid = '"+selectConid+"' ) a start with a.treeid = "+
							" (SELECT t.treeid from equ_type_tree t where t.uuid = '"+selectUuid+"' " +
							" and a.conid = '"+selectConid+"') connect by PRIOR  a.treeid =  a.parentid";
					var treeuuidstr = "";
					DWREngine.setAsync(false);
					baseDao.getData(sql,function(list){
						for(i = 0; i < list.length; i++) {
							treeuuidstr += ",'"+list[i]+"'";		
						}
					});	
					DWREngine.setAsync(true);
					treeuuidstr = treeuuidstr.substring(1);
					dsBack.baseParams.params = "pid='"+CURRENTAPPID+"' and conid='"+selectConid+"' and treeuids in ("+treeuuidstr+")"+querywheres;
					dsBack.reload();
			}
		}else{
			dsBack.baseParams.params = "dataType='"+DATA_TYPE+"' and pid='"+CURRENTAPPID+"'"+querywheres;
		    dsBack.reload();
		}
	};
	//选择操作
	function EditHandler(){
		var btnId = this.id;
		var record = smOutBack.getSelected();
		var url = BASE_PATH+"Business/equipment/equMgm/equ.goods.stock.out.estimate.list.jsp"
//		if(selectUuid == "" || selectConid == ""){
//			Ext.example.msg('提示信息','请先选择左边的合同分类树！');
//	    	return ;
//		}
//		var records = sm.getSelections();
//    		if(records == null || records.length == 0){
//    			Ext.example.msg('提示信息','请先选择库存中的设备！');
//    			return;
//    		}
//    	var OutSubUids = new Array()
//		var sbType=records[0].get('equType');
//		for (var i = 0; i < records.length; i++) {
//			if(records[i].get('equType')!=sbType){
//				Ext.example.msg('提示信息','请先选择库存中相同设备类型的设备！');
//			    return;
//			}
//			OutSubUids.push(records[i].get("uids"));
//		}	
        url += "?conid="+edit_conid+"&treeuids="+edit_treeUids+"&edit_flag=chck&dataType="+DATA_TYPE;
		if(selectWin){
	    	selectWin.destroy();
	    }
		selectWin = new Ext.Window({
			id:'selectwin',
			width: 950,
			height: 500,
			modal: true, 
			plain: true, 
			border: false, 
			resizable: false,
			closeAction :"hide",
			html:"<iframe id='equOut' src='' width='100%' height='100%' frameborder='0'></iframe>",
			listeners : {
				'close' : function(p){
					flagaddorupdate=false;
					changeRecord=smOutBack.getSelected();
//					tabPanel.hideTabStripItem(1);
					var subSql="select s.stock_id,s.equ_type from equ_goods_out_back_sub s where s.out_id='"+changeRecord.get('uids')+"'";
					var tempequtype="";//已经选择的设备类型
					var cktreeuids="";//已经选择的设备主键
					DWREngine.setAsync(false);
					baseDao.getData(subSql,function(list){
						for(i = 0; i < list.length; i++) {
							cktreeuids += ",'"+list[i][0]+"'";		
						    tempequtype=list[i][1];
						}
					});	
					DWREngine.setAsync(true);
					cktreeuids = cktreeuids.substring(1);
					if(cktreeuids!=""){
					    ckStockIds=" and uids not in ("+cktreeuids+") and equ_type='"+tempequtype+"'";
					}
					selectUuid=changeRecord.get('treeuids');//根据出库单获得的设备合同分类树主键
					selectConid=changeRecord.get('conid');//根据出库单获得的设备合同主键
					var sql = "select a.uuid from ( select t.* from equ_type_tree t " +
						" where t.conid = '"+selectConid+"' ) a start with a.treeid = "+
						" (SELECT t.treeid from equ_type_tree t where t.uuid = '"+selectUuid+"' " +
						" and a.conid = '"+selectConid+"') connect by PRIOR  a.treeid =  a.parentid";
					var treeuuidstr = "";
					DWREngine.setAsync(false);
					baseDao.getData(sql,function(list){
						for(i = 0; i < list.length; i++) {
							treeuuidstr += ",'"+list[i]+"'";
							selTreeuids.push(list[i]);
						}
					});	
					DWREngine.setAsync(true);
					treeuuidstr = treeuuidstr.substring(1);
					dsBack.baseParams.params = "pid='"+CURRENTAPPID+"' and conid='"+selectConid+"' and treeuids in ("+treeuuidstr+")";
					dsBack.reload();
					dsOutBack.reload();
					
				},
				'hide':function(t){
					flagaddorupdate=true;
					if(selectTreeid==""){
					   selectUuid="";
					   selectConid="";
					}
					changeRecord=null;
					loadFormRecord=null;
					var templength=selTreeuids.length;
					selTreeuids.splice(0,templength);
					ckStockIds="";
					dsOutBack.reload();
					dsOutSubBack.reload();
					queHandler();
				},
				'show' : function(){
					equOut.location.href  = url;
				},
				'beforeshow':function(){
//					tabPanel.unhideTabStripItem(1);
//					tabPanel.setActiveTab(1);
				    dsOutBack.reload();
			}
		}
	    });
		selectWin.show();

	};
	function deleHandler(){
		var record = smOutBack.getSelected();
		Ext.Msg.show({
				title : '提示',
				msg : '是否删除该出库单及其出库详细信息？',
				buttons : Ext.Msg.YESNO,
				icon : Ext.MessageBox.QUESTION,
				fn : function(value) {
					if ("yes" == value) {
						gridPanelOutBack.getEl().mask("loading...");
						equMgm.deleteOutBackAndOutBackSub(record.get('uids'), function(flag) {
							if ("0" == flag) {
								Ext.example.msg('删除成功！',
										'您成功删除了该出库单信息！');
								dsOutBack.reload();
								dsBack.reload();
								if((dsOutBack.getTotalCount()-1)>0){
									smOutBack.selectRow(0);
								}else{
									dsOutSubBack.removeAll();
								}
							} else{
								Ext.Msg.show({
									title : '提示',
									msg : '数据删除失败！',
									buttons : Ext.Msg.OK,
									icon : Ext.MessageBox.ERROR
								});
							}
							gridPanelOutBack.getEl().unmask();
						});
					}
				}
			});
	}
	function formatDate(value){
        return value ? value.dateFormat('Y-m-d') : '';
    };
    
    	        //附件 
    function filelistFn(value, metadata, record){
		    	        var uidsStr = record.get('uids')
						var downloadStr="";
						var billstate = record.get('finished');
						var count=0;
						var editable = true;
						DWREngine.setAsync(false);
				        db2Json.selectData("select count(file_lsh) as num from sgcc_attach_list where transaction_id='"+uidsStr+
				                           "' and transaction_type='"+businessType+"'", function (jsonData) {
					    var list = eval(jsonData);
					    if(list!=null){
					   	 count=list[0].num;
					     		 }  
					      	 });
					    DWREngine.setAsync(true);
						if(billstate == 1){
						   downloadStr="附件["+count+"]";
						   editable = false;
						}else{
						   downloadStr="附件["+count+"]";
						   editable = true;
						}	
						return '<div id="sidebar"><a href="javascript:showUploadWin(\''
									+ businessType + '\', ' + editable + ', \''
									+ uidsStr
									+ '\', \''+'到货单附件'+'\')">' + downloadStr +'</a></div>'
					
			}
});
function finishOutBack(uids,finished){
	if(!isFinance){
        Ext.example.msg('提示信息','当前用户不是财务部用户，不能进行完结操作！');
        finished.checked = false;
        return;
    }
	
	Ext.Msg.show({
		title : '提示',
		msg : '完结后不可取消，不可编辑，确认要完结吗？',
		buttons : Ext.Msg.YESNO,
		icon : Ext.MessageBox.QUESTION,
		fn : function(value) {
			if ("yes" == value) {
				DWREngine.setAsync(false);
				equMgm.equOutBackFinished(uids,function(str){
					if(str == "1"){
						Ext.example.msg('提示信息','出库单完结操作成功！');
						finished.checked = true;
						dsOutBack.reload();
					}else if(str == "2"){
						Ext.example.msg('提示信息','该出库单已经开始安装，不能取消完结！');
						finished.checked = false;
					}else{
						Ext.example.msg('提示信息','操作出错！');
						finished.checked = false;
					}
				});
				DWREngine.setAsync(true);
			}else{
				finished.checked = false;
			}
		}
	});
}
//显示多附件的文件列表
function showUploadWin(businessType, editable, businessId, winTitle) {
	if (businessId == null || businessId == '') {
		Ext.Msg.show({
					title : '上传文件',
					msg : '请先保存记录再进行上传！',
					buttons : Ext.Msg.OK,
					icon : Ext.MessageBox.WARNING
				});
				return;
	}

	var title = '上传文件';
	if (winTitle) {
		title = winTitle;
	}

	fileUploadUrl = CONTEXT_PATH
			+ "/jsp/common/fileUploadMulti/fileUploadSwf.jsp?openType=url&businessType="
			+ businessType + "&editable=" + editable + "&businessId="
			+ businessId;
	var fileWin = new Ext.Window({
				title : title,
				width : 600,
				height : 400,
				minWidth : 300,
				minHeight : 200,
				layout : 'fit',
				closeAction : 'close',
				modal : true,
				html : "<iframe name='fileFrame' src='"
						+ fileUploadUrl
						+ "' frameborder=0 style='width:100%;height:100%;'></iframe>"
			});
	fileWin.show();
	fileWin.on("close",function(){
    dsBack.load({
			params : {
				start : 0,
				limit : PAGE_SIZE,
				params :" orgid='" + USERORGID + "' and indexid in (select indexid from ZlTree) and  (billstate=2 or billstate=3 or billstate=1 or billstate=0) "
			} 
		});
	});
}
