...
Ext.onReady(function(){
	    ...
	    baseMgm.getData("select equid,equno from equ_warehouse where pid='" + CURRENTAPPID
	    ...
	    baseMgm.getData("select uids,equid,equno,waretype from equ_warehouse where pid='" + CURRENTAPPID
	...
	//                temp.push("建管仓库")
	 ...
	 var adnormalWin = new Ext.Window({
			width : 900,
			height : 450,
			tbar : ['<font color=#15428b><B>异常信息<B></font>','->',adnormalBtn],
			modal: true, 
			plain: true, 
			border: false, 
			resizable: false,
...
//		           	 store: getArray,
		...
		var Columns = [
			...
			{name : 'warehouseInType', type:'string'},
			{name : 'equBodysUids', type : 'string'},
        ...
        var newRkNo="";	
//		DWREngine.setAsync(false);
//        var getSql = "select warehouse_no from equ_goods_storein_estimate t where conid='"+edit_conid+"' and treeUids ='"+edit_treeuids+
//                     "' and warehouse_no=(select max(warehouse_no)  from equ_goods_storein_estimate t where conid = '"+edit_conid+"' and treeUids = '"+edit_treeuids+"')";
...
//	        DWREngine.setAsync(true);
//			//处理暂估入库检验单编号
//		   newRkNo = prefix+"-"+conmoneyno+"-暂估入库-";
////		}
//		DWREngine.setAsync(false);
//		equMgm.getEquNewDhNo(CURRENTAPPID,newRkNo,"warehouse_no","equ_goods_storein_estimate",null,function(str){
//			newRkNo = str;
//		});
//		DWREngine.setAsync(true);
        //新设备仓库分类树
...
//	         }
    ...
    getJoinUnitCom.on('select',function(node){
		...
		DWREngine.setAsync(false);
		equMgm.getEquNewDhNoToSb(CURRENTAPPID,newRkNo,"warehouse_no","equ_goods_storein_estimate",null,"data_type='EQUBODY'",function(str){
			newRkNo = str;
		});
		DWREngine.setAsync(true);
...
//	           	triggerAction: 'all'
			...
			'warehouseName' : {name : 'warehouseName',fieldLabel : '设备名称'+requiredMark /*'部件名称'*/,allowBlank : false},
			'ggxh' : {name : 'ggxh',fieldLabel : '规格型号'+requiredMark,allowBlank : false},
			'unit' : {name : 'unit',fieldLabel : '单位'},
			'inWarehouseNo' : {name : 'inWarehouseNo',fieldLabel : '数量'+requiredMark,allowBlank : false,decimalPrecision : 4},
		    ...
		    'equno' : {
			            ...
			            listeners : {
			                 blur : function(v,r,m){
//			                 	  for(var i=0;i<equWareArr.length;i++){
//			                 	       if(Ext.getCmp('equno').getValue()==equWareArr[i][0]){
////			                 	           alert("equWareArr[i]="+equWareArr[i])
//			                 	       }
//			                 	  }
			                 }}
					     },
                         
	        'stockno' : {name : 'stockno', fieldLabel : '存货编码', allowBlank : true,readOnly:true},
		...
		newtreePanel.on('click',function(node,e){
				...
				if(elNode.all('isleaf').innerText =='0'){
					   ...
					   for(var i=0;i<equWareArr.length;i++){
						   if (node.id == equWareArr[i][1]){
						   	     equnoComboBox.setRawValue(equWareArr[i][2]+" - "+equWareArr[i][1])
...
/*        //获取单号前缀。前缀获取说明：在属性代码中配置“单号前缀”
        var prefix = "";
        alert("edit_uids="+edit_uids)
        var sql = "select t.waretypecode,t.warenocode from equ_warehouse t where  t.waretype='SBCK' " +
        		  " and t.waretypecode='SB' and  equid=(select equid from EQU_GOODS_STOREIN_ESTIMATE where uids='"+edit_uids+"')";
        DWREngine.setAsync(false);
        baseMgm.getData(sql, function(str){
		...
		newRkNo = prefix;
		DWREngine.setAsync(false);
		equMgm.getEquNewDhNo(CURRENTAPPID,newRkNo,"stockno","EQU_GOODS_STOREIN_ESTIMATE_SUB",null,function(str){
			newRkNo = str;
		});
		DWREngine.setAsync(true);	*/    
	  ...
	  var addGridBtn = new Ext.Button({
		...
		var equBodyWin = new Ext.Window({
			...
			tbar : ['<font color=#15428b><B>主体设备选择<B></font>','->',chooseEquBodyBtn],
			modal: true, 
			plain: true, 
			border: false, 
			resizable: false,
...
//        });
		...
		var gridTopBar = gridPanelSub.getTopToolbar()
			with(gridTopBar){
				add(selectFromEquBodyBtn,'->',chooseRow);
			}  
		for(var o in fcSub){
	        var name = fcSub[o];
	        var temp = new Array();
		...
		function selectFromEquBodyFun(){
			...
			dsEquBody.load({params:{start:0,limit:PAGE_SIZE}});
			equBodyWin.show();
		}
       //明细单新增
		function insertFn(){
...
//			}
		...
		var getEquid1 = formPanel.getForm().findField("equid").getValue();
		if(getEquid1 == null || getEquid1 == ""){
			...
			var getBoxNo = "";
			DWREngine.setAsync(false);
	        baseMgm.getData("select t.equid,t.waretypecode,t.warenocode from equ_warehouse t where t.equid='"+getEquid1+"'",function(str){
	             if(str.length>0){
	        ...
	        DWREngine.setAsync(true);
	        DWREngine.setAsync(false);
			equMgm.getEquNewDhNo(CURRENTAPPID,getBoxNo,"stockno","equ_goods_storein_estimate_sub",null,function(str){
		...
		function doFormSave(){
		    	...
		    	if (obj.uids == '' || obj.uids == null){
			   		equMgm.saveOrUpdataEquGoodsStoreinEstimateSub(obj, function(str){
			   		       if(str == 'success'){
	  ...
	  //form表单保存
	   function saveFun(){
                ...
                if(equid == null || equid == ""){
		     	...
		     	form.findField('equBodysUids').setValue(edit_partUids);
				...
				for (var i = 0; i < Columns.length; i++) {
		      	...
		      	DWREngine.setAsync(false);
				equMgm.saveOrUpdataEquZGRkGoodsStorein(obj,pid,edit_uids,function(text){
				            if(text == 'success'){
				...
				DWREngine.setAsync(true);
				DWREngine.setAsync(false);
				baseMgm.getData("select uids from equ_goods_storein_estimate " +
						        "where conid='"+getConid+"' and  warehouse_no='"+getWarehouseNo+"' and pid='"+pid+"' and data_type='"+dataType+"'",function(text){
		...
		function delGridFun(){
			 ...
			 Ext.Msg.confirm('系统提示','是否要删除记录',function(btn){
			           if(btn=='yes'){
			           		 DWREngine.setAsync(false);
							 equMgm.checkExceBoxEstimate(getUids,getPid,function(text){
							     if(text=='success'){
		...
		function showNoticeWin(){
			...
			dsNotice.baseParams.params = "open_no not in (select noticeNo from EquGoodsStorein t " +
		    			                 ...
		    			                 " and finished='1' and isStorein <>1 and pid='"+pid+"' and conid='"+edit_conid+"'" +
		    			                 " and treeuids in "+treeuidsStr+"" 
			dsNotice.load();
			noticeWin.show();
...
//			}
            ...
            if(records == null || records.length == 0){
            	...
            	if(getGgxhNull.length>0){
            	    ...
            	    Ext.Msg.alert('信息提示','您选择的主体设备有以下：</br>'+'<span style="color:red;">'+getMsg+'</span>【规格型号】为空，请填写完整保存后选择' +
            	    		'</br>或选择规格型号不为空的记录!')
            	    return;
            	}
            	Ext.Msg.confirm('系统提示','是否确定选择？',function(btn){
            		if(btn == 'yes'){
							DWREngine.setAsync(false);
							equMgm.getEsSubFromEquGoodsBody(getUids,sbrkUids,function(str){
							   if(str=="success"){
        ...
        */
                ...
                if(records == null || records.length == 0){
                    ...
                    for (var i = 0; i < records.length; i++) {
                        ...
                        DWREngine.setAsync(false);
                        equMgm.selectEquCheckToEquInEstimate(uidsArr,getUids,function(text){
	...
	//入库单明细删除按钮重构
		function deleteFn(){
		    ...
		    if(record == null || record==""){
		    	...
		    	var updateSql = "update Equ_Goods_Bodys set del_or_update='1' where equ_no not in (" +
		    			        " select stockno from ( select stockno from EQU_GOODS_STOREIN_ESTIMATE_SUB t where  t.uids<>'"+
		    			         record[0].data.uids+"' union select stockno from EQU_GOODS_STOREIN_SUB where sbrk_uids in ( " +
		    			         		" select uids from EQU_GOODS_STOREIN t where t.data_type='EQUBODY'))) " +
