...
Ext.onReady(function(){
	...
	baseMgm.getData("select uids,equno,equid  from equ_warehouse where pid='" + CURRENTAPPID
    ...
    var fcOut = {
        ...
        'using' : {
            ...
            allowBlank : editBody!=""?false:true, 
            valueField: 'k',
            displayField: 'v',
            mode: 'local',
            typeAhead: true,
        ...
        'dataType' : {name : 'dataType',fieldLabel : '数据类型',  width : 160},
...
//				}
	...
	var fcOutSub = {
        ...
        'price' : {id:'price', name : 'price', fieldLabel : '入库单价'+requiredMark, allowBlank : false},
        'amount' : {id:'amount',name : 'amount', fieldLabel : '出库金额'+requiredMark, allowBlank : false},
        'kcMoney' : {name : 'kcMoney', fieldLabel : '库存金额'+requiredMark},
        'kksNo' : {name : 'kksNo',fieldLabel : 'KKS编码',  width : 160,allowBlank : getFlag = "write"?false:true},
        'useParts' : {name : 'useParts',fieldLabel : '安装部位',  width : 160,allowBlank : getFlag = "write"?false:true}
	};
...
//			hidden : true,
   ...
   if(editFlag ==""){
    ...
    function saveOut(){
        var form = formPanel.getForm();
        var checkBlank = ['recipientsUnit','outNo','equid']
        if(editBody != ""){
...
//        }
        ...
        if(countUseParts >0 && (editBody != "")){
            Ext.example.msg('提示信息','安装部位为必填项，请填写后在保存！');
            return;
        }
        if(countKksNo >0 && (editBody != "")){
		...
		if(records.length!=0){
			...
			if(flag){
			    for(var i=0;i<records.length;i++){
	    			...
	    			var newstockNum=oldstocknum+oldoutNum-records[i].get('outNum');
	    			DWREngine.setAsync(false);
	    			  equMgm.updateStockNum(newstockNum,records[i].get('stockId'),function(){
	...
	function deleteOutSub(){
		...
		DWREngine.setAsync(false);
		  equMgm.updateStockNum(newstockNum,record.get('stockId'),function(){
