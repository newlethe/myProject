...
{
	/**
	*Customize config isWorkMaterialType-是否是材料报告类似结构
	*/
	isWorkMaterialType : false,
	rowId_field_name : "",
	/**
	...
	getActiveNodePageOffset : function()
	{
		...
		if (!this.isWorkMaterialType && this.active_node) {
...
Ext.ux.maximgb.treegrid.GridView = Ext.extend(Ext.grid.GridView, 
{
	...
	if(this.grid.getStore().isWorkMaterialType){	
		ts.master = new Ext.Template(
				'<div class="x-grid3" hidefocus="true">',
					'<div class="x-grid3-viewport">',
						'<div class="x-grid3-header">',
  ...
  renderCellTreeUI : function(record, store)
  {
	...
	expandRow : function(record, initial)
	{
		...
		if (typeof record == 'number') {
			...
			if(ds.rowId_field_name != ""){
				index = ds.find(ds.rowId_field_name,record.get(ds.rowId_field_name))
			}else{
				index = ds.indexOf(record);
			}
	...
	{
		if(this.grid.store.isWorkMaterialType){
			return;
		}
		var ts = this.templates;
		this.breadcrumbs_el.update(
...
Ext.ux.maximgb.treegrid.GridPanel = Ext.extend(Ext.grid.GridPanel, 
{
	...
	initComponent : function()
	{
		...
		if(!this.store.isWorkMaterialType){
   ...
   */
	...
	onTreeGridSelectionChange : function(sm, selection)
	{
		...
		if (sm.getSelected) {
			...
			if(!this.getStore().isWorkMaterialType)
...
Ext.ux.maximgb.treegrid.PagingToolbar = Ext.extend(Ext.PagingToolbar,
{
  ...
  getPageData : function()
  {
	...
	updateUI : function()
	{
		if(this.store.isWorkMaterialType) return;
